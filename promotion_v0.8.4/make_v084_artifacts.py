#!/usr/bin/env python3
"""Generate the v0.8.4 promotion artifacts EXPLICITLY.
...
CSV_OUT = os.path.join(ROOT, "promotion_v0.8.4", "diff_v083_to_v084.csv")
LOG_OUT = os.path.join(ROOT, "promotion_v0.8.4", "regression_log_v084.txt")

RESYNC = {52: "TTU", 65: "UNC", 68: "STAN", 101: "MOST", 124: "SJSU"}
CORRECT = {91: "TULN", 123: "NIU"}


def classify(sheet, coord, row):
    if (sheet, coord) == ("START HERE", "A1"):
        return "DOCUMENTATION — version identifier and confidence census"
    if row in RESYNC:
        return f"QB DATA — RESYNC from live production master ({RESYNC[row]})"
    if row in CORRECT:
        return f"QB DATA — CANDIDATE/CONFIDENCE CORRECTION ({CORRECT[row]})"
    ...
    for s in a.sheetnames:
        ...
        for r in range(1, max(wa.max_row, wbk.max_row) + 1):
            for c in range(1, max(wa.max_column, wbk.max_column) + 1):
                ...
                if x != y:
                    changed.append((s, wbk.cell(row=r, column=c).coordinate, x[1], y[1], r))
    assert len(changed) == 45, f"expected 45 changed cells, found {len(changed)}"
    with open(CSV_OUT, "w", newline="") as fh:
        w = csv.writer(fh)
        w.writerow(["sheet", "cell", "classification", "old", "new"])
        for s, coord, o, n, row in changed:
            w.writerow([s, coord, classify(s, coord, row),
                        "" if o is None else o, "" if n is None else n])
    print(f"wrote {CSV_OUT} ({len(changed)} rows)")

    r = subprocess.run([sys.executable, os.path.join(ROOT, "promotion_v0.8.4", "verify_v084.py")],
                       capture_output=True, text=True)
    with open(LOG_OUT, "w") as fh:
        fh.write(r.stdout)
        if r.stderr:
            fh.write("\nSTDERR\n" + r.stderr)
        fh.write(f"\nverify_v084.py exit code: {r.returncode}\n")
