...
- `C` Favorite → the **team abbreviation**, e.g. `UGA` (never a bare
  ...
  `M-OH`)
- `D` Spread → always a **positive number**, regardless of whether the
  favorite is the home team or the away team — never type a negative
  spread
- `E` Total
...
- `A` Type, `B` Target (abbrev or GameID), `C` Value (points, must be a
  number), `D` Reason (**required** — leave nothing here blank),
  `H` Active = `Y`
Keep single adjustments **small** (a few points). As of v0.6.1, a row
with a missing reason, a non-numeric value, or an oversized value (over
4 points, unless it's a MARGIN OVERRIDE or TOTAL OVERRIDE type) is
automatically excluded from the model — column `K` shows why. Still
review flagged rows yourself; the flag tells you *what's* wrong, not
...
still QB UNCERTAIN). **Note:** the QB UNCERTAIN count can read 0 even
while real QB problems exist, if those same games are also missing a
market line — PENDING LINE outranks QB UNCERTAIN, so a game in that
state gets counted as PENDING LINE, not QB UNCERTAIN, until a line is
entered. Enter the line first, then re-check this count. Then open
**DASHBOARD** and work games in Priority order (highest first). A game's
`Status` column tells you exactly what's blocking it, in this order —
the first one that applies is the one you'll see:
...
can fix. (DATA INCOMPLETE is a known status name but currently can't
actually appear — a documented, unrepaired defect, not something you
did wrong if you never see it.)
