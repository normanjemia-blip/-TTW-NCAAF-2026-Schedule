...
## Classification correction flagged (Phase 8.2) → re-verified and REJECTED (Phase 8.3)

- **Vanderbilt (VAN):** Phase 8.2 flagged VAN's L / "open competition" as a
  possible misclassification, recommending reclassification to **Medium** on
  the assumption that Jared Curtis was the consistently projected,
  uncontested Day-1 starter and that the Low was driven **solely** by
  true-freshman youth.
- **Phase 8.3 re-verification (2026-07-22) REJECTS that recommendation.**
  Fresh sourcing — HC **Clark Lea at Vanderbilt's SEC Media Days session
  (2026-07-21)** — confirms a **genuine, coach-acknowledged open
  competition** between Curtis and **Blaze Berlowitz**, who is a **credible
  veteran** (two seasons in OC Tim Beck's system, Diego Pavia's 2025 backup),
  not a mere backup. Lea **declined to name a starter**, said the battle may
  run to the opener, and indicated both QBs could play. VAN therefore **fails**
  both "consistently projected Day-1 starter" and "uncontested by a genuine
  equal competitor." The Low is **not** youth-driven; it reflects a real
  unresolved competition.
- **Disposition:** **VAN stays L / UNCERTAIN — NOT reclassified, NOT
  initialized.** See `vanderbilt_verification_note.md`. It remains in the
  open-competition group with `eligible_for_zero_init = N` and a
  `next_research_date` of 2026-08-03 (fall camp / HC availability).
