#!/usr/bin/env python3
"""Generate the v0.8.5 promotion artifacts EXPLICITLY.
...
CSV_OUT = os.path.join(ROOT, "promotion_v0.8.5", "diff_v084_to_v085.csv")
LOG_OUT = os.path.join(ROOT, "promotion_v0.8.5", "regression_log_v085.txt")
...
def classify(sheet, coord, row):
    ...
    if row in ACTIVATED:
        return f"QB DATA — ACTIVATION ({ACTIVATED[row]})"
    if row in CORRECT:
        return f"QB DATA — RECORD CORRECTION ({CORRECT[row]})"
...
def main():
    ...
    assert len(changed) == 33, f"expected 33 changed cells, found {len(changed)}"
    ...
    r = subprocess.run([sys.executable, os.path.join(ROOT, "promotion_v0.8.5", "verify_v085.py")],
                       capture_output=True, text=True)
    with open(LOG_OUT, "w") as fh:
        fh.write(r.stdout)
        if r.stderr:
            fh.write("\nSTDERR\n" + r.stderr)
        fh.write(f"\nverify_v085.py exit code: {r.returncode}\n")
