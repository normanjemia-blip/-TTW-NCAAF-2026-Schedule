...
def build_master(files, teams, conferences, coordinators, qbs, abbreviations):
    lines = [
        ...
        "| 9 | Betting Concepts (29 entries) + Statistics Reference | "
    ...
    lines.append(
        ...
        "The guide states a win total three separate ways — the conference table's "
        "DraftKings number (138 teams), the team page's standalone Over/Under line "
        "(138 teams), and Steve Makinen's feature on pp. 22–27 (29 bets). The "
        "database keeps all three apart, so start from the question rather than "
        "from a team:\n\n"
        "| Question | Start here |\n| --- | --- |\n"
        "| *Every Makinen Over* | [00_OVER_INDEX.md](../06_Win_Totals/00_OVER_INDEX.md) — 14 |\n"
        "| *Every Makinen Under* | [00_UNDER_INDEX.md](../06_Win_Totals/00_UNDER_INDEX.md) — 15 |\n"
        "| *All 29 bets, and all 138 posted totals* | "
        "[00_FEATURE_PICKS.md](../06_Win_Totals/00_FEATURE_PICKS.md) · "
        "[00_ALL_TEAMS.md](../06_Win_Totals/00_ALL_TEAMS.md) |\n"
        "| *His strongest stated win-total opinions* | "
        "[00_FEATURE_PICKS.md](../06_Win_Totals/00_FEATURE_PICKS.md) → field 7 of "
        "each record, which records a confidence level only where he states one |\n"
        "| *QB-dependent win totals* | "
        "[00_DEPENDENCY_INDEX.md](../06_Win_Totals/00_DEPENDENCY_INDEX.md) → "
        "quarterback-dependent |\n"
        "| *Coaching-dependent win totals* | "
        "[00_DEPENDENCY_INDEX.md](../06_Win_Totals/00_DEPENDENCY_INDEX.md) → "
        "coaching-dependent |\n"
        "| *Schedule-dependent win totals* | "
        "[00_DEPENDENCY_INDEX.md](../06_Win_Totals/00_DEPENDENCY_INDEX.md) → "
        "schedule-dependent (roster and portal dependencies are listed there too) |\n"
        "| *Where several parts of the guide agree* | "
        "[00_AGREEMENT_INDEX.md](../06_Win_Totals/00_AGREEMENT_INDEX.md) |\n"
        "| *Where the guide contradicts itself* | "
        "[00_DISAGREEMENT_INDEX.md](../06_Win_Totals/00_DISAGREEMENT_INDEX.md) · "
        "[00_SOURCE_CONFLICTS.md](../06_Win_Totals/00_SOURCE_CONFLICTS.md) |\n"
        "| *Win total against Makinen's power rating* | "
        "[00_WINTOTAL_VS_POWER.md](../06_Win_Totals/00_WINTOTAL_VS_POWER.md) — "
        "**TTW DERIVED** |\n"
        "\nTwo cautions carry with this database. The agreement index records that "
        "parts of the guide point the same way; it is **not** a confidence grade, a "
        "score, or a model input, and the guide agreeing with itself is not "
        "independent evidence that it is right. The disagreement and source-conflict "
        "indexes are reproduced **unresolved** — 11 teams where the feature and the "
    ...
    lines.append(
        ...
        "Three connected reference layers. `11_Betting_Concepts` explains the "
        "ideas, `14_Statistics_Reference` holds the numbers, and "
        "`13_Situational_Angles` covers game context.\n\n"
        "**Read the source class before you quote anything here.** Concept "
        "pages carry two, separated by heading and never blended in one "
        "claim: *how the guide defines it* and *how the guide uses it* are "
        "**GUIDE CONTENT**; *working definition* and *why it matters* are "
        "**TTW DERIVED** — this library's words, supplied because the guide "
        "supplies none.\n\n"
        "| Question | Start here |\n| --- | --- |\n"
        "| *What does the guide mean by a term?* | "
        "[`11_Betting_Concepts`](../11_Betting_Concepts/README.md) — 29 "
        "concepts, one entry each |\n"
        "| *Is this the guide's definition or TTW's?* | every concept page "
        "separates them by heading; the table in "
        "[the README](../11_Betting_Concepts/README.md) marks which of the "
        "29 the guide defines at all |\n"
        "| *What does an abbreviation mean?* | "
        "[00_GLOSSARY.md](../11_Betting_Concepts/00_GLOSSARY.md) — all 45 "
        "p. 2 entries, wholly GUIDE CONTENT |\n"
        "| *Concepts the guide uses but never defines* | "
        "[00_GAPS.md](../11_Betting_Concepts/00_GAPS.md) — **21 of the 29** |\n"
        "| *Team statistics, values and national ranks* | "
        "[`14_Statistics_Reference`](../14_Statistics_Reference/README.md) — "
        "[offense](../14_Statistics_Reference/00_OFFENSE.md) · "
        "[defense](../14_Statistics_Reference/00_DEFENSE.md) |\n"
        "| *Who leads each statistical category* | "
        "[00_LEADERS.md](../14_Statistics_Reference/00_LEADERS.md) — the "
        "guide's own printed ranks, not a TTW re-derivation |\n"
        "| *Advanced metrics — EPA, success rate, explosiveness* | "
        "[00_GAPS.md](../11_Betting_Concepts/00_GAPS.md) — cited in "
        "contributor prose only, in neither the glossary nor the tables |\n"
        "| *Rest, travel, lookahead and letdown spots* | "
        "[`13_Situational_Angles`](../13_Situational_Angles/README.md) — "
        "conceptual material; historical and system-based angles are "
        "Phase 10 |\n"
        "| *Which pages carry a concept* | every concept page ends with its "
        "full page list, from the Phase 1 "
        "[concept index](10_Betting_Concept_Index.md) |\n"
        "\n**That 21 of 29 figure is a finding, not a shortfall.** The "
        "guide's only glossary is the 45-entry list on p. 2; tempo, "
        "regression, explosiveness and situational betting are used "
        "constantly and defined nowhere. Where the guide neither defines nor "
        "develops a concept — Closing Line Value and Conference Strength, one "
        "page each — the entry reads `Not addressed in guide.` throughout "
        "rather than importing a definition from general betting knowledge. "
        "The absence is preserved as metadata.\n\n"
        "Two source properties are reproduced rather than corrected: p. 2 "
        "prints `PYPG – Passing Yards per Page`, and the defensive "
        "statistics table omits plays per game and time of possession, so "
        "**defensive tempo cannot be read from this guide at all**. Two "
        "teams promoted from FCS carry the table headings with "
        "`PARTICIPATED IN FCS IN 2025` in place of every value.\n"
    )

    lines.append(
        "\n### Historical trends — `12_Historical_Trends`\n\n"
        "> **These are the guide's own records, reproduced as printed. TTW "
        "has NOT independently backtested any of them.** The library holds no "
        "game-level historical data, so no hit rate here is recomputed, no "
        "span extended and no system re-run. They are **GUIDE CONTENT** — a "
        "record of what VSiN published, not a TTW finding.\n\n"
        "| Question | Start here |\n| --- | --- |\n"
        "| *The Stability System's rules* | "
        "[00_STABILITY_SYSTEM.md](../12_Historical_Trends/00_STABILITY_SYSTEM.md) "
        "— **two** rules, PLAY ON and FADE, each with its own record |\n"
        "| *The stability component records* | "
        "[00_STABILITY_SYSTEM.md](../12_Historical_Trends/00_STABILITY_SYSTEM.md) "
        "→ six components, first four weeks, since 2021 |\n"
        "| *Coaching-change history* | same page → new head coaches "
        "(110-132 ATS) and new coordinators |\n"
        "| *Returning-production history* | same page → 17+ returning "
        "starters (108-86 ATS) and 0–7 returning starters |\n"
        "| *Quarterback-change history* | same page → new starting "
        "quarterbacks (157-172 ATS); a transfer QB counts as NEW |\n"
        "| *Win-total historical performance* | "
        "[00_BY_ANGLE.md](../12_Historical_Trends/00_BY_ANGLE.md) → Makinen's "
        "self-reported 57-42-2, of which 34-17-1 on Unders |\n"
        "| *Heisman base rates* | "
        "[00_BY_ANGLE.md](../12_Historical_Trends/00_BY_ANGLE.md) → "
        "quarterbacks, transfers, and the preseason favourite |\n"
        "| *Angles applied to a specific team* | "
        "[00_BY_TEAM.md](../12_Historical_Trends/00_BY_TEAM.md) — only the "
        "**7 teams the guide names**; the other 131 carry none |\n"
        "| *Situational systems with a printed record* | "
        "[`13_Situational_Angles`](../13_Situational_Angles/README.md) — "
        "offseason transition only; rest, travel, look-ahead spots and "
        "weather have **no printed history at all** |\n"
        "| *Every historical claim, by page* | "
        "[00_TREND_REGISTER.md](../12_Historical_Trends/00_TREND_REGISTER.md) "
        "— 98 claims across 60 pages |\n"
        "| *Records that do not match their own percentages* | "
        "[00_STABILITY_SYSTEM.md](../12_Historical_Trends/00_STABILITY_SYSTEM.md) "
        "→ the reconciliation check, and the printed slip beside it |\n"
        "\n**Two cautions travel with this database.** A historical record is "
        "not a current edge: these describe what happened, not what will. And "
        "the Stability System's long-run figures — 55.4% for PLAY ON, 55.6% "
        "for FADE, both since 2013 — are **never shown without last season's "
        "30-36 ATS beside them**, because the guide itself leads with that "
        "failure and describes the revision it prompted. A validation gate "
        "enforces that pairing.\n\n"
        "Nothing in this directory is a TTW score, grade, model input or "
        "automatic betting rule, and nothing in it should be converted into "
        "one. Two source properties are reproduced rather than corrected: a "
        "bullet comparing new starting quarterbacks against *returning "
        "defensive coordinators*, and a record printed at 45.4% where it "
        "rounds to 45.5%.\n"
    )
