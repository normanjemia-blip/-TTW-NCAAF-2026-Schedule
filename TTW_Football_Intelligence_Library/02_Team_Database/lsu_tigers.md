<!-- GENERATED FILE — do not hand-edit.
     Rebuild:  python3 _tools/build_teams.py
     Source:   2026 VSiN College Football Betting Guide (345 pp.) -->

# LSU Tigers

> **Source: 2026 VSiN College Football Betting Guide, pp. 296–297** and every other page that names this team. GUIDE CONTENT throughout unless a line says otherwise. No outside research. Nothing inferred to fill a heading.


## 1. Program Snapshot

| | |
| --- | --- |
| **Conference** | SEC |
| **Guide pages** | 296–297 |
| **Head coach** | Lane Kiffin (1st season) |
| **2025 straight up** | 7-6 |
| **2025 against the spread** | 4-9 |
| **2025 over/under** | 4-9 |
| **Makinen power rating** | 60.5 |
| **National rank** | #13 of 138 |
| **Conference rank** | #6 of 16 |
| **DraftKings win total** | 8.5 |
| **Makinen projected wins** | 8.5–3.5 overall, 5.7–3.3 in conference |
| **Schedule strength** | 48.72 (#23 toughest of 138) |

### Season outlook — VSiN's analysis in reference form (p. 296)

- Lane Kiffin's drawn-out departure from Ole Miss during a playoff chase left ill-feeling in Oxford, though the guide notes Pete Golding holds none. Its view is that winning silences it. *(p. 296)*
- Kiffin moved aggressively in the offseason, landing the best transfer class in the country and bringing key staff from Ole Miss. *(p. 296)*
- The guide rates Kiffin an elite coach for whom success on the biggest stage has been elusive, and says LSU gives him the resources — leaving the outcome on him. *(p. 296)*
- Recommendation is Under 8.5 wins. The schedule is the core reason: LSU avoids Georgia but draws Alabama, Texas and Texas A&M, all three at home, visits Ole Miss, and opens at home with Clemson. Road trips to Kentucky, Auburn and Tennessee are not treated as certain wins. *(p. 296)*
- The guide allows the transfer class might be enough to lift Kiffin to the top of the SEC, but flags a hard schedule and a quarterback returning from a serious foot injury as reasons for caution. It calls eight wins a good season and the likeliest outcome. *(p. 296)*

*TTW reference notes summarising VSiN's analysis. GUIDE CONTENT — facts, conclusions and reasoning are the guide's; the wording is TTW's.*


## 2. Conference

**SEC** — see the conference file for league-wide context: [sec.md](../01_Conference_Database/sec.md)

Makinen projects **5.7–3.3** in conference play, ranking this team #6 of 16 in the SEC by power rating.

## 3. VSiN Team Rank / Conference Rank

- **National rank:** #13 of 138 *(p. 297)*
- **Conference rank:** #6 of 16 *(p. 297)*
- **Matt Youmans' Preseason Top 50:** **#8** *(pp. 16–20)*

## 4. Steve Makinen Power Rating

**60.5** — as printed on p. 297, reproduced exactly and not converted to any other scale.

Ranks #13 of 138 nationally and #6 of 16 in the SEC. Methodology is set out on pp. 46–47.

> No comparison against the TTW workbook is made here. That is a later-phase deliverable.

## 5. Home-Field Advantage Reference

- **Home field rating:** 3.5 *(p. 296)*
- **Road field rating:** -0.3 *(p. 296)*

Printed on the team page as `field ratings (HOME/ROAD)` and verified against the same values in the conference projected standings table. The guide's general treatment of home-field advantage is on p. 21.

## 6. Head Coach

**Lane Kiffin** — 1st season at the programme *(p. 296)*.

Profiled among the guide's new head coaches in The Coaching Carousel *(p. 36)*.

## 7. Coordinator Notes

Not addressed in guide.

## 8. Coaching Continuity / Changes

**Steve Makinen's Stability Score: 6** *(p. 43)*

| Component | Status | Points |
| --- | --- | --- |
| Head coach | new for 2026 | 0 |
| Offensive coordinator | new for 2026 | 0 |
| Defensive coordinator | returns | 3 |
| Starting quarterback | new for 2026 | 0 |
| Returning starters | 14 | 3 |

Scoring is the guide's own: head coach 4, offensive coordinator 3, defensive coordinator 3, starting quarterback 4, plus points for returning starters. A returning transfer quarterback counts as a **new** quarterback in this system *(p. 41)*.

- Lane Kiffin's drawn-out departure from Ole Miss during a playoff chase left ill-feeling in Oxford, though the guide notes Pete Golding holds none. Its view is that winning silences it. *(p. 296)*
- Kiffin moved aggressively in the offseason, landing the best transfer class in the country and bringing key staff from Ole Miss. *(p. 296)*
- The guide rates Kiffin an elite coach for whom success on the biggest stage has been elusive, and says LSU gives him the resources — leaving the outcome on him. *(p. 296)*
- **Will Blake Baker continue to work his magic with this defense?** Kiffin's $91 million contract drew the headlines, but the guide points to the more than $3 million a year paid to Baker, entering his third season. Before he arrived LSU allowed 28 points and 417 yards per game; in 2025 it gave up 19.8 points, 3.7 yards per rush and 5.0 yards per play, finishing eighth in adjusted opponent EPA per play. Six starters return alongside Umanmielen, who produced 51 pressures and 11 sacks at Ole Miss, and TJ Dottery, another Ole Miss transfer who tied for the sixth-most tackles in the SEC. The guide expects the trend to continue. *(p. 297)*

Also referenced on **pp. 8, 18, 38, 67, 84** — not reproduced here.

## 9. Quarterback Situation

**Paul Stone ranks Sam Leavitt the #9 quarterback in the country** *(p. 45)*.

**Starting quarterback continuity:** the Stability Score table treats this team as having a new starting quarterback for 2026 *(p. 43)*. A transfer expected to start counts as new in that system.

**Returning starting quarterback:** no — the guide marks returning quarterbacks with an asterisk on the returning-starters line *(p. 296)*.

- The guide allows the transfer class might be enough to lift Kiffin to the top of the SEC, but flags a hard schedule and a quarterback returning from a serious foot injury as reasons for caution. It calls eight wins a good season and the likeliest outcome. *(p. 296)*
- **Will Sam Leavitt be healthy and effective?** Landing Leavitt is rated a win comparable to hiring Kiffin. The top-ranked transfer quarterback per 247, he completed 61.3% over 20 games at Arizona State with 34 touchdowns against nine interceptions and led the Sun Devils to the 2024 playoff. A Lisfranc injury ended his 2025 season and his recovery cost him spring practice. Kiffin told ESPN his concern was Leavitt's transition into the offense rather than his health. With only fall camp to get reps, the guide warns of a possible slow start and tells bettors to monitor how effective he is coming off that injury. *(p. 297)*

Also referenced on **pp. 20, 67, 165, 182** — not reproduced here.

## 10. Returning Production

| | Returning starters |
| --- | --- |
| **Total** | 14 |
| **Offence** | 8 |
| **Defence** | 6 |

Read by position from p. 296. The three figures sit under the printed labels `total / offense / defense`; they are matched to those labels by their place on the page, because the PDF text layer emits them in a different order than they are printed.

The Stability Score table prints the same figure independently: **14 returning starters** *(p. 43)*.

## 11. Transfer Portal

- Kiffin moved aggressively in the offseason, landing the best transfer class in the country and bringing key staff from Ole Miss. *(p. 296)*
- The guide allows the transfer class might be enough to lift Kiffin to the top of the SEC, but flags a hard schedule and a quarterback returning from a serious foot injury as reasons for caution. It calls eight wins a good season and the likeliest outcome. *(p. 296)*
- **Will Sam Leavitt be healthy and effective?** Landing Leavitt is rated a win comparable to hiring Kiffin. The top-ranked transfer quarterback per 247, he completed 61.3% over 20 games at Arizona State with 34 touchdowns against nine interceptions and led the Sun Devils to the 2024 playoff. A Lisfranc injury ended his 2025 season and his recovery cost him spring practice. Kiffin told ESPN his concern was Leavitt's transition into the offense rather than his health. With only fall camp to get reps, the guide warns of a possible slow start and tells bettors to monitor how effective he is coming off that injury. *(p. 297)*
- **Does landing the best transfer class of 2026 pay off?** Kiffin took 41 portal commits, sixth-most nationally. Beyond Leavitt, five-star transfers defensive end Princewill Umanmielen and offensive tackle Jordan Seaton arrived, and per Phil Steele the class brought 107 career starts. The guide keeps the question genuinely open: entire position groups are being rebuilt — nine receiver transfers, five more defensive linemen — and it notes others have been burned building the plane while flying it, while allowing Kiffin may be good enough to finish the build. *(p. 297)*
- **Will Blake Baker continue to work his magic with this defense?** Kiffin's $91 million contract drew the headlines, but the guide points to the more than $3 million a year paid to Baker, entering his third season. Before he arrived LSU allowed 28 points and 417 yards per game; in 2025 it gave up 19.8 points, 3.7 yards per rush and 5.0 yards per play, finishing eighth in adjusted opponent EPA per play. Six starters return alongside Umanmielen, who produced 51 pressures and 11 sacks at Ole Miss, and TJ Dottery, another Ole Miss transfer who tied for the sixth-most tackles in the SEC. The guide expects the trend to continue. *(p. 297)*


Referenced in the guide on **pp. 10, 11, 18, 20, 119, 182, 321** — those passages are not reproduced here; see the pages for VSiN's own wording.

## 12. Recruiting / Roster Notes

Not addressed in guide under this heading.

The guide names this team on **pp. 11, 118, 297**, but no passage there was extracted under this heading; see those pages for VSiN's own wording.

## 13. Offensive Identity

Not addressed in guide under this heading.

The guide names this team on **pp. 118, 297**, but no passage there was extracted under this heading; see those pages for VSiN's own wording.

## 14. Defensive Identity

- **Will Blake Baker continue to work his magic with this defense?** Kiffin's $91 million contract drew the headlines, but the guide points to the more than $3 million a year paid to Baker, entering his third season. Before he arrived LSU allowed 28 points and 417 yards per game; in 2025 it gave up 19.8 points, 3.7 yards per rush and 5.0 yards per play, finishing eighth in adjusted opponent EPA per play. Six starters return alongside Umanmielen, who produced 51 pressures and 11 sacks at Ole Miss, and TJ Dottery, another Ole Miss transfer who tied for the sixth-most tackles in the SEC. The guide expects the trend to continue. *(p. 297)*


Referenced in the guide on **pp. 18, 165** — those passages are not reproduced here; see the pages for VSiN's own wording.

## 15. Key Strengths

National top-25 finishes in the guide's printed categories *(p. 297)*:

| Side | Category | Value | National rank |
| --- | --- | --- | --- |
| Defense | TURNOVERS | 1.8 | #21 |
| Defense | POINTS PER GAME | 20.7 | #24 |
| Defense | YARDS PER POINT | 16.4 | #24 |

## 16. Key Weaknesses

National bottom-25 finishes in the guide's printed categories *(p. 297)*:

| Side | Category | Value | National rank |
| --- | --- | --- | --- |
| Offense | RUSH ATTEMPTS PER GAME | 28.3 | #132 |
| Offense | RUSH YARDS PER GAME | 102.1 | #122 |
| Offense | PLAYS PER GAME | 63.8 | #117 |
| Offense | TOTAL YARDS PER GAME | 317.8 | #116 |

## 17. Schedule Overview

Schedule strength **48.72**, **#23 toughest of 138** *(p. 296)*.

| Date | Opponent | Site | Projected line | Opponent power rating |
| --- | --- | --- | --- | --- |
| 9/5 | CLEMSON | Home | -9.9 | 54.0 |
| 9/12 | LOUISIANA TECH | Home | -31.8 | 33.0 |
| 9/19 | at Ole Miss | Away | +3.6 | 60.5 |
| 9/26 | TEXAS A&M | Home | -1.9 | 62.5 |
| 10/3 | MCNEESE ST | Home | -53.5 | 11.0 |
| 10/10 | at Kentucky | Away | -10.8 | 47.0 |
| 10/17 | MISSISSIPPI ST | Home | -16.9 | 47.0 |
| 10/24 | at Auburn | Away | -6 | 52.5 |
| 11/7 | ALABAMA | Home | -3.5 | 61.0 |
| 11/14 | TEXAS | Home | +1.7 | 66.0 |
| 11/21 | at Tennessee | Away | +0.7 | 57.0 |
| 11/28 | at Arkansas | Away | -13.1 | 45.5 |

Projected lines and opponent power ratings are Makinen's, printed alongside the schedule on the team page.

## 18. Difficult Stretches / Trap Spots

Not addressed in guide.

## 19. Win Total Discussion

- **DraftKings win total:** 8.5 *(p. 283)*
- **Makinen projected wins:** 8.5 *(p. 283)*
- **Difference:** +0.0 wins against the posted number
- **Recommendation on the team page:** **UNDER 8.5** *(p. 296)*

## 20. Futures / Conference / Playoff Discussion

Prices printed on the team page *(p. 297)*:

| Market | Price |
| --- | --- |
| CFP Championship | 18-1 |
| make the playoff | +160 |
| SEC | +900 |

**VSiN staff predictions naming this team** *(p. 4)*:

| Category | Votes |
| --- | --- |
| CFP FINAL FOUR #4 | 2 of 22 |

## 21. Betting Notes / Best Bets

**Host best bets naming this team** *(pp. 5–15)*:

| Contributor | Pick | Page |
| --- | --- | --- |
| Jensen Lewis | LSU TO MAKE COLLEGE FOOTBALL PLAYOFF (+160) | 8 |
| Zachary Cohen | LSU OVER 8.5 WINS (-104 - 3 UNITS) & TO MAKE COLLEGE FOOTBALL PLAYOFF (+240) | 8 |

Where contributors disagree, every position is kept separately and none is reconciled.

- Recommendation is Under 8.5 wins. The schedule is the core reason: LSU avoids Georgia but draws Alabama, Texas and Texas A&M, all three at home, visits Ole Miss, and opens at home with Clemson. Road trips to Kentucky, Auburn and Tennessee are not treated as certain wins. *(p. 296)*
- The guide allows the transfer class might be enough to lift Kiffin to the top of the SEC, but flags a hard schedule and a quarterback returning from a serious foot injury as reasons for caution. It calls eight wins a good season and the likeliest outcome. *(p. 296)*
- **Will Sam Leavitt be healthy and effective?** Landing Leavitt is rated a win comparable to hiring Kiffin. The top-ranked transfer quarterback per 247, he completed 61.3% over 20 games at Arizona State with 34 touchdowns against nine interceptions and led the Sun Devils to the 2024 playoff. A Lisfranc injury ended his 2025 season and his recovery cost him spring practice. Kiffin told ESPN his concern was Leavitt's transition into the offense rather than his health. With only fall camp to get reps, the guide warns of a possible slow start and tells bettors to monitor how effective he is coming off that injury. *(p. 297)*

Also referenced on **pp. 38** — not reproduced here.

## 22. Historical / Situational Trends

- Lane Kiffin's drawn-out departure from Ole Miss during a playoff chase left ill-feeling in Oxford, though the guide notes Pete Golding holds none. Its view is that winning silences it. *(p. 296)*


Referenced in the guide on **pp. 10, 22, 45, 67, 128, 165** — those passages are not reproduced here; see the pages for VSiN's own wording.

## 23. Important Statistics

As printed on p. 297. Each category carries a value and a national rank.

### Offence

| Category | Value | National rank |
| --- | --- | --- |
| POINTS PER GAME | 20.1 | 111 |
| YARDS PER POINT | 15.8 | 101 |
| PLAYS PER GAME | 63.8 | 117 |
| TIME OF POSSESSION | 29:53 | 69 |
| 3RD DOWN CONV. % | 34.4% | 104 |
| TOTAL YARDS PER GAME | 317.8 | 116 |
| YARDS PER PLAY | 5 | 105 |
| RUSH ATTEMPTS PER GAME | 28.3 | 132 |
| RUSH YARDS PER GAME | 102.1 | 122 |
| YARDS PER RUSH ATTEMPT | 3.6 | 106 |
| PASS ATTEMPTS PER GAME | 33.3 | 43 |
| COMPLETION % | 64.0% | 42 |
| PASSING YARDS PER GAME | 215.8 | 80 |
| YARDS PER PASS ATTEMPT | 6.5 | 101 |
| TURNOVERS | 1.2 | 46 |

### Defence

| Category | Value | National rank |
| --- | --- | --- |
| POINTS PER GAME | 20.7 | 24 |
| YARDS PER POINT | 16.4 | 24 |
| 3RD DOWN CONV. % | 35.9% | 39 |
| TOTAL YARDS PER GAME | 339.4 | 28 |
| YARDS PER PLAY | 5 | 32 |
| RUSH YARDS PER GAME | 126.9 | 29 |
| YARDS PER RUSH ATTEMPT | 3.8 | 33 |
| COMPLETION % | 60.0% | 38 |
| PASSING YARDS PER GAME | 212.5 | 49 |
| YARDS PER PASS ATTEMPT | 6.5 | 26 |
| SACKS | 1.9 | 69 |
| TURNOVERS | 1.8 | 21 |

## 24. Bull Case

- The guide rates Kiffin an elite coach for whom success on the biggest stage has been elusive, and says LSU gives him the resources — leaving the outcome on him. *(p. 296)*

*4 further statements about this team carry both positive and negative language and are not classified to either side. They appear in full under the season outlook and *Open Questions / Risks*.*

> *Assembled from statements the guide makes about this team. The statements are GUIDE CONTENT with page references; the selection of which ones argue this side is PERSONAL INFERENCE.*

## 25. Bear Case

- The team page recommends **UNDER 8.5**.

*4 further statements about this team carry both positive and negative language and are not classified to either side. They appear in full under the season outlook and *Open Questions / Risks*.*

> *Assembled from statements the guide makes about this team. The statements are GUIDE CONTENT with page references; the selection of which ones argue this side is PERSONAL INFERENCE.*

## 26. Open Questions / Risks

VSiN poses *Three Burning Questions for the 2026 Season* *(p. 297)*. Its analysis, in reference form:

### Will Sam Leavitt be healthy and effective?

Landing Leavitt is rated a win comparable to hiring Kiffin. The top-ranked transfer quarterback per 247, he completed 61.3% over 20 games at Arizona State with 34 touchdowns against nine interceptions and led the Sun Devils to the 2024 playoff. A Lisfranc injury ended his 2025 season and his recovery cost him spring practice. Kiffin told ESPN his concern was Leavitt's transition into the offense rather than his health. With only fall camp to get reps, the guide warns of a possible slow start and tells bettors to monitor how effective he is coming off that injury. *(p. 297)*

### Does landing the best transfer class of 2026 pay off?

Kiffin took 41 portal commits, sixth-most nationally. Beyond Leavitt, five-star transfers defensive end Princewill Umanmielen and offensive tackle Jordan Seaton arrived, and per Phil Steele the class brought 107 career starts. The guide keeps the question genuinely open: entire position groups are being rebuilt — nine receiver transfers, five more defensive linemen — and it notes others have been burned building the plane while flying it, while allowing Kiffin may be good enough to finish the build. *(p. 297)*

### Will Blake Baker continue to work his magic with this defense?

Kiffin's $91 million contract drew the headlines, but the guide points to the more than $3 million a year paid to Baker, entering his third season. Before he arrived LSU allowed 28 points and 417 yards per game; in 2025 it gave up 19.8 points, 3.7 yards per rush and 5.0 yards per play, finishing eighth in adjusted opponent EPA per play. Six starters return alongside Umanmielen, who produced 51 pressures and 11 sacks at Ole Miss, and TJ Dottery, another Ole Miss transfer who tied for the sixth-most tackles in the SEC. The guide expects the trend to continue. *(p. 297)*

*TTW reference notes. The questions are VSiN's; the answers summarise VSiN's reasoning in TTW's wording.*

## 27. Source Conflicts

No source conflict identified for this team.

## 28. Relevant Page References

| Pages | Content |
| --- | --- |
| 296–297 | This team's two-page preview |
| 283 | SEC conference preview and projected standings |
| 4 | VSiN staff season predictions |
| 46–47 | Steve Makinen power rating methodology |
| 21 | Home-field advantage article |
| 8, 9, 10, 11, 15, 18, 20, 22, 36, 38, 39, 41, 45, 67, 84, 118, 119, 128, 165, 182, 284, 299, 304, 310, 312, 321, 323, 338 | Other pages naming this team |

This team is named in **48 sentences across 30 pages** of the guide.

Every page naming this team outside its own spread: pp. 8, 9, 10, 11, 15, 18, 20, 22, 36, 38, 39, 41, 45, 67, 84, 118, 119, 128, 165, 182, 284, 299, 304, 310, 312, 321, 323, 338.

## 29. Cross-Links

**Conference** — [SEC](../01_Conference_Database/sec.md)

**Master Index** — [Team Index](../00_Master_Index/03_Team_Index.md) · [Power Rating Index](../00_Master_Index/09_Power_Rating_Index.md) · [Coaching Index](../00_Master_Index/04_Coaching_Index.md) · [Quarterback Index](../00_Master_Index/06_Quarterback_Index.md)

**Every other database, for this team:**

- Head coach — [03_Coaching_Database/lsu_tigers.md](../03_Coaching_Database/lsu_tigers.md)
- Quarterback — [04_Quarterback_Database/lsu_tigers.md](../04_Quarterback_Database/lsu_tigers.md)
- Power rating — [05_Power_Ratings/00_MAKINEN_RATINGS.md](../05_Power_Ratings/00_MAKINEN_RATINGS.md)
- Win total — [06_Win_Totals/00_ALL_TEAMS.md](../06_Win_Totals/00_ALL_TEAMS.md)
- Futures board — [07_Futures/00_TEAM_FUTURES.md](../07_Futures/00_TEAM_FUTURES.md)
- Returning production — [08_Returning_Production/README.md](../08_Returning_Production/README.md)
- Schedule — [10_Schedule_Intelligence/00_BY_TEAM.md](../10_Schedule_Intelligence/00_BY_TEAM.md)
- Historical angles — [12_Historical_Trends/00_BY_TEAM.md](../12_Historical_Trends/00_BY_TEAM.md)
- Everything at once — [99_Search_Index/02_TEAM_LOOKUP.md](../99_Search_Index/02_TEAM_LOOKUP.md)

