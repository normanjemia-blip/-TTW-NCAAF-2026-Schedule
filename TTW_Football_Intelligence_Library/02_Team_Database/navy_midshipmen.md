<!-- GENERATED FILE — do not hand-edit.
     Rebuild:  python3 _tools/build_teams.py
     Source:   2026 VSiN College Football Betting Guide (345 pp.) -->

# Navy Midshipmen

> **Source: 2026 VSiN College Football Betting Guide, pp. 60–61** and every other page that names this team. GUIDE CONTENT throughout unless a line says otherwise. No outside research. Nothing inferred to fill a heading.


## 1. Program Snapshot

| | |
| --- | --- |
| **Conference** | American |
| **Guide pages** | 60–61 |
| **Head coach** | Brian Newberry (5th season) |
| **2025 straight up** | 11-2 |
| **2025 against the spread** | 5-8 |
| **2025 over/under** | 8-5 |
| **Makinen power rating** | 44.0 |
| **National rank** | #63 of 138 |
| **Conference rank** | #1 of 14 |
| **DraftKings win total** | 7.5 |
| **Makinen projected wins** | 8.5–3.5 overall, 6.2–1.8 in conference |
| **Schedule strength** | 35.19 (#86 toughest of 138) |

### Season outlook — VSiN's analysis in reference form (p. 60)

- The guide credits Brian Newberry for hiring coordinator Drew Cronic, whose wrinkles gave Navy new life in the American. *(p. 60)*
- It sets the pattern that makes this year risky: Navy went 14-2 in conference play in its first two AAC seasons before declining as teams adjusted, and under Ken Niumatalolo had only one further winning conference season after that start. Under Newberry Navy is 18-7. *(p. 60)*
- Cronic's Wing-T modified triple option gives opponents much to prepare for. The production shift is stark: 17.7 points per game in 2023, then 31.3 and 31.5 with 6.3 and 6.5 yards per play in two seasons under him. *(p. 60)*
- The guide then poses the central question — whether that was Cronic or a generational run of offensive leaders — with Blake Horvath, Alex Tecza and all-time leading receiver Eli Heidenreich all gone. *(p. 60)*
- Recommendation is Under 7.5 wins. The conference schedule is not too demanding and Navy should again be better than both service academies. *(p. 60)*
- Schedule detail: road games at UTSA and an improved Florida Atlantic, plus Notre Dame and a home game with Memphis, but no Tulane or East Carolina. Both North Texas and Notre Dame face Navy off byes. *(p. 60)*
- Its conclusion is that with two years of film on the Cronic offense and massive personnel losses, heavy regression is possible — while candidly noting its own projection is 7.56 wins. *(p. 60)*

*TTW reference notes summarising VSiN's analysis. GUIDE CONTENT — facts, conclusions and reasoning are the guide's; the wording is TTW's.*


## 2. Conference

**American** — see the conference file for league-wide context: [american.md](../01_Conference_Database/american.md)

Makinen projects **6.2–1.8** in conference play, ranking this team #1 of 14 in the American by power rating.

## 3. VSiN Team Rank / Conference Rank

- **National rank:** #63 of 138 *(p. 61)*
- **Conference rank:** #1 of 14 *(p. 61)*
- **Matt Youmans' Preseason Top 50:** **#50** *(pp. 16–20)*

## 4. Steve Makinen Power Rating

**44.0** — as printed on p. 61, reproduced exactly and not converted to any other scale.

Ranks #63 of 138 nationally and #1 of 14 in the American. Methodology is set out on pp. 46–47.

> No comparison against the TTW workbook is made here. That is a later-phase deliverable.

## 5. Home-Field Advantage Reference

- **Home field rating:** 2.3 *(p. 60)*
- **Road field rating:** 0.8 *(p. 60)*

Printed on the team page as `field ratings (HOME/ROAD)` and verified against the same values in the conference projected standings table. The guide's general treatment of home-field advantage is on p. 21.

## 6. Head Coach

**Brian Newberry** — 5th season at the programme *(p. 60)*.

## 7. Coordinator Notes

- **OC Drew Cronic** *(p. 60)*

## 8. Coaching Continuity / Changes

**Steve Makinen's Stability Score: 9** *(p. 43)*

| Component | Status | Points |
| --- | --- | --- |
| Head coach | returns | 4 |
| Offensive coordinator | returns | 3 |
| Defensive coordinator | new for 2026 | 0 |
| Starting quarterback | new for 2026 | 0 |
| Returning starters | 11 | 2 |

Scoring is the guide's own: head coach 4, offensive coordinator 3, defensive coordinator 3, starting quarterback 4, plus points for returning starters. A returning transfer quarterback counts as a **new** quarterback in this system *(p. 41)*.

- The guide credits Brian Newberry for hiring coordinator Drew Cronic, whose wrinkles gave Navy new life in the American. *(p. 60)*
- **What does that mean for the defense?** The guide calls this the million-dollar question. Sensing urgency, Newberry is returning to his roots as his own defensive play-caller, having demoted P.J. Volker to safeties coach and promoted Eric Lewis to coordinator. Losses are far lighter here with five of the top seven tacklers back, but the 5.9 yards per play and 25 points per game allowed were the most since 2021. It adds an important methodological caution: Navy's defensive numbers must be read in the context of an offensive scheme that limits possessions — and with offensive production expected to fall, the defense could bear the brunt. *(p. 61)*

Also referenced on **pp. 27, 49** — not reproduced here.

## 9. Quarterback Situation

**Starting quarterback continuity:** the Stability Score table treats this team as having a new starting quarterback for 2026 *(p. 43)*. A transfer expected to start counts as new in that system.

**Returning starting quarterback:** no — the guide marks returning quarterbacks with an asterisk on the returning-starters line *(p. 60)*.

- The guide then poses the central question — whether that was Cronic or a generational run of offensive leaders — with Blake Horvath, Alex Tecza and all-time leading receiver Eli Heidenreich all gone. *(p. 60)*
- **How can Navy replicate the wrath of Horvath?** The guide places Horvath alongside Keenan Reynolds and Malcolm Perry among Navy greats: over 3,000 passing yards and over 2,600 rushing yards, with only Reynolds and Bill Byrne throwing more passing touchdowns in programme history — and it notes Reynolds played four years to Horvath's three. Braxton Woodson has enormous shoes to fill but knows the scheme, having started games in each of the last two seasons when Horvath was banged up, and averaged 9.4 yards per carry on 44 attempts. The guide identifies the specific gap precisely: Horvath's 58.8% completion rate trailed only Roger Staubach among Navy passers with at least 120 attempts, while Woodson is at 46.3%. *(p. 61)*

Also referenced on **pp. 20, 49, 50** — not reproduced here.

## 10. Returning Production

| | Returning starters |
| --- | --- |
| **Total** | 11 |
| **Offence** | 5 |
| **Defence** | 6 |

Read by position from p. 60. The three figures sit under the printed labels `total / offense / defense`; they are matched to those labels by their place on the page, because the PDF text layer emits them in a different order than they are printed.

The Stability Score table prints the same figure independently: **11 returning starters** *(p. 43)*.

## 11. Transfer Portal

Not addressed in guide.

## 12. Recruiting / Roster Notes

Not addressed in guide.

## 13. Offensive Identity

- The guide credits Brian Newberry for hiring coordinator Drew Cronic, whose wrinkles gave Navy new life in the American. *(p. 60)*
- Cronic's Wing-T modified triple option gives opponents much to prepare for. The production shift is stark: 17.7 points per game in 2023, then 31.3 and 31.5 with 6.3 and 6.5 yards per play in two seasons under him. *(p. 60)*
- The guide then poses the central question — whether that was Cronic or a generational run of offensive leaders — with Blake Horvath, Alex Tecza and all-time leading receiver Eli Heidenreich all gone. *(p. 60)*
- **How can Navy replicate the wrath of Horvath?** The guide places Horvath alongside Keenan Reynolds and Malcolm Perry among Navy greats: over 3,000 passing yards and over 2,600 rushing yards, with only Reynolds and Bill Byrne throwing more passing touchdowns in programme history — and it notes Reynolds played four years to Horvath's three. Braxton Woodson has enormous shoes to fill but knows the scheme, having started games in each of the last two seasons when Horvath was banged up, and averaged 9.4 yards per carry on 44 attempts. The guide identifies the specific gap precisely: Horvath's 58.8% completion rate trailed only Roger Staubach among Navy passers with at least 120 attempts, while Woodson is at 46.3%. *(p. 61)*
- **What about the other losses?** Tecza and Heidenreich, a seventh-round Steelers pick, are described as monumental losses. Tecza ran for 902 yards at 5.7 per carry with 10 touchdowns and finished 12th in Navy rushing yards at 2,236, seventh all-time among running backs. Heidenreich had over 500 rushing yards and 941 receiving yards on 51 catches, passing Rob Taylor for the programme receiving record and finishing fifth in career scrimmage yards. Fourth-leading rusher and second-leading receiver Brandon Chatman also exhausted eligibility. The guide grants the scheme creates productive players, then quantifies the hole: Navy is missing 93.5% of its passing yards, over 83% of its rushing yards and over 84% of its receiving yards. *(p. 61)*


Referenced in the guide on **pp. 20** — those passages are not reproduced here; see the pages for VSiN's own wording.

## 14. Defensive Identity

- **What does that mean for the defense?** The guide calls this the million-dollar question. Sensing urgency, Newberry is returning to his roots as his own defensive play-caller, having demoted P.J. Volker to safeties coach and promoted Eric Lewis to coordinator. Losses are far lighter here with five of the top seven tacklers back, but the 5.9 yards per play and 25 points per game allowed were the most since 2021. It adds an important methodological caution: Navy's defensive numbers must be read in the context of an offensive scheme that limits possessions — and with offensive production expected to fall, the defense could bear the brunt. *(p. 61)*


Referenced in the guide on **pp. 20** — those passages are not reproduced here; see the pages for VSiN's own wording.

## 15. Key Strengths

National top-25 finishes in the guide's printed categories *(p. 61)*:

| Side | Category | Value | National rank |
| --- | --- | --- | --- |
| Offense | RUSH YARDS PER GAME | 270.8 | #1 |
| Offense | YARDS PER PASS ATTEMPT | 9.5 | #2 |
| Offense | RUSH ATTEMPTS PER GAME | 49.1 | #3 |
| Offense | YARDS PER RUSH ATTEMPT | 5.5 | #6 |
| Offense | 3RD DOWN CONV. % | 49.7% | #7 |
| Offense | TIME OF POSSESSION | 32:22 | #14 |
| Offense | YARDS PER PLAY | 6.3 | #18 |

## 16. Key Weaknesses

National bottom-25 finishes in the guide's printed categories *(p. 61)*:

| Side | Category | Value | National rank |
| --- | --- | --- | --- |
| Offense | PASS ATTEMPTS PER GAME | 14 | #135 |
| Offense | PASSING YARDS PER GAME | 133.2 | #133 |
| Offense | PLAYS PER GAME | 63.8 | #117 |
| Defense | YARDS PER PASS ATTEMPT | 8.1 | #114 |

## 17. Schedule Overview

Schedule strength **35.19**, **#86 toughest of 138** *(p. 60)*.

| Date | Opponent | Site | Projected line | Opponent power rating |
| --- | --- | --- | --- | --- |
| 9/5 | TOWSON | Home | -28.9 | 16.5 |
| 9/12 | at Fla Atlantic | Away | -9.3 | 33.5 |
| 9/25 | at UAB | Away | -16.1 | 26.0 |
| 10/3 | at Air Force | Away | -5.3 | 37.0 |
| 10/10 | TULSA | Home | -14.4 | 32.5 |
| 10/17 | at TX-San Antonio | Away | +0.3 | 41.0 |
| 10/24 | NORTH TEXAS | Home | -11.7 | 34.5 |
| 10/31 | vs. Notre Dame | Neutral | +24.7 | 68.5 |
| 11/7 | TEMPLE | Home | -13.1 | 34.0 |
| 11/21 | MEMPHIS | Home | -4.6 | 41.0 |
| 11/28 | at Charlotte | Away | -23.3 | 20.5 |
| 12/12 | vs. Army | Neutral | -4 | 40.0 |

Projected lines and opponent power ratings are Makinen's, printed alongside the schedule on the team page.

## 18. Difficult Stretches / Trap Spots

- Schedule detail: road games at UTSA and an improved Florida Atlantic, plus Notre Dame and a home game with Memphis, but no Tulane or East Carolina. Both North Texas and Notre Dame face Navy off byes. *(p. 60)*

## 19. Win Total Discussion

- **DraftKings win total:** 7.5 *(p. 49)*
- **Makinen projected wins:** 8.5 *(p. 49)*
- **Difference:** +1.0 wins against the posted number
- **Recommendation on the team page:** **UNDER 7.5** *(p. 60)*

## 20. Futures / Conference / Playoff Discussion

Prices printed on the team page *(p. 61)*:

| Market | Price |
| --- | --- |
| CFP Championship | 600-1 |
| make the playoff | +750 |
| AMERICAN | +380 |

**VSiN staff predictions naming this team** *(p. 4)*:

| Category | Votes |
| --- | --- |
| AAC CHAMPION | 8 of 22 |

## 21. Betting Notes / Best Bets

- Recommendation is Under 7.5 wins. The conference schedule is not too demanding and Navy should again be better than both service academies. *(p. 60)*
- Its conclusion is that with two years of film on the Cronic offense and massive personnel losses, heavy regression is possible — while candidly noting its own projection is 7.56 wins. *(p. 60)*

Also referenced on **pp. 49** — not reproduced here.

## 22. Historical / Situational Trends

- It sets the pattern that makes this year risky: Navy went 14-2 in conference play in its first two AAC seasons before declining as teams adjusted, and under Ken Niumatalolo had only one further winning conference season after that start. Under Newberry Navy is 18-7. *(p. 60)*
- **How can Navy replicate the wrath of Horvath?** The guide places Horvath alongside Keenan Reynolds and Malcolm Perry among Navy greats: over 3,000 passing yards and over 2,600 rushing yards, with only Reynolds and Bill Byrne throwing more passing touchdowns in programme history — and it notes Reynolds played four years to Horvath's three. Braxton Woodson has enormous shoes to fill but knows the scheme, having started games in each of the last two seasons when Horvath was banged up, and averaged 9.4 yards per carry on 44 attempts. The guide identifies the specific gap precisely: Horvath's 58.8% completion rate trailed only Roger Staubach among Navy passers with at least 120 attempts, while Woodson is at 46.3%. *(p. 61)*


Referenced in the guide on **pp. 50, 51, 64** — those passages are not reproduced here; see the pages for VSiN's own wording.

## 23. Important Statistics

As printed on p. 61. Each category carries a value and a national rank.

### Offence

| Category | Value | National rank |
| --- | --- | --- |
| POINTS PER GAME | 29.8 | 34 |
| YARDS PER POINT | 13.6 | 46 |
| PLAYS PER GAME | 63.8 | 117 |
| TIME OF POSSESSION | 32:22 | 14 |
| 3RD DOWN CONV. % | 49.7% | 7 |
| TOTAL YARDS PER GAME | 403.9 | 39 |
| YARDS PER PLAY | 6.3 | 18 |
| RUSH ATTEMPTS PER GAME | 49.1 | 3 |
| RUSH YARDS PER GAME | 270.8 | 1 |
| YARDS PER RUSH ATTEMPT | 5.5 | 6 |
| PASS ATTEMPTS PER GAME | 14 | 135 |
| COMPLETION % | 57.7% | 107 |
| PASSING YARDS PER GAME | 133.2 | 133 |
| YARDS PER PASS ATTEMPT | 9.5 | 2 |
| TURNOVERS | 1.2 | 46 |

### Defence

| Category | Value | National rank |
| --- | --- | --- |
| POINTS PER GAME | 26.5 | 74 |
| YARDS PER POINT | 14.5 | 68 |
| 3RD DOWN CONV. % | 40.3% | 75 |
| TOTAL YARDS PER GAME | 384.6 | 74 |
| YARDS PER PLAY | 5.8 | 94 |
| RUSH YARDS PER GAME | 151.8 | 66 |
| YARDS PER RUSH ATTEMPT | 4.2 | 60 |
| COMPLETION % | 64.8% | 103 |
| PASSING YARDS PER GAME | 232.8 | 84 |
| YARDS PER PASS ATTEMPT | 8.1 | 114 |
| SACKS | 1.8 | 82 |
| TURNOVERS | 1.3 | 71 |

## 24. Bull Case

- Makinen projects **8.5 wins** against a posted total of 7.5 — **+1.0** in this team's favour.
- The guide credits Brian Newberry for hiring coordinator Drew Cronic, whose wrinkles gave Navy new life in the American. *(p. 60)*
- It sets the pattern that makes this year risky: Navy went 14-2 in conference play in its first two AAC seasons before declining as teams adjusted, and under Ken Niumatalolo had only one further winning conference season after that start. Under Newberry Navy is 18-7. *(p. 60)*

*1 further statement about this team carries both positive and negative language and is not classified to either side. It appears in full under the season outlook and *Open Questions / Risks*.*

> *Assembled from statements the guide makes about this team. The statements are GUIDE CONTENT with page references; the selection of which ones argue this side is PERSONAL INFERENCE.*

## 25. Bear Case

- The team page recommends **UNDER 7.5**.
- The guide then poses the central question — whether that was Cronic or a generational run of offensive leaders — with Blake Horvath, Alex Tecza and all-time leading receiver Eli Heidenreich all gone. *(p. 60)*
- Its conclusion is that with two years of film on the Cronic offense and massive personnel losses, heavy regression is possible — while candidly noting its own projection is 7.56 wins. *(p. 60)*

*1 further statement about this team carries both positive and negative language and is not classified to either side. It appears in full under the season outlook and *Open Questions / Risks*.*

> *Assembled from statements the guide makes about this team. The statements are GUIDE CONTENT with page references; the selection of which ones argue this side is PERSONAL INFERENCE.*

## 26. Open Questions / Risks

VSiN poses *Three Burning Questions for the 2026 Season* *(p. 61)*. Its analysis, in reference form:

### How can Navy replicate the wrath of Horvath?

The guide places Horvath alongside Keenan Reynolds and Malcolm Perry among Navy greats: over 3,000 passing yards and over 2,600 rushing yards, with only Reynolds and Bill Byrne throwing more passing touchdowns in programme history — and it notes Reynolds played four years to Horvath's three. Braxton Woodson has enormous shoes to fill but knows the scheme, having started games in each of the last two seasons when Horvath was banged up, and averaged 9.4 yards per carry on 44 attempts. The guide identifies the specific gap precisely: Horvath's 58.8% completion rate trailed only Roger Staubach among Navy passers with at least 120 attempts, while Woodson is at 46.3%. *(p. 61)*

### What about the other losses?

Tecza and Heidenreich, a seventh-round Steelers pick, are described as monumental losses. Tecza ran for 902 yards at 5.7 per carry with 10 touchdowns and finished 12th in Navy rushing yards at 2,236, seventh all-time among running backs. Heidenreich had over 500 rushing yards and 941 receiving yards on 51 catches, passing Rob Taylor for the programme receiving record and finishing fifth in career scrimmage yards. Fourth-leading rusher and second-leading receiver Brandon Chatman also exhausted eligibility. The guide grants the scheme creates productive players, then quantifies the hole: Navy is missing 93.5% of its passing yards, over 83% of its rushing yards and over 84% of its receiving yards. *(p. 61)*

### What does that mean for the defense?

The guide calls this the million-dollar question. Sensing urgency, Newberry is returning to his roots as his own defensive play-caller, having demoted P.J. Volker to safeties coach and promoted Eric Lewis to coordinator. Losses are far lighter here with five of the top seven tacklers back, but the 5.9 yards per play and 25 points per game allowed were the most since 2021. It adds an important methodological caution: Navy's defensive numbers must be read in the context of an offensive scheme that limits possessions — and with offensive production expected to fall, the defense could bear the brunt. *(p. 61)*

*TTW reference notes. The questions are VSiN's; the answers summarise VSiN's reasoning in TTW's wording.*

## 27. Source Conflicts

- **Head coach tenure stated inconsistently.** The American conference preview (p. 49) describes Brian Newberry as beginning his fourth season; Navy's team page (p. 60) states his fifth. Both are reproduced as printed and neither is corrected.

## 28. Relevant Page References

| Pages | Content |
| --- | --- |
| 60–61 | This team's two-page preview |
| 49 | American conference preview and projected standings |
| 4 | VSiN staff season predictions |
| 46–47 | Steve Makinen power rating methodology |
| 21 | Home-field advantage article |
| 14, 20, 27, 46, 50, 51, 56, 58, 62, 64, 65, 68, 69, 70, 72, 93 | Other pages naming this team |

This team is named in **31 sentences across 19 pages** of the guide.

Every page naming this team outside its own spread: pp. 14, 20, 27, 46, 50, 51, 56, 58, 62, 64, 65, 68, 69, 70, 72, 93.

## 29. Cross-Links

**Conference** — [American](../01_Conference_Database/american.md)

**Master Index** — [Team Index](../00_Master_Index/03_Team_Index.md) · [Power Rating Index](../00_Master_Index/09_Power_Rating_Index.md) · [Coaching Index](../00_Master_Index/04_Coaching_Index.md) · [Quarterback Index](../00_Master_Index/06_Quarterback_Index.md)

**Every other database, for this team:**

- Head coach — [03_Coaching_Database/navy_midshipmen.md](../03_Coaching_Database/navy_midshipmen.md)
- Quarterback — [04_Quarterback_Database/navy_midshipmen.md](../04_Quarterback_Database/navy_midshipmen.md)
- Power rating — [05_Power_Ratings/00_MAKINEN_RATINGS.md](../05_Power_Ratings/00_MAKINEN_RATINGS.md)
- Win total — [06_Win_Totals/00_ALL_TEAMS.md](../06_Win_Totals/00_ALL_TEAMS.md)
- Futures board — [07_Futures/00_TEAM_FUTURES.md](../07_Futures/00_TEAM_FUTURES.md)
- Returning production — [08_Returning_Production/README.md](../08_Returning_Production/README.md)
- Schedule — [10_Schedule_Intelligence/00_BY_TEAM.md](../10_Schedule_Intelligence/00_BY_TEAM.md)
- Historical angles — [12_Historical_Trends/00_BY_TEAM.md](../12_Historical_Trends/00_BY_TEAM.md)
- Everything at once — [99_Search_Index/02_TEAM_LOOKUP.md](../99_Search_Index/02_TEAM_LOOKUP.md)

