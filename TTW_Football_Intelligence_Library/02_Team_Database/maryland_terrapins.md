<!-- GENERATED FILE — do not hand-edit.
     Rebuild:  python3 _tools/build_teams.py
     Source:   2026 VSiN College Football Betting Guide (345 pp.) -->

# Maryland Terrapins

> **Source: 2026 VSiN College Football Betting Guide, pp. 156–157** and every other page that names this team. GUIDE CONTENT throughout unless a line says otherwise. No outside research. Nothing inferred to fill a heading.


## 1. Program Snapshot

| | |
| --- | --- |
| **Conference** | Big Ten |
| **Guide pages** | 156–157 |
| **Head coach** | Mike Locksley (8th season) |
| **2025 straight up** | 4-8 |
| **2025 against the spread** | 5-7 |
| **2025 over/under** | 5-7 |
| **Makinen power rating** | 45.5 |
| **National rank** | #54 of 138 |
| **Conference rank** | #14 of 18 |
| **DraftKings win total** | 5.5 |
| **Makinen projected wins** | 5.4–6.6 overall, 3.3–5.7 in conference |
| **Schedule strength** | 45.81 (#39 toughest of 138) |

### Season outlook — VSiN's analysis in reference form (p. 156)

- The guide places Mike Locksley on the hot seat after consecutive 4-8 seasons. Maryland started 4-0 last year, lost each of the next three by four points or fewer, and never recovered, finishing on an eight-game losing streak. *(p. 156)*
- Maryland returns 71% of its production, second nationally to Notre Dame per ESPN's Bill Connelly. *(p. 156)*
- Quarterback Malik Washington is credited with about all you could ask of a true freshman starting in the Big Ten: just under 3,000 passing yards, a 17-to-9 touchdown-to-interception ratio and four rushing scores. Leading rusher DeJuan Williams (501 yards, three touchdowns) returns with four offensive line starters, and eight starters return for second-year coordinator Ted Monachino. *(p. 156)*
- Recommendation is Over 4.5 wins. The guide credits Locksley as a high-level recruiter who has kept DMV talent home, while stating bluntly that he cannot live off the 23-16 run from 2021 to 2023 forever. *(p. 156)*
- It flags the price honestly, calling roughly -170 to -175 a juicy amount to lay, then sets out the path: an opener against Hampton and games at UConn and against Virginia Tech, both with new head coaches. *(p. 156)*
- Maryland travels to Ohio State and USC but avoids Oregon, Indiana and Michigan. If it repeats a 3-0 non-conference start, the guide identifies UCLA, Rutgers, Wisconsin and Purdue away as winnable routes to bowl eligibility and a first postseason game since 2023. *(p. 156)*

*TTW reference notes summarising VSiN's analysis. GUIDE CONTENT — facts, conclusions and reasoning are the guide's; the wording is TTW's.*


## 2. Conference

**Big Ten** — see the conference file for league-wide context: [big_ten.md](../01_Conference_Database/big_ten.md)

Makinen projects **3.3–5.7** in conference play, ranking this team #14 of 18 in the Big Ten by power rating.

## 3. VSiN Team Rank / Conference Rank

- **National rank:** #54 of 138 *(p. 157)*
- **Conference rank:** #14 of 18 *(p. 157)*
- **Matt Youmans' Preseason Top 50:** not ranked *(pp. 16–20)*

## 4. Steve Makinen Power Rating

**45.5** — as printed on p. 157, reproduced exactly and not converted to any other scale.

Ranks #54 of 138 nationally and #14 of 18 in the Big Ten. Methodology is set out on pp. 46–47.

> No comparison against the TTW workbook is made here. That is a later-phase deliverable.

## 5. Home-Field Advantage Reference

- **Home field rating:** 2.1 *(p. 156)*
- **Road field rating:** -0.3 *(p. 156)*

Printed on the team page as `field ratings (HOME/ROAD)` and verified against the same values in the conference projected standings table. The guide's general treatment of home-field advantage is on p. 21.

## 6. Head Coach

**Mike Locksley** — 8th season at the programme *(p. 156)*.

## 7. Coordinator Notes

- **DC Ted Monachino** *(p. 156)*

## 8. Coaching Continuity / Changes

**Steve Makinen's Stability Score: 14** *(p. 43)*

| Component | Status | Points |
| --- | --- | --- |
| Head coach | returns | 4 |
| Offensive coordinator | new for 2026 | 0 |
| Defensive coordinator | returns | 3 |
| Starting quarterback | returns | 4 |
| Returning starters | 16 | 3 |

Scoring is the guide's own: head coach 4, offensive coordinator 3, defensive coordinator 3, starting quarterback 4, plus points for returning starters. A returning transfer quarterback counts as a **new** quarterback in this system *(p. 41)*.

- The guide places Mike Locksley on the hot seat after consecutive 4-8 seasons. Maryland started 4-0 last year, lost each of the next three by four points or fewer, and never recovered, finishing on an eight-game losing streak. *(p. 156)*
- Recommendation is Over 4.5 wins. The guide credits Locksley as a high-level recruiter who has kept DMV talent home, while stating bluntly that he cannot live off the 23-16 run from 2021 to 2023 forever. *(p. 156)*
- **How will Malik Washington take to the new offense?** Pep Hamilton moves to an advisory role and Clint Trickett takes over as coordinator after a season in the same job at Jacksonville State. Trickett wants more RPO, more designed running from Washington, and increased tempo through more motion and more snaps. The guide balances the line's poor run-blocking against a genuine strength — only nine sacks allowed — and notes Washington can throw the ball anywhere but needs a receiving corps that was plagued by drops to become more reliable. *(p. 157)*
- **Can the defense continue to progress under Monachino?** Monachino was on the Ravens staff that won Super Bowl XLVII and coached Terrell Suggs and Elvis Dumervil, so the guide credits him with knowing elite front-seven talent. Edge rushers Zahir Mathis and Sidney Stewart were freshman All-Americans and are joined by five-star Baltimore freshman Zion Elee. Senior linebacker Daniel Wingate (102 tackles, 6.5 tackles for loss) leads the linebackers, with strong safety Lavain Scruggs (64 tackles, two interceptions) heading the secondary and Messiah Delhomme (39 tackles) counted on to replace third-round pick Jalen Huskey. The guide's summary: Maryland can clearly rush the passer but has struggled to stop the run inside. *(p. 157)*

Also referenced on **pp. 150, 193** — not reproduced here.

## 9. Quarterback Situation

**Starting quarterback continuity:** the Stability Score table treats this team as having the same starting quarterback as 2025 *(p. 43)*. A transfer expected to start counts as new in that system.

**Returning starting quarterback:** yes — the guide marks returning quarterbacks with an asterisk on the returning-starters line *(p. 156)*.

- Quarterback Malik Washington is credited with about all you could ask of a true freshman starting in the Big Ten: just under 3,000 passing yards, a 17-to-9 touchdown-to-interception ratio and four rushing scores. Leading rusher DeJuan Williams (501 yards, three touchdowns) returns with four offensive line starters, and eight starters return for second-year coordinator Ted Monachino. *(p. 156)*
- **How will Malik Washington take to the new offense?** Pep Hamilton moves to an advisory role and Clint Trickett takes over as coordinator after a season in the same job at Jacksonville State. Trickett wants more RPO, more designed running from Washington, and increased tempo through more motion and more snaps. The guide balances the line's poor run-blocking against a genuine strength — only nine sacks allowed — and notes Washington can throw the ball anywhere but needs a receiving corps that was plagued by drops to become more reliable. *(p. 157)*

Also referenced on **pp. 5** — not reproduced here.

## 10. Returning Production

| | Returning starters |
| --- | --- |
| **Total** | 16 |
| **Offence** | 8 *(includes the returning quarterback)* |
| **Defence** | 8 |

Read by position from p. 156. The three figures sit under the printed labels `total / offense / defense`; they are matched to those labels by their place on the page, because the PDF text layer emits them in a different order than they are printed.

The Stability Score table prints the same figure independently: **16 returning starters** *(p. 43)*.

## 11. Transfer Portal

- **Can Maryland develop a running game?** The guide's answer is a definitive no in recent years: no 1,000-yard rusher since Roman Hemby in 2022, who later transferred to Indiana. Harry Dalton III, highly touted out of high school, transfers from USC to support Washington and Williams. Maryland ranked second-to-last in the Big Ten at 104.3 rushing yards per game, 122nd nationally. Its conclusion is conditional: Washington is a potential star, but the group needs more production for him to realize it. *(p. 157)*


Referenced in the guide on **pp. 5, 271** — those passages are not reproduced here; see the pages for VSiN's own wording.

## 12. Recruiting / Roster Notes

- Recommendation is Over 4.5 wins. The guide credits Locksley as a high-level recruiter who has kept DMV talent home, while stating bluntly that he cannot live off the 23-16 run from 2021 to 2023 forever. *(p. 156)*
- **Can the defense continue to progress under Monachino?** Monachino was on the Ravens staff that won Super Bowl XLVII and coached Terrell Suggs and Elvis Dumervil, so the guide credits him with knowing elite front-seven talent. Edge rushers Zahir Mathis and Sidney Stewart were freshman All-Americans and are joined by five-star Baltimore freshman Zion Elee. Senior linebacker Daniel Wingate (102 tackles, 6.5 tackles for loss) leads the linebackers, with strong safety Lavain Scruggs (64 tackles, two interceptions) heading the secondary and Messiah Delhomme (39 tackles) counted on to replace third-round pick Jalen Huskey. The guide's summary: Maryland can clearly rush the passer but has struggled to stop the run inside. *(p. 157)*

## 13. Offensive Identity

- Maryland returns 71% of its production, second nationally to Notre Dame per ESPN's Bill Connelly. *(p. 156)*
- Quarterback Malik Washington is credited with about all you could ask of a true freshman starting in the Big Ten: just under 3,000 passing yards, a 17-to-9 touchdown-to-interception ratio and four rushing scores. Leading rusher DeJuan Williams (501 yards, three touchdowns) returns with four offensive line starters, and eight starters return for second-year coordinator Ted Monachino. *(p. 156)*
- **How will Malik Washington take to the new offense?** Pep Hamilton moves to an advisory role and Clint Trickett takes over as coordinator after a season in the same job at Jacksonville State. Trickett wants more RPO, more designed running from Washington, and increased tempo through more motion and more snaps. The guide balances the line's poor run-blocking against a genuine strength — only nine sacks allowed — and notes Washington can throw the ball anywhere but needs a receiving corps that was plagued by drops to become more reliable. *(p. 157)*
- **Can Maryland develop a running game?** The guide's answer is a definitive no in recent years: no 1,000-yard rusher since Roman Hemby in 2022, who later transferred to Indiana. Harry Dalton III, highly touted out of high school, transfers from USC to support Washington and Williams. Maryland ranked second-to-last in the Big Ten at 104.3 rushing yards per game, 122nd nationally. Its conclusion is conditional: Washington is a potential star, but the group needs more production for him to realize it. *(p. 157)*

## 14. Defensive Identity

- Maryland returns 71% of its production, second nationally to Notre Dame per ESPN's Bill Connelly. *(p. 156)*
- Quarterback Malik Washington is credited with about all you could ask of a true freshman starting in the Big Ten: just under 3,000 passing yards, a 17-to-9 touchdown-to-interception ratio and four rushing scores. Leading rusher DeJuan Williams (501 yards, three touchdowns) returns with four offensive line starters, and eight starters return for second-year coordinator Ted Monachino. *(p. 156)*
- **Can the defense continue to progress under Monachino?** Monachino was on the Ravens staff that won Super Bowl XLVII and coached Terrell Suggs and Elvis Dumervil, so the guide credits him with knowing elite front-seven talent. Edge rushers Zahir Mathis and Sidney Stewart were freshman All-Americans and are joined by five-star Baltimore freshman Zion Elee. Senior linebacker Daniel Wingate (102 tackles, 6.5 tackles for loss) leads the linebackers, with strong safety Lavain Scruggs (64 tackles, two interceptions) heading the secondary and Messiah Delhomme (39 tackles) counted on to replace third-round pick Jalen Huskey. The guide's summary: Maryland can clearly rush the passer but has struggled to stop the run inside. *(p. 157)*

## 15. Key Strengths

National top-25 finishes in the guide's printed categories *(p. 157)*:

| Side | Category | Value | National rank |
| --- | --- | --- | --- |
| Offense | PASS ATTEMPTS PER GAME | 42 | #4 |
| Defense | TURNOVERS | 1.7 | #24 |

## 16. Key Weaknesses

National bottom-25 finishes in the guide's printed categories *(p. 157)*:

| Side | Category | Value | National rank |
| --- | --- | --- | --- |
| Offense | RUSH ATTEMPTS PER GAME | 23.8 | #136 |
| Offense | TIME OF POSSESSION | 26:08 | #132 |
| Offense | RUSH YARDS PER GAME | 101.3 | #124 |
| Offense | YARDS PER PASS ATTEMPT | 6 | #121 |
| Defense | 3RD DOWN CONV. % | 45.2% | #119 |
| Offense | COMPLETION % | 56.9% | #118 |

## 17. Schedule Overview

Schedule strength **45.81**, **#39 toughest of 138** *(p. 156)*.

| Date | Opponent | Site | Projected line | Opponent power rating |
| --- | --- | --- | --- | --- |
| 9/5 | HAMPTON | Home | -50.6 | -3.0 |
| 9/12 | at Connecticut | Away | -9.8 | 31.5 |
| 9/19 | VIRGINIA TECH | Home | +3.2 | 51.0 |
| 9/26 | UCLA | Home | +0.7 | 48.0 |
| 10/3 | at Nebraska | Away | +7.6 | 50.5 |
| 10/10 | at Ohio St | Away | +29.6 | 71.0 |
| 10/17 | RUTGERS | Home | -5.1 | 42.5 |
| 10/31 | ILLINOIS | Home | +4.3 | 51.0 |
| 11/7 | at Purdue | Away | -4.7 | 39.0 |
| 11/14 | WISCONSIN | Home | -0.3 | 47.5 |
| 11/21 | at USC | Away | +18.6 | 60.0 |
| 11/28 | PENN ST | Home | +10.2 | 57.0 |

Projected lines and opponent power ratings are Makinen's, printed alongside the schedule on the team page.

## 18. Difficult Stretches / Trap Spots

Not addressed in guide.

## 19. Win Total Discussion

- **DraftKings win total:** 5.5 *(p. 149)*
- **Makinen projected wins:** 5.4 *(p. 149)*
- **Difference:** -0.1 wins against the posted number
- **Recommendation on the team page:** **OVER 4.5** *(p. 156)*

## 20. Futures / Conference / Playoff Discussion

Prices printed on the team page *(p. 157)*:

| Market | Price |
| --- | --- |
| CFP Championship | 800-1 |
| make the playoff | 60-1 |
| BIG TEN | 500-1 |

## 21. Betting Notes / Best Bets

- Recommendation is Over 4.5 wins. The guide credits Locksley as a high-level recruiter who has kept DMV talent home, while stating bluntly that he cannot live off the 23-16 run from 2021 to 2023 forever. *(p. 156)*
- It flags the price honestly, calling roughly -170 to -175 a juicy amount to lay, then sets out the path: an opener against Hampton and games at UConn and against Virginia Tech, both with new head coaches. *(p. 156)*
- Maryland travels to Ohio State and USC but avoids Oregon, Indiana and Michigan. If it repeats a 3-0 non-conference start, the guide identifies UCLA, Rutgers, Wisconsin and Purdue away as winnable routes to bowl eligibility and a first postseason game since 2023. *(p. 156)*

## 22. Historical / Situational Trends

- The guide places Mike Locksley on the hot seat after consecutive 4-8 seasons. Maryland started 4-0 last year, lost each of the next three by four points or fewer, and never recovered, finishing on an eight-game losing streak. *(p. 156)*


Referenced in the guide on **pp. 99, 183, 193** — those passages are not reproduced here; see the pages for VSiN's own wording.

## 23. Important Statistics

As printed on p. 157. Each category carries a value and a national rank.

### Offence

| Category | Value | National rank |
| --- | --- | --- |
| POINTS PER GAME | 21.6 | 102 |
| YARDS PER POINT | 16.4 | 108 |
| PLAYS PER GAME | 66.5 | 95 |
| TIME OF POSSESSION | 26:08 | 132 |
| 3RD DOWN CONV. % | 37.3% | 88 |
| TOTAL YARDS PER GAME | 355.1 | 87 |
| YARDS PER PLAY | 5.3 | 77 |
| RUSH ATTEMPTS PER GAME | 23.8 | 136 |
| RUSH YARDS PER GAME | 101.3 | 124 |
| YARDS PER RUSH ATTEMPT | 4.3 | 67 |
| PASS ATTEMPTS PER GAME | 42 | 4 |
| COMPLETION % | 56.9% | 118 |
| PASSING YARDS PER GAME | 253.8 | 34 |
| YARDS PER PASS ATTEMPT | 6 | 121 |
| TURNOVERS | 1.2 | 56 |

### Defence

| Category | Value | National rank |
| --- | --- | --- |
| POINTS PER GAME | 27.4 | 75 |
| YARDS PER POINT | 15.1 | 52 |
| 3RD DOWN CONV. % | 45.2% | 119 |
| TOTAL YARDS PER GAME | 413.1 | 101 |
| YARDS PER PLAY | 5.5 | 63 |
| RUSH YARDS PER GAME | 186.9 | 107 |
| YARDS PER RUSH ATTEMPT | 4.5 | 84 |
| COMPLETION % | 64.4% | 98 |
| PASSING YARDS PER GAME | 226.2 | 72 |
| YARDS PER PASS ATTEMPT | 7 | 58 |
| SACKS | 2.2 | 48 |
| TURNOVERS | 1.7 | 24 |

## 24. Bull Case

- The team page recommends **OVER 4.5**.
- Maryland returns 71% of its production, second nationally to Notre Dame per ESPN's Bill Connelly. *(p. 156)*
- Recommendation is Over 4.5 wins. The guide credits Locksley as a high-level recruiter who has kept DMV talent home, while stating bluntly that he cannot live off the 23-16 run from 2021 to 2023 forever. *(p. 156)*

*2 further statements about this team carry both positive and negative language and are not classified to either side. They appear in full under the season outlook and *Open Questions / Risks*.*

> *Assembled from statements the guide makes about this team. The statements are GUIDE CONTENT with page references; the selection of which ones argue this side is PERSONAL INFERENCE.*

## 25. Bear Case

- Makinen projects **5.4 wins** against a posted total of 5.5 — **-0.1** against this team.
- The guide places Mike Locksley on the hot seat after consecutive 4-8 seasons. Maryland started 4-0 last year, lost each of the next three by four points or fewer, and never recovered, finishing on an eight-game losing streak. *(p. 156)*

*2 further statements about this team carry both positive and negative language and are not classified to either side. They appear in full under the season outlook and *Open Questions / Risks*.*

> *Assembled from statements the guide makes about this team. The statements are GUIDE CONTENT with page references; the selection of which ones argue this side is PERSONAL INFERENCE.*

## 26. Open Questions / Risks

VSiN poses *Three Burning Questions for the 2026 Season* *(p. 157)*. Its analysis, in reference form:

### How will Malik Washington take to the new offense?

Pep Hamilton moves to an advisory role and Clint Trickett takes over as coordinator after a season in the same job at Jacksonville State. Trickett wants more RPO, more designed running from Washington, and increased tempo through more motion and more snaps. The guide balances the line's poor run-blocking against a genuine strength — only nine sacks allowed — and notes Washington can throw the ball anywhere but needs a receiving corps that was plagued by drops to become more reliable. *(p. 157)*

### Can Maryland develop a running game?

The guide's answer is a definitive no in recent years: no 1,000-yard rusher since Roman Hemby in 2022, who later transferred to Indiana. Harry Dalton III, highly touted out of high school, transfers from USC to support Washington and Williams. Maryland ranked second-to-last in the Big Ten at 104.3 rushing yards per game, 122nd nationally. Its conclusion is conditional: Washington is a potential star, but the group needs more production for him to realize it. *(p. 157)*

### Can the defense continue to progress under Monachino?

Monachino was on the Ravens staff that won Super Bowl XLVII and coached Terrell Suggs and Elvis Dumervil, so the guide credits him with knowing elite front-seven talent. Edge rushers Zahir Mathis and Sidney Stewart were freshman All-Americans and are joined by five-star Baltimore freshman Zion Elee. Senior linebacker Daniel Wingate (102 tackles, 6.5 tackles for loss) leads the linebackers, with strong safety Lavain Scruggs (64 tackles, two interceptions) heading the secondary and Messiah Delhomme (39 tackles) counted on to replace third-round pick Jalen Huskey. The guide's summary: Maryland can clearly rush the passer but has struggled to stop the run inside. *(p. 157)*

*TTW reference notes. The questions are VSiN's; the answers summarise VSiN's reasoning in TTW's wording.*

## 27. Source Conflicts

No source conflict identified for this team.

## 28. Relevant Page References

| Pages | Content |
| --- | --- |
| 156–157 | This team's two-page preview |
| 149 | Big Ten conference preview and projected standings |
| 4 | VSiN staff season predictions |
| 46–47 | Steve Makinen power rating methodology |
| 21 | Home-field advantage article |
| 5, 99, 150, 183, 193, 271 | Other pages naming this team |

This team is named in **10 sentences across 8 pages** of the guide.

Every page naming this team outside its own spread: pp. 5, 99, 150, 183, 193, 271.

## 29. Cross-Links

**Conference** — [Big Ten](../01_Conference_Database/big_ten.md)

**Master Index** — [Team Index](../00_Master_Index/03_Team_Index.md) · [Power Rating Index](../00_Master_Index/09_Power_Rating_Index.md) · [Coaching Index](../00_Master_Index/04_Coaching_Index.md) · [Quarterback Index](../00_Master_Index/06_Quarterback_Index.md)

**Every other database, for this team:**

- Head coach — [03_Coaching_Database/maryland_terrapins.md](../03_Coaching_Database/maryland_terrapins.md)
- Quarterback — [04_Quarterback_Database/maryland_terrapins.md](../04_Quarterback_Database/maryland_terrapins.md)
- Power rating — [05_Power_Ratings/00_MAKINEN_RATINGS.md](../05_Power_Ratings/00_MAKINEN_RATINGS.md)
- Win total — [06_Win_Totals/00_ALL_TEAMS.md](../06_Win_Totals/00_ALL_TEAMS.md)
- Futures board — [07_Futures/00_TEAM_FUTURES.md](../07_Futures/00_TEAM_FUTURES.md)
- Returning production — [08_Returning_Production/README.md](../08_Returning_Production/README.md)
- Schedule — [10_Schedule_Intelligence/00_BY_TEAM.md](../10_Schedule_Intelligence/00_BY_TEAM.md)
- Historical angles — [12_Historical_Trends/00_BY_TEAM.md](../12_Historical_Trends/00_BY_TEAM.md)
- Everything at once — [99_Search_Index/02_TEAM_LOOKUP.md](../99_Search_Index/02_TEAM_LOOKUP.md)

