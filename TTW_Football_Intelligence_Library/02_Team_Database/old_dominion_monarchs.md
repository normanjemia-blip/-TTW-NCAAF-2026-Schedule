<!-- GENERATED FILE — do not hand-edit.
     Rebuild:  python3 _tools/build_teams.py
     Source:   2026 VSiN College Football Betting Guide (345 pp.) -->

# Old Dominion Monarchs

> **Source: 2026 VSiN College Football Betting Guide, pp. 330–331** and every other page that names this team. GUIDE CONTENT throughout unless a line says otherwise. No outside research. Nothing inferred to fill a heading.


## 1. Program Snapshot

| | |
| --- | --- |
| **Conference** | Sun Belt |
| **Guide pages** | 330–331 |
| **Head coach** | Ricky Rahne (6th season) |
| **2025 straight up** | 10-3 |
| **2025 against the spread** | 8-5 |
| **2025 over/under** | 5-8 |
| **Makinen power rating** | 37.0 |
| **National rank** | #86 of 138 |
| **Conference rank** | #2 of 14 |
| **DraftKings win total** | 7.5 |
| **Makinen projected wins** | 7.5–4.5 overall, 5.4–2.6 in conference |
| **Schedule strength** | 30.36 (#122 toughest of 138) |

### Season outlook — VSiN's analysis in reference form (p. 330)

- The guide opens by locating Old Dominion inside someone else's season: Indiana won 10 of its 16 victories by 14 or more points, and one of the six closer games was against ODU, which was badly out-gained in the regular-season opener yet lost only 27-14 — a sign of things to come. *(p. 330)*
- Ricky Rahne's programme then improved by five wins to finish 10-3. *(p. 330)*
- What the guide finds remarkable is the shape of the losses: all three came by 13, 24 and 36 points, with ODU -5 in turnovers against Marshall and pummelled by James Madison. Otherwise, it notes, 10 wins was one fewer than the previous two seasons combined. *(p. 330)*
- It generalises this into a structural observation about the Group of Six era: whiplash seasons are very possible now because of the portal, with some teams completely wrecked and others staying intact enough to take advantage. *(p. 330)*
- ODU is placed on the wrong side of that this year, with very heavy losses — headlined by quarterback Colton Joseph to Wisconsin after rushing for more than 1,000 yards and throwing for over 2,600 — and offensive coordinator Kevin Decker leaving for the same role at Memphis. *(p. 330)*
- Recommendation is Over 7.5 wins, resting almost entirely on the schedule being kind to Rahne. *(p. 330)*
- The specifics: James Madison and Marshall both come to Norfolk; the oddly timed non-conference road game at UConn is very winnable, as are dates with hapless Georgia State, newcomer Louisiana Tech and a Coastal Carolina team in transition; drawing Southern Miss and Louisiana Tech from the West is favourable; and the East Carolina game is at home. *(p. 330)*
- Its arithmetic behind the number: ODU is favoured in nine games and within +1.5 and +0.5 in two more, giving a projection of 7.51 wins. *(p. 330)*

*TTW reference notes summarising VSiN's analysis. GUIDE CONTENT — facts, conclusions and reasoning are the guide's; the wording is TTW's.*


## 2. Conference

**Sun Belt** — see the conference file for league-wide context: [sun_belt.md](../01_Conference_Database/sun_belt.md)

Makinen projects **5.4–2.6** in conference play, ranking this team #2 of 14 in the Sun Belt by power rating.

## 3. VSiN Team Rank / Conference Rank

- **National rank:** #86 of 138 *(p. 331)*
- **Conference rank:** #2 of 14 *(p. 331)*
- **Matt Youmans' Preseason Top 50:** not ranked *(pp. 16–20)*

## 4. Steve Makinen Power Rating

**37.0** — as printed on p. 331, reproduced exactly and not converted to any other scale.

Ranks #86 of 138 nationally and #2 of 14 in the Sun Belt. Methodology is set out on pp. 46–47.

> No comparison against the TTW workbook is made here. That is a later-phase deliverable.

## 5. Home-Field Advantage Reference

- **Home field rating:** 2.8 *(p. 330)*
- **Road field rating:** 0.6 *(p. 330)*

Printed on the team page as `field ratings (HOME/ROAD)` and verified against the same values in the conference projected standings table. The guide's general treatment of home-field advantage is on p. 21.

## 6. Head Coach

**Ricky Rahne** — 6th season at the programme *(p. 330)*.

## 7. Coordinator Notes

- **OC Kevin Decker** *(p. 330)*

## 8. Coaching Continuity / Changes

**Steve Makinen's Stability Score: 7** *(p. 43)*

| Component | Status | Points |
| --- | --- | --- |
| Head coach | returns | 4 |
| Offensive coordinator | new for 2026 | 0 |
| Defensive coordinator | returns | 3 |
| Starting quarterback | new for 2026 | 0 |
| Returning starters | 7 | 0 |

Scoring is the guide's own: head coach 4, offensive coordinator 3, defensive coordinator 3, starting quarterback 4, plus points for returning starters. A returning transfer quarterback counts as a **new** quarterback in this system *(p. 41)*.

- Ricky Rahne's programme then improved by five wins to finish 10-3. *(p. 330)*
- ODU is placed on the wrong side of that this year, with very heavy losses — headlined by quarterback Colton Joseph to Wisconsin after rushing for more than 1,000 yards and throwing for over 2,600 — and offensive coordinator Kevin Decker leaving for the same role at Memphis. *(p. 330)*
- **Can Cook keep the offense cooking?** Kody Cook is promoted to offensive coordinator after coaching wide receivers last season, and also takes the quarterbacks, with other shifts across the staff. The guide's concern is concentration: only three players had 11-plus catches, posting 762, 719 and 667 yards while nobody else exceeded 200 — and all three are gone, so Cook's old position group must be reimagined. It expects a run-heavy team again, listing Devin Roche at 5.7 yards per carry (well short of Trequan Jones' 7.5) and JUCO transfer Trey Hall, originally an Ole Miss commit. Its conclusion is that Quinn Henicle and Jaxon Potter will have skill players to work with, and that Cook — who played at Kansas State and coached at Tennessee — was the logical promotion after Decker's departure. *(p. 331)*
- **Who gets more reps as the starting QB?** The guide weighs continuity against upside. Henicle is the holdover who knows the system, but it says plainly he was not very good — 4.8 yards per carry on 44 attempts, only 42.5% completions, and 11-for-25 against a skeleton-crew USF team in the bowl game. Potter was the higher-graded recruit and was willing to cross the country from Washington State as a California native. In Henicle's favour it notes he was recruited by FCS schools with good rushing attacks and by Army, so he was clearly seen as an impact runner at the position. Its verdict keeps both the doubt and the urgency: neither looks poised to top Joseph's numbers, and Rahne needs to get this right and fix it early in the season if he has not settled it coming out of camp. *(p. 331)*
- **Should we buy or sell this defense?** The guide first credits Rahne, an offensive-minded coach, with finally rounding that side into form — up in yards per carry and points per game in each of the last three seasons — before arguing the real separator was Blake Seiler's defense. That unit cut 9.5 PPG and 0.9 yards per play from the previous season, holding opponents to 18.5 PPG and 4.8 yards per play, with ODU out-gaining opponents by 1.9 yards per play as a team. It tracks Seiler's pass rush across three years — 19 sacks, then 26, then 39 — alongside a top-25 third-down defense. Returning help includes two linebackers who started in 2024 but missed most of last season, plus returning starters at each level. Its calibrated verdict is to buy them as better than 2024 but worse than 2025. *(p. 331)*

Also referenced on **pp. 59** — not reproduced here.

## 9. Quarterback Situation

**Starting quarterback continuity:** the Stability Score table treats this team as having a new starting quarterback for 2026 *(p. 43)*. A transfer expected to start counts as new in that system.

**Returning starting quarterback:** no — the guide marks returning quarterbacks with an asterisk on the returning-starters line *(p. 330)*.

- ODU is placed on the wrong side of that this year, with very heavy losses — headlined by quarterback Colton Joseph to Wisconsin after rushing for more than 1,000 yards and throwing for over 2,600 — and offensive coordinator Kevin Decker leaving for the same role at Memphis. *(p. 330)*
- **Who gets more reps as the starting QB?** The guide weighs continuity against upside. Henicle is the holdover who knows the system, but it says plainly he was not very good — 4.8 yards per carry on 44 attempts, only 42.5% completions, and 11-for-25 against a skeleton-crew USF team in the bowl game. Potter was the higher-graded recruit and was willing to cross the country from Washington State as a California native. In Henicle's favour it notes he was recruited by FCS schools with good rushing attacks and by Army, so he was clearly seen as an impact runner at the position. Its verdict keeps both the doubt and the urgency: neither looks poised to top Joseph's numbers, and Rahne needs to get this right and fix it early in the season if he has not settled it coming out of camp. *(p. 331)*

Also referenced on **pp. 20, 184** — not reproduced here.

## 10. Returning Production

| | Returning starters |
| --- | --- |
| **Total** | 7 |
| **Offence** | 1 |
| **Defence** | 6 |

Read by position from p. 330. The three figures sit under the printed labels `total / offense / defense`; they are matched to those labels by their place on the page, because the PDF text layer emits them in a different order than they are printed.

The Stability Score table prints the same figure independently: **7 returning starters** *(p. 43)*.

## 11. Transfer Portal

- It generalises this into a structural observation about the Group of Six era: whiplash seasons are very possible now because of the portal, with some teams completely wrecked and others staying intact enough to take advantage. *(p. 330)*
- ODU is placed on the wrong side of that this year, with very heavy losses — headlined by quarterback Colton Joseph to Wisconsin after rushing for more than 1,000 yards and throwing for over 2,600 — and offensive coordinator Kevin Decker leaving for the same role at Memphis. *(p. 330)*
- **Can Cook keep the offense cooking?** Kody Cook is promoted to offensive coordinator after coaching wide receivers last season, and also takes the quarterbacks, with other shifts across the staff. The guide's concern is concentration: only three players had 11-plus catches, posting 762, 719 and 667 yards while nobody else exceeded 200 — and all three are gone, so Cook's old position group must be reimagined. It expects a run-heavy team again, listing Devin Roche at 5.7 yards per carry (well short of Trequan Jones' 7.5) and JUCO transfer Trey Hall, originally an Ole Miss commit. Its conclusion is that Quinn Henicle and Jaxon Potter will have skill players to work with, and that Cook — who played at Kansas State and coached at Tennessee — was the logical promotion after Decker's departure. *(p. 331)*


Referenced in the guide on **pp. 8, 20, 73, 184** — those passages are not reproduced here; see the pages for VSiN's own wording.

## 12. Recruiting / Roster Notes

Not addressed in guide under this heading.

The guide names this team on **pp. 331**, but no passage there was extracted under this heading; see those pages for VSiN's own wording.

## 13. Offensive Identity

- **Can Cook keep the offense cooking?** Kody Cook is promoted to offensive coordinator after coaching wide receivers last season, and also takes the quarterbacks, with other shifts across the staff. The guide's concern is concentration: only three players had 11-plus catches, posting 762, 719 and 667 yards while nobody else exceeded 200 — and all three are gone, so Cook's old position group must be reimagined. It expects a run-heavy team again, listing Devin Roche at 5.7 yards per carry (well short of Trequan Jones' 7.5) and JUCO transfer Trey Hall, originally an Ole Miss commit. Its conclusion is that Quinn Henicle and Jaxon Potter will have skill players to work with, and that Cook — who played at Kansas State and coached at Tennessee — was the logical promotion after Decker's departure. *(p. 331)*

## 14. Defensive Identity

- **Should we buy or sell this defense?** The guide first credits Rahne, an offensive-minded coach, with finally rounding that side into form — up in yards per carry and points per game in each of the last three seasons — before arguing the real separator was Blake Seiler's defense. That unit cut 9.5 PPG and 0.9 yards per play from the previous season, holding opponents to 18.5 PPG and 4.8 yards per play, with ODU out-gaining opponents by 1.9 yards per play as a team. It tracks Seiler's pass rush across three years — 19 sacks, then 26, then 39 — alongside a top-25 third-down defense. Returning help includes two linebackers who started in 2024 but missed most of last season, plus returning starters at each level. Its calibrated verdict is to buy them as better than 2024 but worse than 2025. *(p. 331)*

## 15. Key Strengths

National top-25 finishes in the guide's printed categories *(p. 331)*:

| Side | Category | Value | National rank |
| --- | --- | --- | --- |
| Defense | SACKS | 3.1 | #4 |
| Offense | RUSH YARDS PER GAME | 236 | #7 |
| Offense | YARDS PER RUSH ATTEMPT | 5.5 | #9 |
| Defense | TURNOVERS | 1.9 | #11 |
| Offense | RUSH ATTEMPTS PER GAME | 43.3 | #12 |
| Defense | YARDS PER POINT | 17.3 | #12 |
| Offense | TOTAL YARDS PER GAME | 447.3 | #13 |
| Defense | YARDS PER PLAY | 4.6 | #13 |
| Defense | POINTS PER GAME | 19.6 | #15 |
| Offense | YARDS PER PLAY | 6.4 | #16 |
| Offense | YARDS PER PASS ATTEMPT | 8.4 | #18 |
| Defense | YARDS PER PASS ATTEMPT | 6.3 | #18 |

## 16. Key Weaknesses

National bottom-25 finishes in the guide's printed categories *(p. 331)*:

| Side | Category | Value | National rank |
| --- | --- | --- | --- |
| Offense | TURNOVERS | 1.9 | #129 |
| Offense | COMPLETION % | 55.8% | #121 |
| Offense | PASS ATTEMPTS PER GAME | 25.3 | #118 |

## 17. Schedule Overview

Schedule strength **30.36**, **#122 toughest of 138** *(p. 330)*.

| Date | Opponent | Site | Projected line | Opponent power rating |
| --- | --- | --- | --- | --- |
| 9/5 | NORFOLK ST | Home | -43 | -3.5 |
| 9/12 | at Virginia Tech | Away | +15.7 | 51.0 |
| 9/19 | EAST CAROLINA | Home | +1.4 | 40.5 |
| 9/26 | JAMES MADISON | Home | +2.5 | 41.5 |
| 10/3 | at Georgia St | Away | -13.6 | 23.0 |
| 10/10 | at Appalachian St | Away | -5.4 | 30.0 |
| 10/15 | GA SOUTHERN | Home | -6.8 | 33.5 |
| 10/24 | at Louisiana Tech | Away | -0.7 | 33.0 |
| 10/31 | MARSHALL | Home | -5.3 | 34.0 |
| 11/7 | at Coastal Carolina | Away | -7.4 | 28.0 |
| 11/21 | at Connecticut | Away | -2.2 | 31.5 |
| 11/28 | SOUTHERN MISS | Home | -14.9 | 25.5 |

Projected lines and opponent power ratings are Makinen's, printed alongside the schedule on the team page.

## 18. Difficult Stretches / Trap Spots

Not addressed in guide under this heading.

The guide names this team on **pp. 342**, but no passage there was extracted under this heading; see those pages for VSiN's own wording.

## 19. Win Total Discussion

- **DraftKings win total:** 7.5 *(p. 317)*
- **Makinen projected wins:** 7.5 *(p. 317)*
- **Difference:** +0.0 wins against the posted number
- **Recommendation on the team page:** **OVER 7.5** *(p. 330)*

## 20. Futures / Conference / Playoff Discussion

Prices printed on the team page *(p. 331)*:

| Market | Price |
| --- | --- |
| CFP Championship | 1000-1 |
| make the playoff | 35-1 |
| SUN BELT (east) | +460 |

**VSiN staff predictions naming this team** *(p. 4)*:

| Category | Votes |
| --- | --- |
| SUN BELT CHAMPION | 4 of 22 |

## 21. Betting Notes / Best Bets

- Recommendation is Over 7.5 wins, resting almost entirely on the schedule being kind to Rahne. *(p. 330)*
- The specifics: James Madison and Marshall both come to Norfolk; the oddly timed non-conference road game at UConn is very winnable, as are dates with hapless Georgia State, newcomer Louisiana Tech and a Coastal Carolina team in transition; drawing Southern Miss and Louisiana Tech from the West is favourable; and the East Carolina game is at home. *(p. 330)*
- Its arithmetic behind the number: ODU is favoured in nine games and within +1.5 and +0.5 in two more, giving a projection of 7.51 wins. *(p. 330)*

Also referenced on **pp. 11** — not reproduced here.

## 22. Historical / Situational Trends

- The guide opens by locating Old Dominion inside someone else's season: Indiana won 10 of its 16 victories by 14 or more points, and one of the six closer games was against ODU, which was badly out-gained in the regular-season opener yet lost only 27-14 — a sign of things to come. *(p. 330)*
- Ricky Rahne's programme then improved by five wins to finish 10-3. *(p. 330)*
- What the guide finds remarkable is the shape of the losses: all three came by 13, 24 and 36 points, with ODU -5 in turnovers against Marshall and pummelled by James Madison. Otherwise, it notes, 10 wins was one fewer than the previous two seasons combined. *(p. 330)*

## 23. Important Statistics

As printed on p. 331. Each category carries a value and a national rank.

### Offence

| Category | Value | National rank |
| --- | --- | --- |
| POINTS PER GAME | 30.2 | 28 |
| YARDS PER POINT | 14.8 | 78 |
| PLAYS PER GAME | 70.1 | 54 |
| TIME OF POSSESSION | 28:30 | 110 |
| 3RD DOWN CONV. % | 42.4% | 37 |
| TOTAL YARDS PER GAME | 447.3 | 13 |
| YARDS PER PLAY | 6.4 | 16 |
| RUSH ATTEMPTS PER GAME | 43.3 | 12 |
| RUSH YARDS PER GAME | 236 | 7 |
| YARDS PER RUSH ATTEMPT | 5.5 | 9 |
| PASS ATTEMPTS PER GAME | 25.3 | 118 |
| COMPLETION % | 55.8% | 121 |
| PASSING YARDS PER GAME | 211.3 | 87 |
| YARDS PER PASS ATTEMPT | 8.4 | 18 |
| TURNOVERS | 1.9 | 129 |

### Defence

| Category | Value | National rank |
| --- | --- | --- |
| POINTS PER GAME | 19.6 | 15 |
| YARDS PER POINT | 17.3 | 12 |
| 3RD DOWN CONV. % | 34.8% | 27 |
| TOTAL YARDS PER GAME | 339.4 | 28 |
| YARDS PER PLAY | 4.6 | 13 |
| RUSH YARDS PER GAME | 140.4 | 47 |
| YARDS PER RUSH ATTEMPT | 3.7 | 30 |
| COMPLETION % | 61.3% | 58 |
| PASSING YARDS PER GAME | 199 | 31 |
| YARDS PER PASS ATTEMPT | 6.3 | 18 |
| SACKS | 3.1 | 4 |
| TURNOVERS | 1.9 | 11 |

## 24. Bull Case

- The team page recommends **OVER 7.5**.
- Ricky Rahne's programme then improved by five wins to finish 10-3. *(p. 330)*

*1 further statement about this team carries both positive and negative language and is not classified to either side. It appears in full under the season outlook and *Open Questions / Risks*.*

> *Assembled from statements the guide makes about this team. The statements are GUIDE CONTENT with page references; the selection of which ones argue this side is PERSONAL INFERENCE.*

## 25. Bear Case

- The guide opens by locating Old Dominion inside someone else's season: Indiana won 10 of its 16 victories by 14 or more points, and one of the six closer games was against ODU, which was badly out-gained in the regular-season opener yet lost only 27-14 — a sign of things to come. *(p. 330)*
- What the guide finds remarkable is the shape of the losses: all three came by 13, 24 and 36 points, with ODU -5 in turnovers against Marshall and pummelled by James Madison. Otherwise, it notes, 10 wins was one fewer than the previous two seasons combined. *(p. 330)*
- ODU is placed on the wrong side of that this year, with very heavy losses — headlined by quarterback Colton Joseph to Wisconsin after rushing for more than 1,000 yards and throwing for over 2,600 — and offensive coordinator Kevin Decker leaving for the same role at Memphis. *(p. 330)*

*1 further statement about this team carries both positive and negative language and is not classified to either side. It appears in full under the season outlook and *Open Questions / Risks*.*

> *Assembled from statements the guide makes about this team. The statements are GUIDE CONTENT with page references; the selection of which ones argue this side is PERSONAL INFERENCE.*

## 26. Open Questions / Risks

VSiN poses *Three Burning Questions for the 2026 Season* *(p. 331)*. Its analysis, in reference form:

### Can Cook keep the offense cooking?

Kody Cook is promoted to offensive coordinator after coaching wide receivers last season, and also takes the quarterbacks, with other shifts across the staff. The guide's concern is concentration: only three players had 11-plus catches, posting 762, 719 and 667 yards while nobody else exceeded 200 — and all three are gone, so Cook's old position group must be reimagined. It expects a run-heavy team again, listing Devin Roche at 5.7 yards per carry (well short of Trequan Jones' 7.5) and JUCO transfer Trey Hall, originally an Ole Miss commit. Its conclusion is that Quinn Henicle and Jaxon Potter will have skill players to work with, and that Cook — who played at Kansas State and coached at Tennessee — was the logical promotion after Decker's departure. *(p. 331)*

### Who gets more reps as the starting QB?

The guide weighs continuity against upside. Henicle is the holdover who knows the system, but it says plainly he was not very good — 4.8 yards per carry on 44 attempts, only 42.5% completions, and 11-for-25 against a skeleton-crew USF team in the bowl game. Potter was the higher-graded recruit and was willing to cross the country from Washington State as a California native. In Henicle's favour it notes he was recruited by FCS schools with good rushing attacks and by Army, so he was clearly seen as an impact runner at the position. Its verdict keeps both the doubt and the urgency: neither looks poised to top Joseph's numbers, and Rahne needs to get this right and fix it early in the season if he has not settled it coming out of camp. *(p. 331)*

### Should we buy or sell this defense?

The guide first credits Rahne, an offensive-minded coach, with finally rounding that side into form — up in yards per carry and points per game in each of the last three seasons — before arguing the real separator was Blake Seiler's defense. That unit cut 9.5 PPG and 0.9 yards per play from the previous season, holding opponents to 18.5 PPG and 4.8 yards per play, with ODU out-gaining opponents by 1.9 yards per play as a team. It tracks Seiler's pass rush across three years — 19 sacks, then 26, then 39 — alongside a top-25 third-down defense. Returning help includes two linebackers who started in 2024 but missed most of last season, plus returning starters at each level. Its calibrated verdict is to buy them as better than 2024 but worse than 2025. *(p. 331)*

*TTW reference notes. The questions are VSiN's; the answers summarise VSiN's reasoning in TTW's wording.*

## 27. Source Conflicts

- **Quarterback Database — source conflict / ambiguity.** The guide names two candidates and declines to state an expected starter, so this record carries no VSiN preseason expectation for the position. *Recorded in Phase 4, field 23 of this team's quarterback record — [source](../04_Quarterback_Database/old_dominion_monarchs.md).*

## 28. Relevant Page References

| Pages | Content |
| --- | --- |
| 330–331 | This team's two-page preview |
| 317 | Sun Belt conference preview and projected standings |
| 4 | VSiN staff season predictions |
| 46–47 | Steve Makinen power rating methodology |
| 21 | Home-field advantage article |
| 8, 11, 20, 59, 73, 184, 210, 318, 326, 328, 338, 342, 345 | Other pages naming this team |

This team is named in **18 sentences across 15 pages** of the guide.

Every page naming this team outside its own spread: pp. 8, 11, 20, 59, 73, 184, 210, 318, 326, 328, 338, 342, 345.

## 29. Cross-Links

**Conference** — [Sun Belt](../01_Conference_Database/sun_belt.md)

**Master Index** — [Team Index](../00_Master_Index/03_Team_Index.md) · [Power Rating Index](../00_Master_Index/09_Power_Rating_Index.md) · [Coaching Index](../00_Master_Index/04_Coaching_Index.md) · [Quarterback Index](../00_Master_Index/06_Quarterback_Index.md)

**Every other database, for this team:**

- Head coach — [03_Coaching_Database/old_dominion_monarchs.md](../03_Coaching_Database/old_dominion_monarchs.md)
- Quarterback — [04_Quarterback_Database/old_dominion_monarchs.md](../04_Quarterback_Database/old_dominion_monarchs.md)
- Power rating — [05_Power_Ratings/00_MAKINEN_RATINGS.md](../05_Power_Ratings/00_MAKINEN_RATINGS.md)
- Win total — [06_Win_Totals/00_ALL_TEAMS.md](../06_Win_Totals/00_ALL_TEAMS.md)
- Futures board — [07_Futures/00_TEAM_FUTURES.md](../07_Futures/00_TEAM_FUTURES.md)
- Returning production — [08_Returning_Production/README.md](../08_Returning_Production/README.md)
- Schedule — [10_Schedule_Intelligence/00_BY_TEAM.md](../10_Schedule_Intelligence/00_BY_TEAM.md)
- Historical angles — [12_Historical_Trends/00_BY_TEAM.md](../12_Historical_Trends/00_BY_TEAM.md)
- Everything at once — [99_Search_Index/02_TEAM_LOOKUP.md](../99_Search_Index/02_TEAM_LOOKUP.md)

