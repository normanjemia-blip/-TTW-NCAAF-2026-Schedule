<!-- GENERATED FILE — do not hand-edit.
     Rebuild:  python3 _tools/build_teams.py
     Source:   2026 VSiN College Football Betting Guide (345 pp.) -->

# San Jose State Spartans

> **Source: 2026 VSiN College Football Betting Guide, pp. 256–257** and every other page that names this team. GUIDE CONTENT throughout unless a line says otherwise. No outside research. Nothing inferred to fill a heading.


## 1. Program Snapshot

| | |
| --- | --- |
| **Conference** | Mountain West |
| **Guide pages** | 256–257 |
| **Head coach** | Ken Niumatalolo (3rd season) |
| **2025 straight up** | 3-9 |
| **2025 against the spread** | 4-8 |
| **2025 over/under** | 7-5 |
| **Makinen power rating** | 27.0 |
| **National rank** | #121 of 138 |
| **Conference rank** | #8 of 10 |
| **DraftKings win total** | 4.5 |
| **Makinen projected wins** | 4.5–8.5 overall, 2.8–5.2 in conference |
| **Schedule strength** | 34.39 (#90 toughest of 138) |

### Season outlook — VSiN's analysis in reference form (p. 256)

- The guide opens with a joke about blue being the school colour and sadness the emotion the programme evokes, then supplies the record: no bowl win since 2015 and no more than seven wins in a season since 2012 — with that lone bowl win coming in a 6-7 year. *(p. 256)*
- Ken Niumatalolo faces a large rebuild with only three returning starters and just two starters added through the portal. *(p. 256)*
- It credits his fantastic first season in a similar situation while identifying the difference: that came with an ingenious offense that surprised the league, and in his third season many opponents have now seen the tendencies. *(p. 256)*
- Recommendation is Under 4.5 wins. The guide entertains a repeat of 2023's seven-win surprise before explaining why the two ingredients — a surprising offense and a favorable schedule — are both absent. *(p. 256)*
- Schedule detail: difficult road trips to USC, Hawaii and Air Force; home games against an experienced Fresno State plus Wyoming, New Mexico, UNLV and North Dakota State; and road games at Eastern Michigan and UTEP that it rates coin flips at best. *(p. 256)*
- It closes with unusual candour about its own record, noting it was wrong about Niumatalolo in 2023 and is taking its chances saying he cannot do it again. *(p. 256)*

*TTW reference notes summarising VSiN's analysis. GUIDE CONTENT — facts, conclusions and reasoning are the guide's; the wording is TTW's.*


## 2. Conference

**Mountain West** — see the conference file for league-wide context: [mountain_west.md](../01_Conference_Database/mountain_west.md)

Makinen projects **2.8–5.2** in conference play, ranking this team #8 of 10 in the Mountain West by power rating.

## 3. VSiN Team Rank / Conference Rank

- **National rank:** #121 of 138 *(p. 257)*
- **Conference rank:** #8 of 10 *(p. 257)*
- **Matt Youmans' Preseason Top 50:** not ranked *(pp. 16–20)*

## 4. Steve Makinen Power Rating

**27.0** — as printed on p. 257, reproduced exactly and not converted to any other scale.

Ranks #121 of 138 nationally and #8 of 10 in the Mountain West. Methodology is set out on pp. 46–47.

> No comparison against the TTW workbook is made here. That is a later-phase deliverable.

## 5. Home-Field Advantage Reference

- **Home field rating:** 2.3 *(p. 256)*
- **Road field rating:** 0.5 *(p. 256)*

Printed on the team page as `field ratings (HOME/ROAD)` and verified against the same values in the conference projected standings table. The guide's general treatment of home-field advantage is on p. 21.

## 6. Head Coach

**Ken Niumatalolo** — 3rd season at the programme *(p. 256)*.

## 7. Coordinator Notes

- **OC Craig Stutzmann** *(p. 257)*

## 8. Coaching Continuity / Changes

**Steve Makinen's Stability Score: 7** *(p. 44)*

| Component | Status | Points |
| --- | --- | --- |
| Head coach | returns | 4 |
| Offensive coordinator | returns | 3 |
| Defensive coordinator | new for 2026 | 0 |
| Starting quarterback | new for 2026 | 0 |
| Returning starters | 3 | 0 |

Scoring is the guide's own: head coach 4, offensive coordinator 3, defensive coordinator 3, starting quarterback 4, plus points for returning starters. A returning transfer quarterback counts as a **new** quarterback in this system *(p. 41)*.

- Ken Niumatalolo faces a large rebuild with only three returning starters and just two starters added through the portal. *(p. 256)*
- It credits his fantastic first season in a similar situation while identifying the difference: that came with an ingenious offense that surprised the league, and in his third season many opponents have now seen the tendencies. *(p. 256)*
- **Can Niumatalolo do it again?** On paper the guide calls this rough. Per Phil Steele, San Jose State lost 24 starters to the portal and brought in two, with only three full-time starters returning from a three-win team. Receiver, defensive line and defensive back lost every starter and need rebuilding. It notes some former Power Four transfers are involved while adding the realistic caveat that all are making this move for a reason. Its conclusion credits Niumatalolo as a good coach with staff continuity who surprised the conference on arrival, while judging that he would need one of the best coaching jobs of a long career to get this group into the mix. *(p. 257)*

Also referenced on **pp. 27, 34, 86, 237, 273** — not reproduced here.

## 9. Quarterback Situation

**Starting quarterback continuity:** the Stability Score table treats this team as having a new starting quarterback for 2026 *(p. 44)*. A transfer expected to start counts as new in that system.

**Returning starting quarterback:** no — the guide marks returning quarterbacks with an asterisk on the returning-starters line *(p. 256)*.

- **Who will be doing the spreading and the shredding?** Coordinator Craig Stutzmann's Spread and Shred offense took the conference by storm two seasons ago before fizzling in 2025. The depth chart at quarterback is completely new apart from one-time UCLA transfer Robert McDaniel, with Luke Weaver the presumed winner. Weaver started two games at Hawaii in place of the injured Micah Alejado, against Sam Houston State and Portland State, winning both while completing 53 of 85 for 534 yards with five touchdowns and one interception. The guide describes him as a seasoned and well-travelled redshirt senior who worked his way up from Modesto Junior College, and judges he likely gives San Jose State the best chance in this system. *(p. 257)*

Also referenced on **pp. 86** — not reproduced here.

## 10. Returning Production

| | Returning starters |
| --- | --- |
| **Total** | 3 |
| **Offence** | 2 |
| **Defence** | 1 |

Read by position from p. 256. The three figures sit under the printed labels `total / offense / defense`; they are matched to those labels by their place on the page, because the PDF text layer emits them in a different order than they are printed.

The Stability Score table prints the same figure independently: **3 returning starters** *(p. 44)*.

## 11. Transfer Portal

- Ken Niumatalolo faces a large rebuild with only three returning starters and just two starters added through the portal. *(p. 256)*
- **Who will be doing the spreading and the shredding?** Coordinator Craig Stutzmann's Spread and Shred offense took the conference by storm two seasons ago before fizzling in 2025. The depth chart at quarterback is completely new apart from one-time UCLA transfer Robert McDaniel, with Luke Weaver the presumed winner. Weaver started two games at Hawaii in place of the injured Micah Alejado, against Sam Houston State and Portland State, winning both while completing 53 of 85 for 534 yards with five touchdowns and one interception. The guide describes him as a seasoned and well-travelled redshirt senior who worked his way up from Modesto Junior College, and judges he likely gives San Jose State the best chance in this system. *(p. 257)*
- **Does another wide receiver emerge?** The guide identifies a genuine pattern in Stutzmann's offense — a previously unheralded receiver erupting. Nick Nash produced 1,382 yards and 16 touchdowns in year one, and Danny Scudero caught 88 passes for 1,297 yards and 10 touchdowns last season. With the top five pass catchers gone and four transfers making up the room, it names Anthony Ivey as the one to watch, a former four-star from Penn State. It handles that even-handedly: the step down is obviously massive and could be a red flag, but the pedigree is there and so is a system built to maximize him. *(p. 257)*
- **Can Niumatalolo do it again?** On paper the guide calls this rough. Per Phil Steele, San Jose State lost 24 starters to the portal and brought in two, with only three full-time starters returning from a three-win team. Receiver, defensive line and defensive back lost every starter and need rebuilding. It notes some former Power Four transfers are involved while adding the realistic caveat that all are making this move for a reason. Its conclusion credits Niumatalolo as a good coach with staff continuity who surprised the conference on arrival, while judging that he would need one of the best coaching jobs of a long career to get this group into the mix. *(p. 257)*


Referenced in the guide on **pp. 86** — those passages are not reproduced here; see the pages for VSiN's own wording.

## 12. Recruiting / Roster Notes

- **Does another wide receiver emerge?** The guide identifies a genuine pattern in Stutzmann's offense — a previously unheralded receiver erupting. Nick Nash produced 1,382 yards and 16 touchdowns in year one, and Danny Scudero caught 88 passes for 1,297 yards and 10 touchdowns last season. With the top five pass catchers gone and four transfers making up the room, it names Anthony Ivey as the one to watch, a former four-star from Penn State. It handles that even-handedly: the step down is obviously massive and could be a red flag, but the pedigree is there and so is a system built to maximize him. *(p. 257)*


Referenced in the guide on **pp. 27** — those passages are not reproduced here; see the pages for VSiN's own wording.

## 13. Offensive Identity

- It credits his fantastic first season in a similar situation while identifying the difference: that came with an ingenious offense that surprised the league, and in his third season many opponents have now seen the tendencies. *(p. 256)*
- **Who will be doing the spreading and the shredding?** Coordinator Craig Stutzmann's Spread and Shred offense took the conference by storm two seasons ago before fizzling in 2025. The depth chart at quarterback is completely new apart from one-time UCLA transfer Robert McDaniel, with Luke Weaver the presumed winner. Weaver started two games at Hawaii in place of the injured Micah Alejado, against Sam Houston State and Portland State, winning both while completing 53 of 85 for 534 yards with five touchdowns and one interception. The guide describes him as a seasoned and well-travelled redshirt senior who worked his way up from Modesto Junior College, and judges he likely gives San Jose State the best chance in this system. *(p. 257)*
- **Does another wide receiver emerge?** The guide identifies a genuine pattern in Stutzmann's offense — a previously unheralded receiver erupting. Nick Nash produced 1,382 yards and 16 touchdowns in year one, and Danny Scudero caught 88 passes for 1,297 yards and 10 touchdowns last season. With the top five pass catchers gone and four transfers making up the room, it names Anthony Ivey as the one to watch, a former four-star from Penn State. It handles that even-handedly: the step down is obviously massive and could be a red flag, but the pedigree is there and so is a system built to maximize him. *(p. 257)*


Referenced in the guide on **pp. 179** — those passages are not reproduced here; see the pages for VSiN's own wording.

## 14. Defensive Identity

Not addressed in guide under this heading.

The guide names this team on **pp. 257, 273**, but no passage there was extracted under this heading; see those pages for VSiN's own wording.

## 15. Key Strengths

National top-25 finishes in the guide's printed categories *(p. 257)*:

| Side | Category | Value | National rank |
| --- | --- | --- | --- |
| Offense | PASS ATTEMPTS PER GAME | 42.5 | #2 |
| Offense | PASSING YARDS PER GAME | 288.7 | #8 |

## 16. Key Weaknesses

National bottom-25 finishes in the guide's printed categories *(p. 257)*:

| Side | Category | Value | National rank |
| --- | --- | --- | --- |
| Offense | RUSH ATTEMPTS PER GAME | 26.2 | #135 |
| Offense | TURNOVERS | 2.4 | #135 |
| Offense | YARDS PER POINT | 19.5 | #126 |
| Defense | YARDS PER PASS ATTEMPT | 8.7 | #125 |
| Defense | YARDS PER POINT | 12.3 | #124 |
| Offense | COMPLETION % | 55.7% | #122 |
| Defense | COMPLETION % | 66.5% | #118 |
| Offense | TIME OF POSSESSION | 28:21 | #116 |
| Defense | POINTS PER GAME | 32.9 | #116 |
| Offense | RUSH YARDS PER GAME | 112.9 | #114 |

## 17. Schedule Overview

Schedule strength **34.39**, **#90 toughest of 138** *(p. 256)*.

| Date | Opponent | Site | Projected line | Opponent power rating |
| --- | --- | --- | --- | --- |
| 8/29 | at USC | Away | +36.3 | 60.0 |
| 9/4 | at E Michigan | Away | +2.4 | 28.0 |
| 9/12 | CAL POLY SLO | Home | -10.3 | 19.5 |
| 9/19 | FRESNO ST | Home | +12.9 | 42.5 |
| 10/3 | at Hawaii | Away | +14.7 | 38.5 |
| 10/9 | WYOMING | Home | +0.6 | 30.5 |
| 10/17 | at UTEP | Away | -3.4 | 22.5 |
| 10/24 | at Nevada | Away | +1.9 | 28.0 |
| 10/31 | NEW MEXICO | Home | +12.5 | 42.0 |
| 11/7 | N ILLINOIS | Home | -7.3 | 21.5 |
| 11/14 | at Air Force | Away | +12 | 37.0 |
| 11/21 | UNLV | Home | +15.6 | 44.0 |
| 11/27 | N DAKOTA ST | Home | +8.2 | 37.0 |

Projected lines and opponent power ratings are Makinen's, printed alongside the schedule on the team page.

## 18. Difficult Stretches / Trap Spots

Not addressed in guide.

## 19. Win Total Discussion

- **DraftKings win total:** 4.5 *(p. 243)*
- **Makinen projected wins:** 4.5 *(p. 243)*
- **Difference:** +0.0 wins against the posted number
- **Recommendation on the team page:** **UNDER 4.5** *(p. 256)*

**Selected as one of Steve Makinen's win-total bets** *(pp. 22–27)*: **UNDER 4.5**.

## 20. Futures / Conference / Playoff Discussion

Prices printed on the team page *(p. 257)*:

| Market | Price |
| --- | --- |
| CFP Championship | 5000-1 |
| make the playoff | 400-1 |
| MOUNTAIN WEST | 20-1 |

## 21. Betting Notes / Best Bets

- Recommendation is Under 4.5 wins. The guide entertains a repeat of 2023's seven-win surprise before explaining why the two ingredients — a surprising offense and a favorable schedule — are both absent. *(p. 256)*
- It closes with unusual candour about its own record, noting it was wrong about Niumatalolo in 2023 and is taking its chances saying he cannot do it again. *(p. 256)*

Also referenced on **pp. 40** — not reproduced here.

## 22. Historical / Situational Trends

- The guide opens with a joke about blue being the school colour and sadness the emotion the programme evokes, then supplies the record: no bowl win since 2015 and no more than seven wins in a season since 2012 — with that lone bowl win coming in a 6-7 year. *(p. 256)*


Referenced in the guide on **pp. 86** — those passages are not reproduced here; see the pages for VSiN's own wording.

## 23. Important Statistics

As printed on p. 257. Each category carries a value and a national rank.

### Offence

| Category | Value | National rank |
| --- | --- | --- |
| POINTS PER GAME | 20.5 | 108 |
| YARDS PER POINT | 19.5 | 126 |
| PLAYS PER GAME | 69.5 | 59 |
| TIME OF POSSESSION | 28:21 | 116 |
| 3RD DOWN CONV. % | 40.8% | 52 |
| TOTAL YARDS PER GAME | 401.6 | 41 |
| YARDS PER PLAY | 5.8 | 36 |
| RUSH ATTEMPTS PER GAME | 26.2 | 135 |
| RUSH YARDS PER GAME | 112.9 | 114 |
| YARDS PER RUSH ATTEMPT | 4.3 | 61 |
| PASS ATTEMPTS PER GAME | 42.5 | 2 |
| COMPLETION % | 55.7% | 122 |
| PASSING YARDS PER GAME | 288.7 | 8 |
| YARDS PER PASS ATTEMPT | 6.8 | 85 |
| TURNOVERS | 2.4 | 135 |

### Defence

| Category | Value | National rank |
| --- | --- | --- |
| POINTS PER GAME | 32.9 | 116 |
| YARDS PER POINT | 12.3 | 124 |
| 3RD DOWN CONV. % | 35.1% | 33 |
| TOTAL YARDS PER GAME | 405.9 | 88 |
| YARDS PER PLAY | 5.9 | 103 |
| RUSH YARDS PER GAME | 159.5 | 74 |
| YARDS PER RUSH ATTEMPT | 4.1 | 53 |
| COMPLETION % | 66.5% | 118 |
| PASSING YARDS PER GAME | 246.5 | 109 |
| YARDS PER PASS ATTEMPT | 8.7 | 125 |
| SACKS | 1.5 | 104 |
| TURNOVERS | 1.2 | 77 |

## 24. Bull Case

- It credits his fantastic first season in a similar situation while identifying the difference: that came with an ingenious offense that surprised the league, and in his third season many opponents have now seen the tendencies. *(p. 256)*
- Recommendation is Under 4.5 wins. The guide entertains a repeat of 2023's seven-win surprise before explaining why the two ingredients — a surprising offense and a favorable schedule — are both absent. *(p. 256)*

*1 further statement about this team carries both positive and negative language and is not classified to either side. It appears in full under the season outlook and *Open Questions / Risks*.*

> *Assembled from statements the guide makes about this team. The statements are GUIDE CONTENT with page references; the selection of which ones argue this side is PERSONAL INFERENCE.*

## 25. Bear Case

- The team page recommends **UNDER 4.5**.

*1 further statement about this team carries both positive and negative language and is not classified to either side. It appears in full under the season outlook and *Open Questions / Risks*.*

> *Assembled from statements the guide makes about this team. The statements are GUIDE CONTENT with page references; the selection of which ones argue this side is PERSONAL INFERENCE.*

## 26. Open Questions / Risks

VSiN poses *Three Burning Questions for the 2026 Season* *(p. 257)*. Its analysis, in reference form:

### Who will be doing the spreading and the shredding?

Coordinator Craig Stutzmann's Spread and Shred offense took the conference by storm two seasons ago before fizzling in 2025. The depth chart at quarterback is completely new apart from one-time UCLA transfer Robert McDaniel, with Luke Weaver the presumed winner. Weaver started two games at Hawaii in place of the injured Micah Alejado, against Sam Houston State and Portland State, winning both while completing 53 of 85 for 534 yards with five touchdowns and one interception. The guide describes him as a seasoned and well-travelled redshirt senior who worked his way up from Modesto Junior College, and judges he likely gives San Jose State the best chance in this system. *(p. 257)*

### Does another wide receiver emerge?

The guide identifies a genuine pattern in Stutzmann's offense — a previously unheralded receiver erupting. Nick Nash produced 1,382 yards and 16 touchdowns in year one, and Danny Scudero caught 88 passes for 1,297 yards and 10 touchdowns last season. With the top five pass catchers gone and four transfers making up the room, it names Anthony Ivey as the one to watch, a former four-star from Penn State. It handles that even-handedly: the step down is obviously massive and could be a red flag, but the pedigree is there and so is a system built to maximize him. *(p. 257)*

### Can Niumatalolo do it again?

On paper the guide calls this rough. Per Phil Steele, San Jose State lost 24 starters to the portal and brought in two, with only three full-time starters returning from a three-win team. Receiver, defensive line and defensive back lost every starter and need rebuilding. It notes some former Power Four transfers are involved while adding the realistic caveat that all are making this move for a reason. Its conclusion credits Niumatalolo as a good coach with staff continuity who surprised the conference on arrival, while judging that he would need one of the best coaching jobs of a long career to get this group into the mix. *(p. 257)*

*TTW reference notes. The questions are VSiN's; the answers summarise VSiN's reasoning in TTW's wording.*

## 27. Source Conflicts

No source conflict identified for this team.

## 28. Relevant Page References

| Pages | Content |
| --- | --- |
| 256–257 | This team's two-page preview |
| 243 | Mountain West conference preview and projected standings |
| 4 | VSiN staff season predictions |
| 46–47 | Steve Makinen power rating methodology |
| 21 | Home-field advantage article |
| 5, 27, 34, 40, 86, 116, 179, 218, 237, 260, 270, 273 | Other pages naming this team |

This team is named in **23 sentences across 14 pages** of the guide.

Every page naming this team outside its own spread: pp. 5, 27, 34, 40, 86, 116, 179, 218, 237, 260, 270, 273.

## 29. Cross-Links

**Conference** — [Mountain West](../01_Conference_Database/mountain_west.md)

**Master Index** — [Team Index](../00_Master_Index/03_Team_Index.md) · [Power Rating Index](../00_Master_Index/09_Power_Rating_Index.md) · [Coaching Index](../00_Master_Index/04_Coaching_Index.md) · [Quarterback Index](../00_Master_Index/06_Quarterback_Index.md)

**Every other database, for this team:**

- Head coach — [03_Coaching_Database/san_jose_state_spartans.md](../03_Coaching_Database/san_jose_state_spartans.md)
- Quarterback — [04_Quarterback_Database/san_jose_state_spartans.md](../04_Quarterback_Database/san_jose_state_spartans.md)
- Power rating — [05_Power_Ratings/00_MAKINEN_RATINGS.md](../05_Power_Ratings/00_MAKINEN_RATINGS.md)
- Win total — [06_Win_Totals/00_ALL_TEAMS.md](../06_Win_Totals/00_ALL_TEAMS.md)
- Futures board — [07_Futures/00_TEAM_FUTURES.md](../07_Futures/00_TEAM_FUTURES.md)
- Returning production — [08_Returning_Production/README.md](../08_Returning_Production/README.md)
- Schedule — [10_Schedule_Intelligence/00_BY_TEAM.md](../10_Schedule_Intelligence/00_BY_TEAM.md)
- Historical angles — [12_Historical_Trends/00_BY_TEAM.md](../12_Historical_Trends/00_BY_TEAM.md)
- Everything at once — [99_Search_Index/02_TEAM_LOOKUP.md](../99_Search_Index/02_TEAM_LOOKUP.md)

