<!-- GENERATED FILE — do not hand-edit.
     Rebuild:  python3 _tools/build_teams.py
     Source:   2026 VSiN College Football Betting Guide (345 pp.) -->

# Army Black Knights

> **Source: 2026 VSiN College Football Betting Guide, pp. 50–51** and every other page that names this team. GUIDE CONTENT throughout unless a line says otherwise. No outside research. Nothing inferred to fill a heading.


## 1. Program Snapshot

| | |
| --- | --- |
| **Conference** | American |
| **Guide pages** | 50–51 |
| **Head coach** | Jeff Monken (13th season) |
| **2025 straight up** | 7-6 |
| **2025 against the spread** | 7-6 |
| **2025 over/under** | 5-8 |
| **Makinen power rating** | 40.0 |
| **National rank** | #78 of 138 |
| **Conference rank** | #7 of 14 |
| **DraftKings win total** | 7.5 |
| **Makinen projected wins** | 7.5–4.5 overall, 4.8–3.2 in conference |
| **Schedule strength** | 34.11 (#92 toughest of 138) |

### Season outlook — VSiN's analysis in reference form (p. 50)

- Navy held the advantage entering last season because Blake Horvath still had eligibility while Bryson Daily did not, and the guide notes Daily's loss was compounded by star running back Kanye Udoh leaving. *(p. 50)*
- Repeating 12-2 was always going to be insurmountable, and the guide reads a 7-6 winning record — helped by a bowl blowout of a severely undermanned UConn — as further evidence of how solid Jeff Monken has been. *(p. 50)*
- Monken begins his 13th year. The guide makes the tenure point vividly: setting aside UTSA's Jeff Traylor, his 12 years nearly equal the rest of the conference's head coaches combined. *(p. 50)*
- Army has had one losing season since 2015 and again has a high floor. *(p. 50)*
- Recommendation is Under 7.5 wins. A Hellums injury would be brutal since there is no game experience behind him, and losing Woody hurts the defense. *(p. 50)*
- The core statistical argument is a reversal: Army was outgained by nearly a full yard per play in conference play. The guide concedes some of that is expected given how much they run relative to opponents, but stresses it was not the case in 2024, when Army outgained conference foes by nearly 1.2 yards per play. *(p. 50)*
- It reads that as the conference adjusting, noting it took Navy a long time to adjust back when the same happened to them. *(p. 50)*
- Situationally, Army plays at a rest disadvantage in each of its first three October games, and UTSA is the only projected top-half conference team it avoids. Projection is 6.92 wins. *(p. 50)*

*TTW reference notes summarising VSiN's analysis. GUIDE CONTENT — facts, conclusions and reasoning are the guide's; the wording is TTW's.*


## 2. Conference

**American** — see the conference file for league-wide context: [american.md](../01_Conference_Database/american.md)

Makinen projects **4.8–3.2** in conference play, ranking this team #7 of 14 in the American by power rating.

## 3. VSiN Team Rank / Conference Rank

- **National rank:** #78 of 138 *(p. 51)*
- **Conference rank:** #7 of 14 *(p. 51)*
- **Matt Youmans' Preseason Top 50:** not ranked *(pp. 16–20)*

## 4. Steve Makinen Power Rating

**40.0** — as printed on p. 51, reproduced exactly and not converted to any other scale.

Ranks #78 of 138 nationally and #7 of 14 in the American. Methodology is set out on pp. 46–47.

> No comparison against the TTW workbook is made here. That is a later-phase deliverable.

## 5. Home-Field Advantage Reference

- **Home field rating:** 2.5 *(p. 50)*
- **Road field rating:** 0.8 *(p. 50)*

Printed on the team page as `field ratings (HOME/ROAD)` and verified against the same values in the conference projected standings table. The guide's general treatment of home-field advantage is on p. 21.

## 6. Head Coach

**Jeff Monken** — 13th season at the programme *(p. 50)*.

## 7. Coordinator Notes

- **DC Nate Woody** *(p. 51, 125)*
- **OC Cody Worley** *(p. 51)*

## 8. Coaching Continuity / Changes

**Steve Makinen's Stability Score: 13** *(p. 42)*

| Component | Status | Points |
| --- | --- | --- |
| Head coach | returns | 4 |
| Offensive coordinator | returns | 3 |
| Defensive coordinator | new for 2026 | 0 |
| Starting quarterback | returns | 4 |
| Returning starters | 11 | 2 |

Scoring is the guide's own: head coach 4, offensive coordinator 3, defensive coordinator 3, starting quarterback 4, plus points for returning starters. A returning transfer quarterback counts as a **new** quarterback in this system *(p. 41)*.

- Repeating 12-2 was always going to be insurmountable, and the guide reads a 7-6 winning record — helped by a bowl blowout of a severely undermanned UConn — as further evidence of how solid Jeff Monken has been. *(p. 50)*
- Monken begins his 13th year. The guide makes the tenure point vividly: setting aside UTSA's Jeff Traylor, his 12 years nearly equal the rest of the conference's head coaches combined. *(p. 50)*
- **Who is going to step up on defense?** With coordinator Nate Woody now at Cincinnati, pass game coordinator Daryl Dixon takes the role. The guide notes Dixon has been with the programme for ten seasons so he knows the scheme and the player types. Army is almost always undersized in the trenches but usually experienced — and this season is an exception: despite many seniors on the two-deep, the top seven tacklers are gone, including Andon Thomas and his team-leading 113 tackles. Dixon, mostly a defensive backs coach, needs more negative plays after Army managed 15 sacks. *(p. 51)*

Also referenced on **pp. 125, 224** — not reproduced here.

## 9. Quarterback Situation

**Starting quarterback continuity:** the Stability Score table treats this team as having the same starting quarterback as 2025 *(p. 42)*. A transfer expected to start counts as new in that system.

**Returning starting quarterback:** yes — the guide marks returning quarterbacks with an asterisk on the returning-starters line *(p. 50)*.

- Navy held the advantage entering last season because Blake Horvath still had eligibility while Bryson Daily did not, and the guide notes Daily's loss was compounded by star running back Kanye Udoh leaving. *(p. 50)*
- Recommendation is Under 7.5 wins. A Hellums injury would be brutal since there is no game experience behind him, and losing Woody hurts the defense. *(p. 50)*
- **Can Cale complete the season?** Cale Hellums, who split time to a degree with Dewayne Coleman, carried 304 times for 1,223 yards and 18 touchdowns while adding four passing scores and 694 yards. The guide's concern is workload: that is a brutal load for a 5-foot-10 player and must be factored into any long-term position on Army. It allows Monken and coordinator Cody Worley may have found something in converted quarterback Godspower Nwawuihe, who ran for 171 yards in the Fenway Bowl. With Noah Short, Hayden Reed, Coleman and Briggs Bartosh all gone, it stresses somebody must take carries off Hellums, who had nearly 41% of them. *(p. 51)*

Also referenced on **pp. 331** — not reproduced here.

## 10. Returning Production

| | Returning starters |
| --- | --- |
| **Total** | 11 |
| **Offence** | 8 *(includes the returning quarterback)* |
| **Defence** | 3 |

Read by position from p. 50. The three figures sit under the printed labels `total / offense / defense`; they are matched to those labels by their place on the page, because the PDF text layer emits them in a different order than they are printed.

The Stability Score table prints the same figure independently: **11 returning starters** *(p. 42)*.

## 11. Transfer Portal

Not addressed in guide.

## 12. Recruiting / Roster Notes

Not addressed in guide under this heading.

The guide names this team on **pp. 331**, but no passage there was extracted under this heading; see those pages for VSiN's own wording.

## 13. Offensive Identity

- The core statistical argument is a reversal: Army was outgained by nearly a full yard per play in conference play. The guide concedes some of that is expected given how much they run relative to opponents, but stresses it was not the case in 2024, when Army outgained conference foes by nearly 1.2 yards per play. *(p. 50)*
- **Can Cale complete the season?** Cale Hellums, who split time to a degree with Dewayne Coleman, carried 304 times for 1,223 yards and 18 touchdowns while adding four passing scores and 694 yards. The guide's concern is workload: that is a brutal load for a 5-foot-10 player and must be factored into any long-term position on Army. It allows Monken and coordinator Cody Worley may have found something in converted quarterback Godspower Nwawuihe, who ran for 171 yards in the Fenway Bowl. With Noah Short, Hayden Reed, Coleman and Briggs Bartosh all gone, it stresses somebody must take carries off Hellums, who had nearly 41% of them. *(p. 51)*
- **Will the conference adjust to Army in Year 3?** Army is 12-4 in American play, but the guide breaks that into 8-0 in year one and 4-4 in year two. Its historical parallel is precise: when Navy joined the AAC in 2015 it went 7-1 in league play in each of its first two seasons before falling to 4-4 in year three and 2-6 in year four. It acknowledges the absences of Daily and Udoh played a role while citing the underlying decline from 5.7 yards per carry in 2024 conference games to 4.38 in 2025. It also notes Army is +12 in turnover margin across two years of conference play, granting that its style limits turnover volume while calling it something to think about. *(p. 51)*


Referenced in the guide on **pp. 224, 331** — those passages are not reproduced here; see the pages for VSiN's own wording.

## 14. Defensive Identity

- Recommendation is Under 7.5 wins. A Hellums injury would be brutal since there is no game experience behind him, and losing Woody hurts the defense. *(p. 50)*
- The core statistical argument is a reversal: Army was outgained by nearly a full yard per play in conference play. The guide concedes some of that is expected given how much they run relative to opponents, but stresses it was not the case in 2024, when Army outgained conference foes by nearly 1.2 yards per play. *(p. 50)*
- **Who is going to step up on defense?** With coordinator Nate Woody now at Cincinnati, pass game coordinator Daryl Dixon takes the role. The guide notes Dixon has been with the programme for ten seasons so he knows the scheme and the player types. Army is almost always undersized in the trenches but usually experienced — and this season is an exception: despite many seniors on the two-deep, the top seven tacklers are gone, including Andon Thomas and his team-leading 113 tackles. Dixon, mostly a defensive backs coach, needs more negative plays after Army managed 15 sacks. *(p. 51)*


Referenced in the guide on **pp. 125** — those passages are not reproduced here; see the pages for VSiN's own wording.

## 15. Key Strengths

National top-25 finishes in the guide's printed categories *(p. 51)*:

| Side | Category | Value | National rank |
| --- | --- | --- | --- |
| Offense | TIME OF POSSESSION | 34:25 | #1 |
| Offense | RUSH ATTEMPTS PER GAME | 56.7 | #1 |
| Offense | RUSH YARDS PER GAME | 253.1 | #4 |
| Offense | TURNOVERS | 0.7 | #5 |
| Offense | YARDS PER PASS ATTEMPT | 8.4 | #16 |
| Defense | PASSING YARDS PER GAME | 193.6 | #23 |
| Defense | YARDS PER POINT | 16.4 | #25 |

## 16. Key Weaknesses

National bottom-25 finishes in the guide's printed categories *(p. 51)*:

| Side | Category | Value | National rank |
| --- | --- | --- | --- |
| Offense | PASS ATTEMPTS PER GAME | 9.1 | #136 |
| Offense | PASSING YARDS PER GAME | 76.7 | #136 |
| Defense | SACKS | 1 | #128 |
| Offense | COMPLETION % | 55.1% | #125 |
| Defense | 3RD DOWN CONV. % | 46.3% | #124 |

## 17. Schedule Overview

Schedule strength **34.11**, **#92 toughest of 138** *(p. 50)*.

| Date | Opponent | Site | Projected line | Opponent power rating |
| --- | --- | --- | --- | --- |
| 9/5 | BRYANT | Home | -36.6 | 5.5 |
| 9/12 | SOUTH FLORIDA | Home | -1.2 | 41.0 |
| 9/25 | at Temple | Away | -3.4 | 34.0 |
| 10/3 | at Louisiana Tech | Away | -3.9 | 33.0 |
| 10/10 | TULANE | Home | +0.6 | 42.5 |
| 10/17 | FLA ATLANTIC | Home | -9.5 | 33.5 |
| 10/23 | at Tulsa | Away | -6.8 | 32.5 |
| 10/31 | at Memphis | Away | +3.1 | 41.0 |
| 11/7 | AIR FORCE | Home | -5.6 | 37.0 |
| 11/21 | EAST CAROLINA | Home | -1.3 | 40.5 |
| 11/28 | at Rice | Away | -10.1 | 27.5 |
| 12/12 | vs. Navy | Neutral | +4 | 44.0 |

Projected lines and opponent power ratings are Makinen's, printed alongside the schedule on the team page.

## 18. Difficult Stretches / Trap Spots

- Situationally, Army plays at a rest disadvantage in each of its first three October games, and UTSA is the only projected top-half conference team it avoids. Projection is 6.92 wins. *(p. 50)*


Referenced in the guide on **pp. 54** — those passages are not reproduced here; see the pages for VSiN's own wording.

## 19. Win Total Discussion

- **DraftKings win total:** 7.5 *(p. 49)*
- **Makinen projected wins:** 7.5 *(p. 49)*
- **Difference:** +0.0 wins against the posted number
- **Recommendation on the team page:** **UNDER 7.5** *(p. 50)*
- **Projection stated in the team-page essay:** 6.92 wins *(p. 50)*

## 20. Futures / Conference / Playoff Discussion

Prices printed on the team page *(p. 51)*:

| Market | Price |
| --- | --- |
| CFP Championship | 1000-1 |
| make the playoff | 10-1 |
| AMERICAN | +850 |

## 21. Betting Notes / Best Bets

- Recommendation is Under 7.5 wins. A Hellums injury would be brutal since there is no game experience behind him, and losing Woody hurts the defense. *(p. 50)*
- The core statistical argument is a reversal: Army was outgained by nearly a full yard per play in conference play. The guide concedes some of that is expected given how much they run relative to opponents, but stresses it was not the case in 2024, when Army outgained conference foes by nearly 1.2 yards per play. *(p. 50)*
- Situationally, Army plays at a rest disadvantage in each of its first three October games, and UTSA is the only projected top-half conference team it avoids. Projection is 6.92 wins. *(p. 50)*

Also referenced on **pp. 49, 77** — not reproduced here.

## 22. Historical / Situational Trends

- Navy held the advantage entering last season because Blake Horvath still had eligibility while Bryson Daily did not, and the guide notes Daily's loss was compounded by star running back Kanye Udoh leaving. *(p. 50)*
- Repeating 12-2 was always going to be insurmountable, and the guide reads a 7-6 winning record — helped by a bowl blowout of a severely undermanned UConn — as further evidence of how solid Jeff Monken has been. *(p. 50)*
- Army has had one losing season since 2015 and again has a high floor. *(p. 50)*
- It reads that as the conference adjusting, noting it took Navy a long time to adjust back when the same happened to them. *(p. 50)*
- **Will the conference adjust to Army in Year 3?** Army is 12-4 in American play, but the guide breaks that into 8-0 in year one and 4-4 in year two. Its historical parallel is precise: when Navy joined the AAC in 2015 it went 7-1 in league play in each of its first two seasons before falling to 4-4 in year three and 2-6 in year four. It acknowledges the absences of Daily and Udoh played a role while citing the underlying decline from 5.7 yards per carry in 2024 conference games to 4.38 in 2025. It also notes Army is +12 in turnover margin across two years of conference play, granting that its style limits turnover volume while calling it something to think about. *(p. 51)*


Referenced in the guide on **pp. 65, 68, 77, 337** — those passages are not reproduced here; see the pages for VSiN's own wording.

## 23. Important Statistics

As printed on p. 51. Each category carries a value and a national rank.

### Offence

| Category | Value | National rank |
| --- | --- | --- |
| POINTS PER GAME | 23.6 | 87 |
| YARDS PER POINT | 14 | 54 |
| PLAYS PER GAME | 66.3 | 97 |
| TIME OF POSSESSION | 34:25 | 1 |
| 3RD DOWN CONV. % | 41.5% | 44 |
| TOTAL YARDS PER GAME | 329.8 | 105 |
| YARDS PER PLAY | 5 | 106 |
| RUSH ATTEMPTS PER GAME | 56.7 | 1 |
| RUSH YARDS PER GAME | 253.1 | 4 |
| YARDS PER RUSH ATTEMPT | 4.5 | 47 |
| PASS ATTEMPTS PER GAME | 9.1 | 136 |
| COMPLETION % | 55.1% | 125 |
| PASSING YARDS PER GAME | 76.7 | 136 |
| YARDS PER PASS ATTEMPT | 8.4 | 16 |
| TURNOVERS | 0.7 | 5 |

### Defence

| Category | Value | National rank |
| --- | --- | --- |
| POINTS PER GAME | 20.9 | 27 |
| YARDS PER POINT | 16.4 | 25 |
| 3RD DOWN CONV. % | 46.3% | 124 |
| TOTAL YARDS PER GAME | 343.5 | 37 |
| YARDS PER PLAY | 5.6 | 73 |
| RUSH YARDS PER GAME | 149.9 | 63 |
| YARDS PER RUSH ATTEMPT | 4.5 | 81 |
| COMPLETION % | 64.4% | 99 |
| PASSING YARDS PER GAME | 193.6 | 23 |
| YARDS PER PASS ATTEMPT | 7.1 | 60 |
| SACKS | 1 | 128 |
| TURNOVERS | 1.3 | 58 |

## 24. Bull Case

- Repeating 12-2 was always going to be insurmountable, and the guide reads a 7-6 winning record — helped by a bowl blowout of a severely undermanned UConn — as further evidence of how solid Jeff Monken has been. *(p. 50)*
- Army has had one losing season since 2015 and again has a high floor. *(p. 50)*

*1 further statement about this team carries both positive and negative language and is not classified to either side. It appears in full under the season outlook and *Open Questions / Risks*.*

> *Assembled from statements the guide makes about this team. The statements are GUIDE CONTENT with page references; the selection of which ones argue this side is PERSONAL INFERENCE.*

## 25. Bear Case

- The team page recommends **UNDER 7.5**.
- Navy held the advantage entering last season because Blake Horvath still had eligibility while Bryson Daily did not, and the guide notes Daily's loss was compounded by star running back Kanye Udoh leaving. *(p. 50)*
- Recommendation is Under 7.5 wins. A Hellums injury would be brutal since there is no game experience behind him, and losing Woody hurts the defense. *(p. 50)*

*1 further statement about this team carries both positive and negative language and is not classified to either side. It appears in full under the season outlook and *Open Questions / Risks*.*

> *Assembled from statements the guide makes about this team. The statements are GUIDE CONTENT with page references; the selection of which ones argue this side is PERSONAL INFERENCE.*

## 26. Open Questions / Risks

VSiN poses *Three Burning Questions for the 2026 Season* *(p. 51)*. Its analysis, in reference form:

### Can Cale complete the season?

Cale Hellums, who split time to a degree with Dewayne Coleman, carried 304 times for 1,223 yards and 18 touchdowns while adding four passing scores and 694 yards. The guide's concern is workload: that is a brutal load for a 5-foot-10 player and must be factored into any long-term position on Army. It allows Monken and coordinator Cody Worley may have found something in converted quarterback Godspower Nwawuihe, who ran for 171 yards in the Fenway Bowl. With Noah Short, Hayden Reed, Coleman and Briggs Bartosh all gone, it stresses somebody must take carries off Hellums, who had nearly 41% of them. *(p. 51)*

### Will the conference adjust to Army in Year 3?

Army is 12-4 in American play, but the guide breaks that into 8-0 in year one and 4-4 in year two. Its historical parallel is precise: when Navy joined the AAC in 2015 it went 7-1 in league play in each of its first two seasons before falling to 4-4 in year three and 2-6 in year four. It acknowledges the absences of Daily and Udoh played a role while citing the underlying decline from 5.7 yards per carry in 2024 conference games to 4.38 in 2025. It also notes Army is +12 in turnover margin across two years of conference play, granting that its style limits turnover volume while calling it something to think about. *(p. 51)*

### Who is going to step up on defense?

With coordinator Nate Woody now at Cincinnati, pass game coordinator Daryl Dixon takes the role. The guide notes Dixon has been with the programme for ten seasons so he knows the scheme and the player types. Army is almost always undersized in the trenches but usually experienced — and this season is an exception: despite many seniors on the two-deep, the top seven tacklers are gone, including Andon Thomas and his team-leading 113 tackles. Dixon, mostly a defensive backs coach, needs more negative plays after Army managed 15 sacks. *(p. 51)*

*TTW reference notes. The questions are VSiN's; the answers summarise VSiN's reasoning in TTW's wording.*

## 27. Source Conflicts

No source conflict identified for this team.

## 28. Relevant Page References

| Pages | Content |
| --- | --- |
| 50–51 | This team's two-page preview |
| 49 | American conference preview and projected standings |
| 4 | VSiN staff season predictions |
| 46–47 | Steve Makinen power rating methodology |
| 21 | Home-field advantage article |
| 54, 64, 65, 68, 69, 70, 76, 77, 125, 197, 224, 230, 331, 337 | Other pages naming this team |

This team is named in **32 sentences across 17 pages** of the guide.

Every page naming this team outside its own spread: pp. 54, 64, 65, 68, 69, 70, 76, 77, 125, 197, 224, 230, 331, 337.

## 29. Cross-Links

**Conference** — [American](../01_Conference_Database/american.md)

**Master Index** — [Team Index](../00_Master_Index/03_Team_Index.md) · [Power Rating Index](../00_Master_Index/09_Power_Rating_Index.md) · [Coaching Index](../00_Master_Index/04_Coaching_Index.md) · [Quarterback Index](../00_Master_Index/06_Quarterback_Index.md)

**Every other database, for this team:**

- Head coach — [03_Coaching_Database/army_black_knights.md](../03_Coaching_Database/army_black_knights.md)
- Quarterback — [04_Quarterback_Database/army_black_knights.md](../04_Quarterback_Database/army_black_knights.md)
- Power rating — [05_Power_Ratings/00_MAKINEN_RATINGS.md](../05_Power_Ratings/00_MAKINEN_RATINGS.md)
- Win total — [06_Win_Totals/00_ALL_TEAMS.md](../06_Win_Totals/00_ALL_TEAMS.md)
- Futures board — [07_Futures/00_TEAM_FUTURES.md](../07_Futures/00_TEAM_FUTURES.md)
- Returning production — [08_Returning_Production/README.md](../08_Returning_Production/README.md)
- Schedule — [10_Schedule_Intelligence/00_BY_TEAM.md](../10_Schedule_Intelligence/00_BY_TEAM.md)
- Historical angles — [12_Historical_Trends/00_BY_TEAM.md](../12_Historical_Trends/00_BY_TEAM.md)
- Everything at once — [99_Search_Index/02_TEAM_LOOKUP.md](../99_Search_Index/02_TEAM_LOOKUP.md)

