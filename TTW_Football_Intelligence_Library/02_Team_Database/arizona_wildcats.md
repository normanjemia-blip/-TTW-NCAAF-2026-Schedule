<!-- GENERATED FILE — do not hand-edit.
     Rebuild:  python3 _tools/build_teams.py
     Source:   2026 VSiN College Football Betting Guide (345 pp.) -->

# Arizona Wildcats

> **Source: 2026 VSiN College Football Betting Guide, pp. 116–117** and every other page that names this team. GUIDE CONTENT throughout unless a line says otherwise. No outside research. Nothing inferred to fill a heading.


## 1. Program Snapshot

| | |
| --- | --- |
| **Conference** | Big 12 |
| **Guide pages** | 116–117 |
| **Head coach** | Brent Brennan (3rd season) |
| **2025 straight up** | 9-4 |
| **2025 against the spread** | 8-5 |
| **2025 over/under** | 4-9 |
| **Makinen power rating** | 52.5 |
| **National rank** | #29 of 138 |
| **Conference rank** | #5 of 16 |
| **DraftKings win total** | 7.5 |
| **Makinen projected wins** | 7.8–4.2 overall, 5.0–4.0 in conference |
| **Schedule strength** | 44.57 (#46 toughest of 138) |

### Season outlook — VSiN's analysis in reference form (p. 116)

- Brent Brennan inherited a hard job, following Jedd Fisch after a successful run at San Jose State. Fisch had turned Arizona around before leaving for a good Big Ten job, and the guide notes the late start left Brennan no time to get things right. *(p. 116)*
- Arizona fell to 4-8 in 2024, putting Brennan on the hot seat, and he answered with a 9-4 season in 2025 that the guide says was a couple of very close losses from something bigger. *(p. 116)*
- His staff changes are called a home run, his portal work strong and his recruiting instincts sound, leaving him in good standing with the university. *(p. 116)*
- Expectations are somewhat high with star quarterback Noah Fifita back alongside 13 other starters. *(p. 116)*
- Recommendation is Under 7.5 wins. The guide's arithmetic: Arizona will be an underdog against BYU and Texas Tech and plays six further games it treats as coin flips, so eight wins feels unrealistic even if the team is as good as last year's. *(p. 116)*
- A regression argument is added explicitly — Arizona had a +18 turnover margin last year, and the guide expects some of that to come back. Its conclusion is that Brennan is doing good work but a slight dip may be coming. *(p. 116)*

*TTW reference notes summarising VSiN's analysis. GUIDE CONTENT — facts, conclusions and reasoning are the guide's; the wording is TTW's.*


## 2. Conference

**Big 12** — see the conference file for league-wide context: [big_12.md](../01_Conference_Database/big_12.md)

Makinen projects **5.0–4.0** in conference play, ranking this team #5 of 16 in the Big 12 by power rating.

## 3. VSiN Team Rank / Conference Rank

- **National rank:** #29 of 138 *(p. 117)*
- **Conference rank:** #5 of 16 *(p. 117)*
- **Matt Youmans' Preseason Top 50:** **#28** *(pp. 16–20)*

## 4. Steve Makinen Power Rating

**52.5** — as printed on p. 117, reproduced exactly and not converted to any other scale.

Ranks #29 of 138 nationally and #5 of 16 in the Big 12. Methodology is set out on pp. 46–47.

> No comparison against the TTW workbook is made here. That is a later-phase deliverable.

## 5. Home-Field Advantage Reference

- **Home field rating:** 2.7 *(p. 116)*
- **Road field rating:** 0.5 *(p. 116)*

Printed on the team page as `field ratings (HOME/ROAD)` and verified against the same values in the conference projected standings table. The guide's general treatment of home-field advantage is on p. 21.

## 6. Head Coach

**Brent Brennan** — 3rd season at the programme *(p. 116)*.

## 7. Coordinator Notes

- **DC Danny Gonzales** *(p. 117)*
- **OC Seth Doege** *(p. 117)*

## 8. Coaching Continuity / Changes

**Steve Makinen's Stability Score: 17** *(p. 42)*

| Component | Status | Points |
| --- | --- | --- |
| Head coach | returns | 4 |
| Offensive coordinator | returns | 3 |
| Defensive coordinator | returns | 3 |
| Starting quarterback | returns | 4 |
| Returning starters | 14 | 3 |

Scoring is the guide's own: head coach 4, offensive coordinator 3, defensive coordinator 3, starting quarterback 4, plus points for returning starters. A returning transfer quarterback counts as a **new** quarterback in this system *(p. 41)*.

- Brent Brennan inherited a hard job, following Jedd Fisch after a successful run at San Jose State. Fisch had turned Arizona around before leaving for a good Big Ten job, and the guide notes the late start left Brennan no time to get things right. *(p. 116)*
- Arizona fell to 4-8 in 2024, putting Brennan on the hot seat, and he answered with a 9-4 season in 2025 that the guide says was a couple of very close losses from something bigger. *(p. 116)*
- His staff changes are called a home run, his portal work strong and his recruiting instincts sound, leaving him in good standing with the university. *(p. 116)*

Also referenced on **pp. 34, 237** — not reproduced here.

## 9. Quarterback Situation

**Starting quarterback continuity:** the Stability Score table treats this team as having the same starting quarterback as 2025 *(p. 42)*. A transfer expected to start counts as new in that system.

**Returning starting quarterback:** yes — the guide marks returning quarterbacks with an asterisk on the returning-starters line *(p. 116)*.

- Expectations are somewhat high with star quarterback Noah Fifita back alongside 13 other starters. *(p. 116)*
- **What type of year can be expected from Noah Fifita?** Fifita threw for 3,228 yards with 29 touchdowns and six interceptions and finished as PFF's 17th-ranked passer, a major bounceback from a poor 2024 that makes him one of the top returning starters. He has solid weapons, a decent run game and an established rapport with returning coordinator Seth Doege. The guide then deliberately withholds the easy conclusion, listing three reasons for doubt: the 2024 season is hard to ignore entirely, Arizona must replace three offensive line starters, and Fifita occasionally has injury trouble. Its closing line is a caution — replicating or building on 2025 is the hope, but that is also what Tucson hoped for in 2024. *(p. 117)*

## 10. Returning Production

| | Returning starters |
| --- | --- |
| **Total** | 12 |
| **Offence** | 4 *(includes the returning quarterback)* |
| **Defence** | 8 |

Read by position from p. 116. The three figures sit under the printed labels `total / offense / defense`; they are matched to those labels by their place on the page, because the PDF text layer emits them in a different order than they are printed.

The Stability Score table prints the same figure independently: **14 returning starters** *(p. 42)*.

> **SOURCE CONFLICT.** The team page (p. 116) prints 12 returning starters; the Stability Score table (p. 42) prints 14 for the same team. The team page is internally consistent — offence 4 plus defence 8 equals its own total — so both figures are reproduced as printed and neither is corrected.

## 11. Transfer Portal

- His staff changes are called a home run, his portal work strong and his recruiting instincts sound, leaving him in good standing with the university. *(p. 116)*
- **Can the Arizona passing defense survive key losses in the secondary?** Arizona tied for seventh in Game on Paper's Adjusted EPA per play allowed at -0.14, driven by a third-ranked EPA per dropback allowed of -0.24. Coordinator Danny Gonzales is credited with excellent work but faces a harder task: four defensive backs left for the NFL, including second-round pick and All-Big 12 corner Treydan Stukes and All-Big 12 safety Dalton Johnson. Brennan and Gonzales hit the portal hard for potential instant-impact starters. The guide's warning is precise — last year's defense was special specifically because it slowed the conference's most explosive passing games, so any substantial secondary dropoff will hurt. *(p. 117)*

## 12. Recruiting / Roster Notes

- His staff changes are called a home run, his portal work strong and his recruiting instincts sound, leaving him in good standing with the university. *(p. 116)*

## 13. Offensive Identity

- **What type of year can be expected from Noah Fifita?** Fifita threw for 3,228 yards with 29 touchdowns and six interceptions and finished as PFF's 17th-ranked passer, a major bounceback from a poor 2024 that makes him one of the top returning starters. He has solid weapons, a decent run game and an established rapport with returning coordinator Seth Doege. The guide then deliberately withholds the easy conclusion, listing three reasons for doubt: the 2024 season is hard to ignore entirely, Arizona must replace three offensive line starters, and Fifita occasionally has injury trouble. Its closing line is a caution — replicating or building on 2025 is the hope, but that is also what Tucson hoped for in 2024. *(p. 117)*

## 14. Defensive Identity

- **Can the Arizona passing defense survive key losses in the secondary?** Arizona tied for seventh in Game on Paper's Adjusted EPA per play allowed at -0.14, driven by a third-ranked EPA per dropback allowed of -0.24. Coordinator Danny Gonzales is credited with excellent work but faces a harder task: four defensive backs left for the NFL, including second-round pick and All-Big 12 corner Treydan Stukes and All-Big 12 safety Dalton Johnson. Brennan and Gonzales hit the portal hard for potential instant-impact starters. The guide's warning is precise — last year's defense was special specifically because it slowed the conference's most explosive passing games, so any substantial secondary dropoff will hurt. *(p. 117)*

## 15. Key Strengths

National top-25 finishes in the guide's printed categories *(p. 117)*:

| Side | Category | Value | National rank |
| --- | --- | --- | --- |
| Defense | TURNOVERS | 2.4 | #1 |
| Defense | YARDS PER PASS ATTEMPT | 5.8 | #6 |
| Defense | PASSING YARDS PER GAME | 173.4 | #7 |
| Defense | COMPLETION % | 54.9% | #9 |
| Defense | YARDS PER PLAY | 4.7 | #14 |
| Defense | TOTAL YARDS PER GAME | 318.9 | #20 |
| Defense | POINTS PER GAME | 20.7 | #24 |

## 16. Key Weaknesses

No category ranks inside the national bottom 25.

## 17. Schedule Overview

Schedule strength **44.57**, **#46 toughest of 138** *(p. 116)*.

| Date | Opponent | Site | Projected line | Opponent power rating |
| --- | --- | --- | --- | --- |
| 9/5 | N ARIZONA | Home | -32.8 | 22.0 |
| 9/12 | at BYU | Away | +7.7 | 57.5 |
| 9/19 | N ILLINOIS | Home | -33.2 | 21.5 |
| 9/26 | at Washington St | Away | -11.6 | 38.0 |
| 10/3 | CINCINNATI | Home | -11 | 44.5 |
| 10/10 | at West Virginia | Away | -5.2 | 45.0 |
| 10/24 | IOWA ST | Home | -13.4 | 41.5 |
| 10/31 | at Texas Tech | Away | +12.4 | 61.5 |
| 11/6 | TCU | Home | -4.2 | 51.0 |
| 11/14 | UTAH | Home | -0.5 | 54.5 |
| 11/21 | at Kansas St | Away | +1.8 | 51.0 |
| 11/28 | ARIZONA ST | Home | -5.2 | 49.5 |

Projected lines and opponent power ratings are Makinen's, printed alongside the schedule on the team page.

## 18. Difficult Stretches / Trap Spots

Not addressed in guide.

## 19. Win Total Discussion

- **DraftKings win total:** 7.5 *(p. 115)*
- **Makinen projected wins:** 7.8 *(p. 115)*
- **Difference:** +0.3 wins against the posted number
- **Recommendation on the team page:** **UNDER 7.5** *(p. 116)*

## 20. Futures / Conference / Playoff Discussion

Prices printed on the team page *(p. 117)*:

| Market | Price |
| --- | --- |
| CFP Championship | 250-1 |
| make the playoff | 10-1 |
| BIG 12 | 19-1 |

## 21. Betting Notes / Best Bets

- Recommendation is Under 7.5 wins. The guide's arithmetic: Arizona will be an underdog against BYU and Texas Tech and plays six further games it treats as coin flips, so eight wins feels unrealistic even if the team is as good as last year's. *(p. 116)*
- A regression argument is added explicitly — Arizona had a +18 turnover margin last year, and the guide expects some of that to come back. Its conclusion is that Brennan is doing good work but a slight dip may be coming. *(p. 116)*

## 22. Historical / Situational Trends

- Brent Brennan inherited a hard job, following Jedd Fisch after a successful run at San Jose State. Fisch had turned Arizona around before leaving for a good Big Ten job, and the guide notes the late start left Brennan no time to get things right. *(p. 116)*
- Arizona fell to 4-8 in 2024, putting Brennan on the hot seat, and he answered with a 9-4 season in 2025 that the guide says was a couple of very close losses from something bigger. *(p. 116)*
- A regression argument is added explicitly — Arizona had a +18 turnover margin last year, and the guide expects some of that to come back. Its conclusion is that Brennan is doing good work but a slight dip may be coming. *(p. 116)*


Referenced in the guide on **pp. 19, 102, 270** — those passages are not reproduced here; see the pages for VSiN's own wording.

## 23. Important Statistics

As printed on p. 117. Each category carries a value and a national rank.

### Offence

| Category | Value | National rank |
| --- | --- | --- |
| POINTS PER GAME | 30.2 | 28 |
| YARDS PER POINT | 13.1 | 27 |
| PLAYS PER GAME | 72.5 | 31 |
| TIME OF POSSESSION | 30:48 | 41 |
| 3RD DOWN CONV. % | 40.5% | 54 |
| TOTAL YARDS PER GAME | 395.3 | 49 |
| YARDS PER PLAY | 5.5 | 68 |
| RUSH ATTEMPTS PER GAME | 35.3 | 63 |
| RUSH YARDS PER GAME | 152.6 | 64 |
| YARDS PER RUSH ATTEMPT | 4.3 | 60 |
| PASS ATTEMPTS PER GAME | 34.9 | 27 |
| COMPLETION % | 62.3% | 60 |
| PASSING YARDS PER GAME | 242.7 | 49 |
| YARDS PER PASS ATTEMPT | 6.9 | 77 |
| TURNOVERS | 1.1 | 38 |

### Defence

| Category | Value | National rank |
| --- | --- | --- |
| POINTS PER GAME | 20.7 | 24 |
| YARDS PER POINT | 15.4 | 39 |
| 3RD DOWN CONV. % | 35.5% | 36 |
| TOTAL YARDS PER GAME | 318.9 | 20 |
| YARDS PER PLAY | 4.7 | 14 |
| RUSH YARDS PER GAME | 145.5 | 57 |
| YARDS PER RUSH ATTEMPT | 4 | 45 |
| COMPLETION % | 54.9% | 9 |
| PASSING YARDS PER GAME | 173.4 | 7 |
| YARDS PER PASS ATTEMPT | 5.8 | 6 |
| SACKS | 1.6 | 101 |
| TURNOVERS | 2.4 | 1 |

## 24. Bull Case

- Makinen projects **7.8 wins** against a posted total of 7.5 — **+0.3** in this team's favour.
- Brent Brennan inherited a hard job, following Jedd Fisch after a successful run at San Jose State. Fisch had turned Arizona around before leaving for a good Big Ten job, and the guide notes the late start left Brennan no time to get things right. *(p. 116)*
- His staff changes are called a home run, his portal work strong and his recruiting instincts sound, leaving him in good standing with the university. *(p. 116)*
- Recommendation is Under 7.5 wins. The guide's arithmetic: Arizona will be an underdog against BYU and Texas Tech and plays six further games it treats as coin flips, so eight wins feels unrealistic even if the team is as good as last year's. *(p. 116)*

*1 further statement about this team carries both positive and negative language and is not classified to either side. It appears in full under the season outlook and *Open Questions / Risks*.*

> *Assembled from statements the guide makes about this team. The statements are GUIDE CONTENT with page references; the selection of which ones argue this side is PERSONAL INFERENCE.*

## 25. Bear Case

- The team page recommends **UNDER 7.5**.
- Arizona fell to 4-8 in 2024, putting Brennan on the hot seat, and he answered with a 9-4 season in 2025 that the guide says was a couple of very close losses from something bigger. *(p. 116)*

*1 further statement about this team carries both positive and negative language and is not classified to either side. It appears in full under the season outlook and *Open Questions / Risks*.*

> *Assembled from statements the guide makes about this team. The statements are GUIDE CONTENT with page references; the selection of which ones argue this side is PERSONAL INFERENCE.*

## 26. Open Questions / Risks

VSiN poses *Three Burning Questions for the 2026 Season* *(p. 117)*. Its analysis, in reference form:

### What type of year can be expected from Noah Fifita?

Fifita threw for 3,228 yards with 29 touchdowns and six interceptions and finished as PFF's 17th-ranked passer, a major bounceback from a poor 2024 that makes him one of the top returning starters. He has solid weapons, a decent run game and an established rapport with returning coordinator Seth Doege. The guide then deliberately withholds the easy conclusion, listing three reasons for doubt: the 2024 season is hard to ignore entirely, Arizona must replace three offensive line starters, and Fifita occasionally has injury trouble. Its closing line is a caution — replicating or building on 2025 is the hope, but that is also what Tucson hoped for in 2024. *(p. 117)*

### Can the Arizona passing defense survive key losses in the secondary?

Arizona tied for seventh in Game on Paper's Adjusted EPA per play allowed at -0.14, driven by a third-ranked EPA per dropback allowed of -0.24. Coordinator Danny Gonzales is credited with excellent work but faces a harder task: four defensive backs left for the NFL, including second-round pick and All-Big 12 corner Treydan Stukes and All-Big 12 safety Dalton Johnson. Brennan and Gonzales hit the portal hard for potential instant-impact starters. The guide's warning is precise — last year's defense was special specifically because it slowed the conference's most explosive passing games, so any substantial secondary dropoff will hurt. *(p. 117)*

### Is the schedule simply too tough for the Wildcats?

Road games come at BYU, Texas Tech, Kansas State and West Virginia. The first two are expected near the top of the Big 12; Kansas State's 7.5 win total suggests it is on par with Arizona and would likely be favored in Manhattan; and the guide notes West Virginia has historically been a hard place to play when the team is good, with the Mountaineers expected to be much better. Arizona also hosts Utah, TCU, Cincinnati and Arizona State. Its conclusion is that finishing above .500 in conference play will be a challenge. *(p. 117)*

*TTW reference notes. The questions are VSiN's; the answers summarise VSiN's reasoning in TTW's wording.*

## 27. Source Conflicts

- **Returning starters printed differently in two places.** The team page (p. 116) prints 12 returning starters; the Stability Score table (p. 42) prints 14 for the same team. The team page is internally consistent — offence 4 plus defence 8 equals its own total — so both figures are reproduced as printed and neither is corrected.
- **Quarterback Database — source conflict / ambiguity.** The team page (p. 116) prints 12 returning starters while the Stability Score table (p. 42) prints 14 for the same team. Both are reproduced as printed in the Team Database and neither is corrected here. *Recorded in Phase 4, field 23 of this team's quarterback record — [source](../04_Quarterback_Database/arizona_wildcats.md).*

## 28. Relevant Page References

| Pages | Content |
| --- | --- |
| 116–117 | This team's two-page preview |
| 115 | Big 12 conference preview and projected standings |
| 4 | VSiN staff season predictions |
| 46–47 | Steve Makinen power rating methodology |
| 21 | Home-field advantage article |
| 15, 19, 34, 101, 102, 134, 138, 218, 237, 254, 265, 270, 280 | Other pages naming this team |

This team is named in **30 sentences across 16 pages** of the guide.

Every page naming this team outside its own spread: pp. 15, 19, 34, 101, 102, 134, 138, 218, 237, 254, 265, 270, 280.

## 29. Cross-Links

**Conference** — [Big 12](../01_Conference_Database/big_12.md)

**Master Index** — [Team Index](../00_Master_Index/03_Team_Index.md) · [Power Rating Index](../00_Master_Index/09_Power_Rating_Index.md) · [Coaching Index](../00_Master_Index/04_Coaching_Index.md) · [Quarterback Index](../00_Master_Index/06_Quarterback_Index.md)

**Every other database, for this team:**

- Head coach — [03_Coaching_Database/arizona_wildcats.md](../03_Coaching_Database/arizona_wildcats.md)
- Quarterback — [04_Quarterback_Database/arizona_wildcats.md](../04_Quarterback_Database/arizona_wildcats.md)
- Power rating — [05_Power_Ratings/00_MAKINEN_RATINGS.md](../05_Power_Ratings/00_MAKINEN_RATINGS.md)
- Win total — [06_Win_Totals/00_ALL_TEAMS.md](../06_Win_Totals/00_ALL_TEAMS.md)
- Futures board — [07_Futures/00_TEAM_FUTURES.md](../07_Futures/00_TEAM_FUTURES.md)
- Returning production — [08_Returning_Production/README.md](../08_Returning_Production/README.md)
- Schedule — [10_Schedule_Intelligence/00_BY_TEAM.md](../10_Schedule_Intelligence/00_BY_TEAM.md)
- Historical angles — [12_Historical_Trends/00_BY_TEAM.md](../12_Historical_Trends/00_BY_TEAM.md)
- Everything at once — [99_Search_Index/02_TEAM_LOOKUP.md](../99_Search_Index/02_TEAM_LOOKUP.md)

