<!-- GENERATED FILE — do not hand-edit.
     Rebuild:  python3 _tools/build_teams.py
     Source:   2026 VSiN College Football Betting Guide (345 pp.) -->

# Toledo Rockets

> **Source: 2026 VSiN College Football Betting Guide, pp. 238–239** and every other page that names this team. GUIDE CONTENT throughout unless a line says otherwise. No outside research. Nothing inferred to fill a heading.


## 1. Program Snapshot

| | |
| --- | --- |
| **Conference** | MAC |
| **Guide pages** | 238–239 |
| **Head coach** | Mike Jacobs (1st season) |
| **2025 straight up** | 8-5 |
| **2025 against the spread** | 9-4 |
| **2025 over/under** | 7-6 |
| **Makinen power rating** | 35.5 |
| **National rank** | #93 of 138 |
| **Conference rank** | #3 of 13 |
| **DraftKings win total** | 7.5 |
| **Makinen projected wins** | 8.2–3.8 overall, 5.9–2.1 in conference |
| **Schedule strength** | 27.24 (#132 toughest of 138) |

### Season outlook — VSiN's analysis in reference form (p. 238)

- The guide marks the scale of the break: for the first time since 2016 Toledo will not play for Jason Candle, and for the first time since 2009 Candle is not with the programme at all, having taken the UConn job. *(p. 238)*
- Toledo moved quickly to hire Mike Jacobs, who spent the last two seasons with Mercer at the FCS level. *(p. 238)*
- It emphasises the regional coherence of the staff: Jacobs is a northwest Ohio native, coordinator Cris Reisert is the former Tiffin and Ohio Dominican head coach, and defensive coordinator Jahmal Brown is a Cleveland native who came from Youngstown State. *(p. 238)*
- Jacobs' background is given in full: a long snapper and offensive lineman at Ohio State from 1997-2001 who moved into coaching almost immediately, with a first head-coaching job at Notre Dame College about half an hour from downtown Cleveland. *(p. 238)*
- It calls this something of a step up for the 47-year-old while making a strong claim on his behalf — he took Mercer to the playoffs in back-to-back seasons and may well be an upgrade on Candle, which it notes is hard to say about a coach who won 65% of his games overall and 68% in conference. *(p. 238)*
- Recommendation is Over 7.5 wins, resting first on a soft Year 1 schedule: Toledo misses Miami of Ohio, hosts reigning champion Western Michigan, and gets a visit from San Diego State in non-conference. *(p. 238)*
- It expects a slow start, warning the opener against Michigan State may not be pretty — particularly for a defense that allowed just 13.3 PPG and 96 rushing yards per game last season and must now be rebuilt. *(p. 238)*
- Its conclusion is that Jacobs seems like a good hire, with at least some familiarity thanks to transfers from a team that finished in the FCS Top 25 in each of the last two seasons. Projection is 8.04 wins. *(p. 238)*

*TTW reference notes summarising VSiN's analysis. GUIDE CONTENT — facts, conclusions and reasoning are the guide's; the wording is TTW's.*


## 2. Conference

**MAC** — see the conference file for league-wide context: [mac.md](../01_Conference_Database/mac.md)

Makinen projects **5.9–2.1** in conference play, ranking this team #3 of 13 in the MAC by power rating.

## 3. VSiN Team Rank / Conference Rank

- **National rank:** #93 of 138 *(p. 239)*
- **Conference rank:** #3 of 13 *(p. 239)*
- **Matt Youmans' Preseason Top 50:** not ranked *(pp. 16–20)*

## 4. Steve Makinen Power Rating

**35.5** — as printed on p. 239, reproduced exactly and not converted to any other scale.

Ranks #93 of 138 nationally and #3 of 13 in the MAC. Methodology is set out on pp. 46–47.

> No comparison against the TTW workbook is made here. That is a later-phase deliverable.

## 5. Home-Field Advantage Reference

- **Home field rating:** 4 *(p. 238)*
- **Road field rating:** -0.5 *(p. 238)*

Printed on the team page as `field ratings (HOME/ROAD)` and verified against the same values in the conference projected standings table. The guide's general treatment of home-field advantage is on p. 21.

## 6. Head Coach

**Mike Jacobs** — 1st season at the programme *(p. 238)*.

Profiled among the guide's new head coaches in The Coaching Carousel *(p. 34)*.

## 7. Coordinator Notes

- **DC Jahmal Brown** *(p. 238)*
- **OC Cris Reisert** *(p. 238)*

## 8. Coaching Continuity / Changes

**Steve Makinen's Stability Score: 0** *(p. 44)*

| Component | Status | Points |
| --- | --- | --- |
| Head coach | new for 2026 | 0 |
| Offensive coordinator | new for 2026 | 0 |
| Defensive coordinator | new for 2026 | 0 |
| Starting quarterback | new for 2026 | 0 |
| Returning starters | 6 | 0 |

Scoring is the guide's own: head coach 4, offensive coordinator 3, defensive coordinator 3, starting quarterback 4, plus points for returning starters. A returning transfer quarterback counts as a **new** quarterback in this system *(p. 41)*.

- The guide marks the scale of the break: for the first time since 2016 Toledo will not play for Jason Candle, and for the first time since 2009 Candle is not with the programme at all, having taken the UConn job. *(p. 238)*
- Toledo moved quickly to hire Mike Jacobs, who spent the last two seasons with Mercer at the FCS level. *(p. 238)*
- It emphasises the regional coherence of the staff: Jacobs is a northwest Ohio native, coordinator Cris Reisert is the former Tiffin and Ohio Dominican head coach, and defensive coordinator Jahmal Brown is a Cleveland native who came from Youngstown State. *(p. 238)*
- Jacobs' background is given in full: a long snapper and offensive lineman at Ohio State from 1997-2001 who moved into coaching almost immediately, with a first head-coaching job at Notre Dame College about half an hour from downtown Cleveland. *(p. 238)*
- It calls this something of a step up for the 47-year-old while making a strong claim on his behalf — he took Mercer to the playoffs in back-to-back seasons and may well be an upgrade on Candle, which it notes is hard to say about a coach who won 65% of his games overall and 68% in conference. *(p. 238)*
- Its conclusion is that Jacobs seems like a good hire, with at least some familiarity thanks to transfers from a team that finished in the FCS Top 25 in each of the last two seasons. Projection is 8.04 wins. *(p. 238)*
- **Holy Toledo, who are these guys?** The guide's point is the near-total turnover: virtually no recognisable names remain, with the top returning tackler at 35 tackles and the top returnee in yards of any kind at 194 receiving yards. It explains the mechanism rather than treating it as a surprise — Candle left as many of his players were running out of eligibility, and a head coach leaving opens the portal completely. 247Sports lists 37 transfers for Toledo, the third-best MAC class in its ranking. It names John Alan Richter as a returnee Rockets fans may recognise, and notes the incoming group is heavy with Mercer players who followed Jacobs, including running back CJ Miller and receiver Adjatay Dabbs, plus offensive linemen familiar to Reisert who came with him from Gardner-Webb. *(p. 239)*
- **What kind of offense should we expect to see?** The guide projects from Mercer's numbers under Jacobs: 6.8 yards per play last season with a balanced approach that ranked third nationally in yards per game, 4.9 yards per carry on more than 400 rushing attempts, and over 3,800 passing yards. It corrects for game state in reading Toledo's own tendencies — the Rockets may have been a little more run-heavy, but that reflects protecting a lot of leads, some of them big. It then flags that this year's team will not have last year's defense, so the game state itself is unknown. Its expectation is a lean on the run at the outset, given Miller, Penn State transfer Cory Smith and two senior tackles who played for Toledo last season. *(p. 239)*

Also referenced on **pp. 6, 24, 34, 107, 209, 210, 211, 215, 231** — not reproduced here.

## 9. Quarterback Situation

**Starting quarterback continuity:** the Stability Score table treats this team as having a new starting quarterback for 2026 *(p. 44)*. A transfer expected to start counts as new in that system.

**Returning starting quarterback:** no — the guide marks returning quarterbacks with an asterisk on the returning-starters line *(p. 238)*.


Also referenced on **pp. 233** — not reproduced here.

## 10. Returning Production

| | Returning starters |
| --- | --- |
| **Total** | 6 |
| **Offence** | 2 |
| **Defence** | 4 |

Read by position from p. 238. The three figures sit under the printed labels `total / offense / defense`; they are matched to those labels by their place on the page, because the PDF text layer emits them in a different order than they are printed.

The Stability Score table prints the same figure independently: **6 returning starters** *(p. 44)*.

## 11. Transfer Portal

- Its conclusion is that Jacobs seems like a good hire, with at least some familiarity thanks to transfers from a team that finished in the FCS Top 25 in each of the last two seasons. Projection is 8.04 wins. *(p. 238)*
- **Holy Toledo, who are these guys?** The guide's point is the near-total turnover: virtually no recognisable names remain, with the top returning tackler at 35 tackles and the top returnee in yards of any kind at 194 receiving yards. It explains the mechanism rather than treating it as a surprise — Candle left as many of his players were running out of eligibility, and a head coach leaving opens the portal completely. 247Sports lists 37 transfers for Toledo, the third-best MAC class in its ranking. It names John Alan Richter as a returnee Rockets fans may recognise, and notes the incoming group is heavy with Mercer players who followed Jacobs, including running back CJ Miller and receiver Adjatay Dabbs, plus offensive linemen familiar to Reisert who came with him from Gardner-Webb. *(p. 239)*
- **What kind of offense should we expect to see?** The guide projects from Mercer's numbers under Jacobs: 6.8 yards per play last season with a balanced approach that ranked third nationally in yards per game, 4.9 yards per carry on more than 400 rushing attempts, and over 3,800 passing yards. It corrects for game state in reading Toledo's own tendencies — the Rockets may have been a little more run-heavy, but that reflects protecting a lot of leads, some of them big. It then flags that this year's team will not have last year's defense, so the game state itself is unknown. Its expectation is a lean on the run at the outset, given Miller, Penn State transfer Cory Smith and two senior tackles who played for Toledo last season. *(p. 239)*
- **What kind of defense should we expect to see?** The guide expects aggression under Brown, and again reasons from Mercer: last year's group allowed only 3.4 yards per carry but 5.7 yards per play, because it got burned for explosives in the passing game. The Bears had 42 sacks in 12 games yet generated only 15 takeaways, and their 96 tackles for loss show how heavily they pursued the run and rushed the passer — while leaving an evasive quarterback the chance to burn them. It weighs that risk against the league, noting there are some running quarterbacks in the MAC but fewer this season than in other years, and concludes that with quite a few Mercer transfers who already know the scheme this should be an exciting defense to watch. *(p. 239)*

## 12. Recruiting / Roster Notes

Not addressed in guide under this heading.

The guide names this team on **pp. 211**, but no passage there was extracted under this heading; see those pages for VSiN's own wording.

## 13. Offensive Identity

- **What kind of offense should we expect to see?** The guide projects from Mercer's numbers under Jacobs: 6.8 yards per play last season with a balanced approach that ranked third nationally in yards per game, 4.9 yards per carry on more than 400 rushing attempts, and over 3,800 passing yards. It corrects for game state in reading Toledo's own tendencies — the Rockets may have been a little more run-heavy, but that reflects protecting a lot of leads, some of them big. It then flags that this year's team will not have last year's defense, so the game state itself is unknown. Its expectation is a lean on the run at the outset, given Miller, Penn State transfer Cory Smith and two senior tackles who played for Toledo last season. *(p. 239)*


Referenced in the guide on **pp. 27, 231** — those passages are not reproduced here; see the pages for VSiN's own wording.

## 14. Defensive Identity

- It expects a slow start, warning the opener against Michigan State may not be pretty — particularly for a defense that allowed just 13.3 PPG and 96 rushing yards per game last season and must now be rebuilt. *(p. 238)*
- **What kind of defense should we expect to see?** The guide expects aggression under Brown, and again reasons from Mercer: last year's group allowed only 3.4 yards per carry but 5.7 yards per play, because it got burned for explosives in the passing game. The Bears had 42 sacks in 12 games yet generated only 15 takeaways, and their 96 tackles for loss show how heavily they pursued the run and rushed the passer — while leaving an evasive quarterback the chance to burn them. It weighs that risk against the league, noting there are some running quarterbacks in the MAC but fewer this season than in other years, and concludes that with quite a few Mercer transfers who already know the scheme this should be an exciting defense to watch. *(p. 239)*

## 15. Key Strengths

National top-25 finishes in the guide's printed categories *(p. 239)*:

| Side | Category | Value | National rank |
| --- | --- | --- | --- |
| Defense | COMPLETION % | 49.9% | #2 |
| Defense | TOTAL YARDS PER GAME | 273.9 | #3 |
| Defense | YARDS PER PLAY | 4.1 | #3 |
| Defense | POINTS PER GAME | 14.4 | #4 |
| Defense | YARDS PER POINT | 19 | #5 |
| Defense | PASSING YARDS PER GAME | 169.5 | #5 |
| Defense | YARDS PER RUSH ATTEMPT | 2.9 | #6 |
| Defense | 3RD DOWN CONV. % | 31.0% | #7 |
| Defense | YARDS PER PASS ATTEMPT | 6 | #9 |
| Defense | RUSH YARDS PER GAME | 104.4 | #14 |
| Defense | SACKS | 2.8 | #14 |

## 16. Key Weaknesses

No category ranks inside the national bottom 25.

## 17. Schedule Overview

Schedule strength **27.24**, **#132 toughest of 138** *(p. 238)*.

| Date | Opponent | Site | Projected line | Opponent power rating |
| --- | --- | --- | --- | --- |
| 9/4 | at Michigan St | Away | +8.7 | 42.5 |
| 9/12 | C CONN ST | Home | -27.8 | 12.0 |
| 9/19 | TEMPLE | Home | -6.3 | 34.0 |
| 9/26 | SAN DIEGO ST | Home | +3.6 | 43.0 |
| 10/3 | at Ball St | Away | -11.3 | 20.0 |
| 10/10 | BUFFALO | Home | -11.1 | 28.0 |
| 10/17 | at E Michigan | Away | -5.1 | 28.0 |
| 10/24 | W MICHIGAN | Home | -2.3 | 37.5 |
| 11/4 | at Sacramento St | Away | -11.7 | 21.5 |
| 11/11 | MASSACHUSETTS | Home | -23.8 | 16.0 |
| 11/20 | BOWLING GREEN | Home | -12.3 | 26.5 |
| 11/27 | at Ohio U | Away | +0.3 | 31.5 |

Projected lines and opponent power ratings are Makinen's, printed alongside the schedule on the team page.

## 18. Difficult Stretches / Trap Spots

Not addressed in guide under this heading.

The guide names this team on **pp. 234, 238**, but no passage there was extracted under this heading; see those pages for VSiN's own wording.

## 19. Win Total Discussion

- **DraftKings win total:** 7.5 *(p. 215)*
- **Makinen projected wins:** 8.2 *(p. 215)*
- **Difference:** +0.7 wins against the posted number
- **Recommendation on the team page:** **OVER 7.5** *(p. 238)*
- **Projection stated in the team-page essay:** 8.04 wins *(p. 238)*

**Selected as one of Steve Makinen's win-total bets** *(pp. 22–27)*: **UNDER 7.5**.

## 20. Futures / Conference / Playoff Discussion

Prices printed on the team page *(p. 239)*:

| Market | Price |
| --- | --- |
| CFP Championship | 1000-1 |
| make the playoff | 150-1 |
| MAC | +400 |

**VSiN staff predictions naming this team** *(p. 4)*:

| Category | Votes |
| --- | --- |
| MAC CHAMPION | 3 of 22 |

## 21. Betting Notes / Best Bets

**Host best bets naming this team** *(pp. 5–15)*:

| Contributor | Pick | Page |
| --- | --- | --- |
| Dave Tuley | TOLEDO TO WIN MAC (+365) | 6 |

Where contributors disagree, every position is kept separately and none is reconciled.

- Recommendation is Over 7.5 wins, resting first on a soft Year 1 schedule: Toledo misses Miami of Ohio, hosts reigning champion Western Michigan, and gets a visit from San Diego State in non-conference. *(p. 238)*
- Its conclusion is that Jacobs seems like a good hire, with at least some familiarity thanks to transfers from a team that finished in the FCS Top 25 in each of the last two seasons. Projection is 8.04 wins. *(p. 238)*

Also referenced on **pp. 209, 215** — not reproduced here.

## 22. Historical / Situational Trends

- The guide marks the scale of the break: for the first time since 2016 Toledo will not play for Jason Candle, and for the first time since 2009 Candle is not with the programme at all, having taken the UConn job. *(p. 238)*
- It calls this something of a step up for the 47-year-old while making a strong claim on his behalf — he took Mercer to the playoffs in back-to-back seasons and may well be an upgrade on Candle, which it notes is hard to say about a coach who won 65% of his games overall and 68% in conference. *(p. 238)*


Referenced in the guide on **pp. 209, 210, 211** — those passages are not reproduced here; see the pages for VSiN's own wording.

## 23. Important Statistics

As printed on p. 239. Each category carries a value and a national rank.

### Offence

| Category | Value | National rank |
| --- | --- | --- |
| POINTS PER GAME | 28.4 | 46 |
| YARDS PER POINT | 14.2 | 57 |
| PLAYS PER GAME | 66.9 | 89 |
| TIME OF POSSESSION | 30:08 | 60 |
| 3RD DOWN CONV. % | 39.4% | 67 |
| TOTAL YARDS PER GAME | 403.4 | 40 |
| YARDS PER PLAY | 6 | 26 |
| RUSH ATTEMPTS PER GAME | 35.4 | 62 |
| RUSH YARDS PER GAME | 170.3 | 43 |
| YARDS PER RUSH ATTEMPT | 4.8 | 31 |
| PASS ATTEMPTS PER GAME | 30.2 | 82 |
| COMPLETION % | 63.3% | 53 |
| PASSING YARDS PER GAME | 233.2 | 59 |
| YARDS PER PASS ATTEMPT | 7.7 | 41 |
| TURNOVERS | 1.2 | 46 |

### Defence

| Category | Value | National rank |
| --- | --- | --- |
| POINTS PER GAME | 14.4 | 4 |
| YARDS PER POINT | 19 | 5 |
| 3RD DOWN CONV. % | 31.0% | 7 |
| TOTAL YARDS PER GAME | 273.9 | 3 |
| YARDS PER PLAY | 4.1 | 3 |
| RUSH YARDS PER GAME | 104.4 | 14 |
| YARDS PER RUSH ATTEMPT | 2.9 | 6 |
| COMPLETION % | 49.9% | 2 |
| PASSING YARDS PER GAME | 169.5 | 5 |
| YARDS PER PASS ATTEMPT | 6 | 9 |
| SACKS | 2.8 | 14 |
| TURNOVERS | 1.7 | 27 |

## 24. Bull Case

- Makinen projects **8.2 wins** against a posted total of 7.5 — **+0.7** in this team's favour.
- The team page recommends **OVER 7.5**.
- It calls this something of a step up for the 47-year-old while making a strong claim on his behalf — he took Mercer to the playoffs in back-to-back seasons and may well be an upgrade on Candle, which it notes is hard to say about a coach who won 65% of his games overall and 68% in conference. *(p. 238)*
- Its conclusion is that Jacobs seems like a good hire, with at least some familiarity thanks to transfers from a team that finished in the FCS Top 25 in each of the last two seasons. Projection is 8.04 wins. *(p. 238)*

> *Assembled from statements the guide makes about this team. The statements are GUIDE CONTENT with page references; the selection of which ones argue this side is PERSONAL INFERENCE.*

## 25. Bear Case

No statement in this file is phrased as an unambiguously pessimistic claim about this team. VSiN's reasoning is on pp. 238; see the season outlook and *Open Questions / Risks*, which carry the same analysis without splitting it by direction.

> *Assembled from statements the guide makes about this team. The statements are GUIDE CONTENT with page references; the selection of which ones argue this side is PERSONAL INFERENCE.*

## 26. Open Questions / Risks

VSiN poses *Three Burning Questions for the 2026 Season* *(p. 239)*. Its analysis, in reference form:

### Holy Toledo, who are these guys?

The guide's point is the near-total turnover: virtually no recognisable names remain, with the top returning tackler at 35 tackles and the top returnee in yards of any kind at 194 receiving yards. It explains the mechanism rather than treating it as a surprise — Candle left as many of his players were running out of eligibility, and a head coach leaving opens the portal completely. 247Sports lists 37 transfers for Toledo, the third-best MAC class in its ranking. It names John Alan Richter as a returnee Rockets fans may recognise, and notes the incoming group is heavy with Mercer players who followed Jacobs, including running back CJ Miller and receiver Adjatay Dabbs, plus offensive linemen familiar to Reisert who came with him from Gardner-Webb. *(p. 239)*

### What kind of offense should we expect to see?

The guide projects from Mercer's numbers under Jacobs: 6.8 yards per play last season with a balanced approach that ranked third nationally in yards per game, 4.9 yards per carry on more than 400 rushing attempts, and over 3,800 passing yards. It corrects for game state in reading Toledo's own tendencies — the Rockets may have been a little more run-heavy, but that reflects protecting a lot of leads, some of them big. It then flags that this year's team will not have last year's defense, so the game state itself is unknown. Its expectation is a lean on the run at the outset, given Miller, Penn State transfer Cory Smith and two senior tackles who played for Toledo last season. *(p. 239)*

### What kind of defense should we expect to see?

The guide expects aggression under Brown, and again reasons from Mercer: last year's group allowed only 3.4 yards per carry but 5.7 yards per play, because it got burned for explosives in the passing game. The Bears had 42 sacks in 12 games yet generated only 15 takeaways, and their 96 tackles for loss show how heavily they pursued the run and rushed the passer — while leaving an evasive quarterback the chance to burn them. It weighs that risk against the league, noting there are some running quarterbacks in the MAC but fewer this season than in other years, and concludes that with quite a few Mercer transfers who already know the scheme this should be an exciting defense to watch. *(p. 239)*

*TTW reference notes. The questions are VSiN's; the answers summarise VSiN's reasoning in TTW's wording.*

## 27. Source Conflicts

- **Quarterback Database — source conflict / ambiguity.** The guide names no quarterback for Toledo anywhere in the spread, so this record carries no VSiN preseason expectation for the position. *Recorded in Phase 4, field 23 of this team's quarterback record — [source](../04_Quarterback_Database/toledo_rockets.md).*
- **Win Totals — the team page and the feature recommend opposite sides.** The feature (pp. 22–27) bets **Under 7.5**; the team page bets **Over 7.5**. Both are reproduced as printed and neither is corrected. *Recorded in Phase 7, one of 11 teams in the win-total source conflict audit — [source](../06_Win_Totals/00_SOURCE_CONFLICTS.md).*

## 28. Relevant Page References

| Pages | Content |
| --- | --- |
| 238–239 | This team's two-page preview |
| 215 | MAC conference preview and projected standings |
| 4 | VSiN staff season predictions |
| 46–47 | Steve Makinen power rating methodology |
| 21 | Home-field advantage article |
| 6, 24, 27, 32, 33, 34, 107, 209, 210, 211, 216, 220, 221, 228, 231, 233, 234, 236, 274 | Other pages naming this team |

This team is named in **41 sentences across 22 pages** of the guide.

Every page naming this team outside its own spread: pp. 6, 24, 27, 32, 33, 34, 107, 209, 210, 211, 216, 220, 221, 228, 231, 233, 234, 236, 274.

## 29. Cross-Links

**Conference** — [MAC](../01_Conference_Database/mac.md)

**Master Index** — [Team Index](../00_Master_Index/03_Team_Index.md) · [Power Rating Index](../00_Master_Index/09_Power_Rating_Index.md) · [Coaching Index](../00_Master_Index/04_Coaching_Index.md) · [Quarterback Index](../00_Master_Index/06_Quarterback_Index.md)

**Every other database, for this team:**

- Head coach — [03_Coaching_Database/toledo_rockets.md](../03_Coaching_Database/toledo_rockets.md)
- Quarterback — [04_Quarterback_Database/toledo_rockets.md](../04_Quarterback_Database/toledo_rockets.md)
- Power rating — [05_Power_Ratings/00_MAKINEN_RATINGS.md](../05_Power_Ratings/00_MAKINEN_RATINGS.md)
- Win total — [06_Win_Totals/00_ALL_TEAMS.md](../06_Win_Totals/00_ALL_TEAMS.md)
- Futures board — [07_Futures/00_TEAM_FUTURES.md](../07_Futures/00_TEAM_FUTURES.md)
- Returning production — [08_Returning_Production/README.md](../08_Returning_Production/README.md)
- Schedule — [10_Schedule_Intelligence/00_BY_TEAM.md](../10_Schedule_Intelligence/00_BY_TEAM.md)
- Historical angles — [12_Historical_Trends/00_BY_TEAM.md](../12_Historical_Trends/00_BY_TEAM.md)
- Everything at once — [99_Search_Index/02_TEAM_LOOKUP.md](../99_Search_Index/02_TEAM_LOOKUP.md)

