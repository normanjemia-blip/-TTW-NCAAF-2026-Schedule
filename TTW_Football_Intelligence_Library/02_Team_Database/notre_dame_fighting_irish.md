<!-- GENERATED FILE — do not hand-edit.
     Rebuild:  python3 _tools/build_teams.py
     Source:   2026 VSiN College Football Betting Guide (345 pp.) -->

# Notre Dame Fighting Irish

> **Source: 2026 VSiN College Football Betting Guide, pp. 212–213** and every other page that names this team. GUIDE CONTENT throughout unless a line says otherwise. No outside research. Nothing inferred to fill a heading.


## 1. Program Snapshot

| | |
| --- | --- |
| **Conference** | Independents |
| **Guide pages** | 212–213 |
| **Head coach** | Marcus Freeman (5th season) |
| **2025 straight up** | 10-2 |
| **2025 against the spread** | 6-5-1 |
| **2025 over/under** | 7-5 |
| **Makinen power rating** | 68.5 |
| **National rank** | #3 of 138 |
| **Conference rank** | #1 of 2 |
| **DraftKings win total** | 11.5 |
| **Makinen projected wins** | 11.2–0.8 overall, 0.0–0.0 in conference |
| **Schedule strength** | 43.31 (#59 toughest of 138) |

### Season outlook — VSiN's analysis in reference form (p. 212)

- The guide asks whether a 10- or 11-win regular season could be a disappointment and concludes that for Notre Dame it just might be. *(p. 212)*
- On the schedule it stops short of calling it a cupcake, settling on a bundt cake, with the 7 November home revenge game against Miami far and away the biggest test. *(p. 212)*
- Marcus Freeman is 34-7 over three seasons with what the guide calls a spectacular roster. *(p. 212)*
- It adds an important qualifier about independence: Notre Dame picks its own schedule but has still been tested, facing 12 ranked teams in regular seasons over that span — whereas this year it may face only two or three. *(p. 212)*
- Recommendation is Over 11.5 wins. The guide names the risk honestly: the pressure of going perfect may prove too much. *(p. 212)*
- Miami is treated as the only major stumbling block, and one that has had to retool heavily on defense. It notes fairly that Miami was underestimated last season, while recalling Notre Dame was favored on the road in that opener. *(p. 212)*
- A culture signal is offered as evidence: the Irish were so aggrieved by the playoff snub that they skipped a bowl, and the only notable departures went to the NFL rather than to other schools. Projection is 11.42 wins, with Notre Dame favored by 8.5 or more in every game. *(p. 212)*

*TTW reference notes summarising VSiN's analysis. GUIDE CONTENT — facts, conclusions and reasoning are the guide's; the wording is TTW's.*


## 2. Conference

**Independents** — see the conference file for league-wide context: [independents.md](../01_Conference_Database/independents.md)

Makinen projects **0.0–0.0** in conference play, ranking this team #1 of 2 in the Independents by power rating.

## 3. VSiN Team Rank / Conference Rank

- **National rank:** #3 of 138 *(p. 213)*
- **Conference rank:** #1 of 2 *(p. 213)*
- **Matt Youmans' Preseason Top 50:** **#1** *(pp. 16–20)*

## 4. Steve Makinen Power Rating

**68.5** — as printed on p. 213, reproduced exactly and not converted to any other scale.

Ranks #3 of 138 nationally and #1 of 2 in the Independents. Methodology is set out on pp. 46–47.

> No comparison against the TTW workbook is made here. That is a later-phase deliverable.

## 5. Home-Field Advantage Reference

- **Home field rating:** 3.8 *(p. 212)*
- **Road field rating:** 1 *(p. 212)*

Printed on the team page as `field ratings (HOME/ROAD)` and verified against the same values in the conference projected standings table. The guide's general treatment of home-field advantage is on p. 21.

## 6. Head Coach

**Marcus Freeman** — 5th season at the programme *(p. 212)*.

## 7. Coordinator Notes

- **DC Chris Ash** *(p. 213)*

## 8. Coaching Continuity / Changes

**Steve Makinen's Stability Score: 18** *(p. 43)*

| Component | Status | Points |
| --- | --- | --- |
| Head coach | returns | 4 |
| Offensive coordinator | returns | 3 |
| Defensive coordinator | returns | 3 |
| Starting quarterback | returns | 4 |
| Returning starters | 17 | 4 |

Scoring is the guide's own: head coach 4, offensive coordinator 3, defensive coordinator 3, starting quarterback 4, plus points for returning starters. A returning transfer quarterback counts as a **new** quarterback in this system *(p. 41)*.

- Marcus Freeman is 34-7 over three seasons with what the guide calls a spectacular roster. *(p. 212)*

Also referenced on **pp. 17, 34, 150, 238, 344** — not reproduced here.

## 9. Quarterback Situation

**Paul Stone ranks Cj Carr the #3 quarterback in the country** *(p. 45)*.

**Starting quarterback continuity:** the Stability Score table treats this team as having the same starting quarterback as 2025 *(p. 43)*. A transfer expected to start counts as new in that system.

**Returning starting quarterback:** yes — the guide marks returning quarterbacks with an asterisk on the returning-starters line *(p. 212)*.

- **Are there any weaknesses on offense?** The guide's answer is no, but it works through the case first. CJ Carr had a 24-to-6 touchdown-to-interception ratio with three rushing scores, though he is not the running threat Riley Leonard was, and the depth behind him is a major worry if anything happens. He spent spring and summer as Heisman favorite, which the guide attributes substantially to the schedule, reasoning that anything under 11 wins would surprise so he should post numbers for a very visible programme. Jeremiyah Love and Jadarian Price take over 2,000 rushing yards and 29 scores with them, but Aneyas Williams should keep the standard; Jordan Faison is the only notable returning receiver though Jaden Greathouse returns from injury; and the line looks very good with Charles Jagusah back. *(p. 213)*

Also referenced on **pp. 9, 17, 18, 20, 107, 220, 267** — not reproduced here.

## 10. Returning Production

| | Returning starters |
| --- | --- |
| **Total** | 17 |
| **Offence** | 7 *(includes the returning quarterback)* |
| **Defence** | 10 |

Read by position from p. 212. The three figures sit under the printed labels `total / offense / defense`; they are matched to those labels by their place on the page, because the PDF text layer emits them in a different order than they are printed.

The Stability Score table prints the same figure independently: **17 returning starters** *(p. 43)*.

## 11. Transfer Portal

- A culture signal is offered as evidence: the Irish were so aggrieved by the playoff snub that they skipped a bowl, and the only notable departures went to the NFL rather than to other schools. Projection is 11.42 wins, with Notre Dame favored by 8.5 or more in every game. *(p. 212)*


Referenced in the guide on **pp. 9, 106, 220** — those passages are not reproduced here; see the pages for VSiN's own wording.

## 12. Recruiting / Roster Notes

Not addressed in guide.

## 13. Offensive Identity

- **Are there any weaknesses on offense?** The guide's answer is no, but it works through the case first. CJ Carr had a 24-to-6 touchdown-to-interception ratio with three rushing scores, though he is not the running threat Riley Leonard was, and the depth behind him is a major worry if anything happens. He spent spring and summer as Heisman favorite, which the guide attributes substantially to the schedule, reasoning that anything under 11 wins would surprise so he should post numbers for a very visible programme. Jeremiyah Love and Jadarian Price take over 2,000 rushing yards and 29 scores with them, but Aneyas Williams should keep the standard; Jordan Faison is the only notable returning receiver though Jaden Greathouse returns from injury; and the line looks very good with Charles Jagusah back. *(p. 213)*

## 14. Defensive Identity

- **Are there any weaknesses on defense?** Again no. Unlike most premier programmes with heavy losses, second-year coordinator Chris Ash returns his top six tacklers. Notre Dame was one of 11 teams holding opponents under three yards per carry and only eight allowed fewer yards per play. Across seven years of coaching and personnel churn it has allowed more than 19.7 points per game only once, and under five yards per play in each of the last three seasons under Freeman. The guide flags one genuine caution — the Irish are +31 in turnover margin over two seasons, an area of concern unless Carr protects the ball — while expecting the ball-hawking to continue after 21 interceptions. *(p. 213)*


Referenced in the guide on **pp. 17, 150** — those passages are not reproduced here; see the pages for VSiN's own wording.

## 15. Key Strengths

National top-25 finishes in the guide's printed categories *(p. 213)*:

| Side | Category | Value | National rank |
| --- | --- | --- | --- |
| Offense | YARDS PER POINT | 10.9 | #1 |
| Offense | YARDS PER PLAY | 7.3 | #1 |
| Offense | POINTS PER GAME | 42 | #2 |
| Offense | YARDS PER RUSH ATTEMPT | 5.8 | #2 |
| Offense | YARDS PER PASS ATTEMPT | 9.5 | #3 |
| Defense | YARDS PER RUSH ATTEMPT | 3 | #7 |
| Offense | TOTAL YARDS PER GAME | 459.8 | #8 |
| Defense | POINTS PER GAME | 17.6 | #9 |
| Defense | YARDS PER POINT | 17.8 | #9 |
| Defense | YARDS PER PLAY | 4.4 | #9 |
| Defense | RUSH YARDS PER GAME | 98.8 | #9 |
| Defense | TURNOVERS | 1.9 | #11 |
| Offense | 3RD DOWN CONV. % | 48.2% | #12 |
| Offense | TURNOVERS | 0.8 | #12 |
| Defense | COMPLETION % | 55.7% | #14 |
| Defense | YARDS PER PASS ATTEMPT | 6.2 | #14 |
| Defense | SACKS | 2.8 | #14 |
| Offense | RUSH YARDS PER GAME | 204.7 | #15 |
| Defense | TOTAL YARDS PER GAME | 312.3 | #16 |
| Offense | COMPLETION % | 67.2% | #17 |

## 16. Key Weaknesses

National bottom-25 finishes in the guide's printed categories *(p. 213)*:

| Side | Category | Value | National rank |
| --- | --- | --- | --- |
| Offense | PLAYS PER GAME | 63.3 | #124 |

## 17. Schedule Overview

Schedule strength **43.31**, **#59 toughest of 138** *(p. 212)*.

| Date | Opponent | Site | Projected line | Opponent power rating |
| --- | --- | --- | --- | --- |
| 9/6 | vs. Wisconsin | Neutral | -21 | 47.5 |
| 9/12 | RICE | Home | -45 | 27.5 |
| 9/19 | MICHIGAN ST | Home | -29.9 | 42.5 |
| 9/26 | at Purdue | Away | -29 | 39.0 |
| 10/3 | at North Carolina | Away | -23.6 | 44.5 |
| 10/10 | STANFORD | Home | -33.9 | 39.0 |
| 10/17 | at BYU | Away | -8.8 | 57.5 |
| 10/31 | vs. Navy | Neutral | -24.7 | 44.0 |
| 11/7 | MIAMI FL | Home | -9.2 | 63.0 |
| 11/14 | BOSTON COLLEGE | Home | -35.4 | 37.5 |
| 11/21 | SMU | Home | -16.7 | 55.5 |
| 11/28 | at Syracuse | Away | -25.1 | 42.0 |

Projected lines and opponent power ratings are Makinen's, printed alongside the schedule on the team page.

## 18. Difficult Stretches / Trap Spots

Not addressed in guide under this heading.

The guide names this team on **pp. 7, 10, 18, 209**, but no passage there was extracted under this heading; see those pages for VSiN's own wording.

## 19. Win Total Discussion

- **DraftKings win total:** 11.5 *(p. 209)*
- **Makinen projected wins:** 11.2 *(p. 209)*
- **Difference:** -0.3 wins against the posted number
- **Recommendation on the team page:** **OVER 11.5** *(p. 212)*
- **Projection stated in the team-page essay:** 11.42 wins *(p. 212)*

## 20. Futures / Conference / Playoff Discussion

Prices printed on the team page *(p. 213)*:

| Market | Price |
| --- | --- |
| CFP Championship | +650 |
| make the playoff | -800 |

**VSiN staff predictions naming this team** *(p. 4)*:

| Category | Votes |
| --- | --- |
| CFP FINAL FOUR #1 | 17 of 22 |
| CFP FINAL FOUR #3 | 1 of 22 |
| CFP FINAL FOUR #4 | 2 of 22 |
| CFP TITLE GAME #1 | 15 of 22 |
| CFP CHAMPION | 7 of 22 |

## 21. Betting Notes / Best Bets

- Recommendation is Over 11.5 wins. The guide names the risk honestly: the pressure of going perfect may prove too much. *(p. 212)*
- Miami is treated as the only major stumbling block, and one that has had to retool heavily on defense. It notes fairly that Miami was underestimated last season, while recalling Notre Dame was favored on the road in that opener. *(p. 212)*
- A culture signal is offered as evidence: the Irish were so aggrieved by the playoff snub that they skipped a bowl, and the only notable departures went to the NFL rather than to other schools. Projection is 11.42 wins, with Notre Dame favored by 8.5 or more in every game. *(p. 212)*
- **Anything else?** The guide adds supporting detail on how dominant the season actually was: the two close losses were followed by blowouts, with Notre Dame outgaining its other ten opponents by more than 1,800 yards and outscoring them by nearly 300 points. Provided cluster injuries are avoided, it calls a perfect season very attainable including the playoff. It closes with schedule mechanics: only four true road games at Purdue, North Carolina, BYU and Syracuse, two neutral-site games in Green Bay and Foxboro, and a well-timed bye giving an extra week to prepare for Navy's triple option after playing at elevation at BYU. *(p. 213)*

Also referenced on **pp. 10, 17, 18, 38, 94, 106** — not reproduced here.

## 22. Historical / Situational Trends

- The guide asks whether a 10- or 11-win regular season could be a disappointment and concludes that for Notre Dame it just might be. *(p. 212)*
- It adds an important qualifier about independence: Notre Dame picks its own schedule but has still been tested, facing 12 ranked teams in regular seasons over that span — whereas this year it may face only two or three. *(p. 212)*
- **Are there any weaknesses on defense?** Again no. Unlike most premier programmes with heavy losses, second-year coordinator Chris Ash returns his top six tacklers. Notre Dame was one of 11 teams holding opponents under three yards per carry and only eight allowed fewer yards per play. Across seven years of coaching and personnel churn it has allowed more than 19.7 points per game only once, and under five yards per play in each of the last three seasons under Freeman. The guide flags one genuine caution — the Irish are +31 in turnover margin over two seasons, an area of concern unless Carr protects the ball — while expecting the ball-hawking to continue after 21 interceptions. *(p. 213)*


Referenced in the guide on **pp. 12, 17, 38, 92, 100, 209** — those passages are not reproduced here; see the pages for VSiN's own wording.

## 23. Important Statistics

As printed on p. 213. Each category carries a value and a national rank.

### Offence

| Category | Value | National rank |
| --- | --- | --- |
| POINTS PER GAME | 42 | 2 |
| YARDS PER POINT | 10.9 | 1 |
| PLAYS PER GAME | 63.3 | 124 |
| TIME OF POSSESSION | 29:17 | 89 |
| 3RD DOWN CONV. % | 48.2% | 12 |
| TOTAL YARDS PER GAME | 459.8 | 8 |
| YARDS PER PLAY | 7.3 | 1 |
| RUSH ATTEMPTS PER GAME | 35.3 | 63 |
| RUSH YARDS PER GAME | 204.7 | 15 |
| YARDS PER RUSH ATTEMPT | 5.8 | 2 |
| PASS ATTEMPTS PER GAME | 26.9 | 109 |
| COMPLETION % | 67.2% | 17 |
| PASSING YARDS PER GAME | 255.1 | 32 |
| YARDS PER PASS ATTEMPT | 9.5 | 3 |
| TURNOVERS | 0.8 | 12 |

### Defence

| Category | Value | National rank |
| --- | --- | --- |
| POINTS PER GAME | 17.6 | 9 |
| YARDS PER POINT | 17.8 | 9 |
| 3RD DOWN CONV. % | 35.4% | 35 |
| TOTAL YARDS PER GAME | 312.3 | 16 |
| YARDS PER PLAY | 4.4 | 9 |
| RUSH YARDS PER GAME | 98.8 | 9 |
| YARDS PER RUSH ATTEMPT | 3 | 7 |
| COMPLETION % | 55.7% | 14 |
| PASSING YARDS PER GAME | 213.4 | 50 |
| YARDS PER PASS ATTEMPT | 6.2 | 14 |
| SACKS | 2.8 | 14 |
| TURNOVERS | 1.9 | 11 |

## 24. Bull Case

- The team page recommends **OVER 11.5**.

*1 further statement about this team carries both positive and negative language and is not classified to either side. It appears in full under the season outlook and *Open Questions / Risks*.*

> *Assembled from statements the guide makes about this team. The statements are GUIDE CONTENT with page references; the selection of which ones argue this side is PERSONAL INFERENCE.*

## 25. Bear Case

- Makinen projects **11.2 wins** against a posted total of 11.5 — **-0.3** against this team.

*1 further statement about this team carries both positive and negative language and is not classified to either side. It appears in full under the season outlook and *Open Questions / Risks*.*

> *Assembled from statements the guide makes about this team. The statements are GUIDE CONTENT with page references; the selection of which ones argue this side is PERSONAL INFERENCE.*

## 26. Open Questions / Risks

VSiN poses *Three Burning Questions for the 2026 Season* *(p. 213)*. Its analysis, in reference form:

### Are there any weaknesses on offense?

The guide's answer is no, but it works through the case first. CJ Carr had a 24-to-6 touchdown-to-interception ratio with three rushing scores, though he is not the running threat Riley Leonard was, and the depth behind him is a major worry if anything happens. He spent spring and summer as Heisman favorite, which the guide attributes substantially to the schedule, reasoning that anything under 11 wins would surprise so he should post numbers for a very visible programme. Jeremiyah Love and Jadarian Price take over 2,000 rushing yards and 29 scores with them, but Aneyas Williams should keep the standard; Jordan Faison is the only notable returning receiver though Jaden Greathouse returns from injury; and the line looks very good with Charles Jagusah back. *(p. 213)*

### Are there any weaknesses on defense?

Again no. Unlike most premier programmes with heavy losses, second-year coordinator Chris Ash returns his top six tacklers. Notre Dame was one of 11 teams holding opponents under three yards per carry and only eight allowed fewer yards per play. Across seven years of coaching and personnel churn it has allowed more than 19.7 points per game only once, and under five yards per play in each of the last three seasons under Freeman. The guide flags one genuine caution — the Irish are +31 in turnover margin over two seasons, an area of concern unless Carr protects the ball — while expecting the ball-hawking to continue after 21 interceptions. *(p. 213)*

### Anything else?

The guide adds supporting detail on how dominant the season actually was: the two close losses were followed by blowouts, with Notre Dame outgaining its other ten opponents by more than 1,800 yards and outscoring them by nearly 300 points. Provided cluster injuries are avoided, it calls a perfect season very attainable including the playoff. It closes with schedule mechanics: only four true road games at Purdue, North Carolina, BYU and Syracuse, two neutral-site games in Green Bay and Foxboro, and a well-timed bye giving an extra week to prepare for Navy's triple option after playing at elevation at BYU. *(p. 213)*

*TTW reference notes. The questions are VSiN's; the answers summarise VSiN's reasoning in TTW's wording.*

## 27. Source Conflicts

No source conflict identified for this team.

## 28. Relevant Page References

| Pages | Content |
| --- | --- |
| 212–213 | This team's two-page preview |
| 209 | Independents conference preview and projected standings |
| 4 | VSiN staff season predictions |
| 46–47 | Steve Makinen power rating methodology |
| 21 | Home-field advantage article |
| 7, 8, 9, 10, 12, 17, 18, 20, 22, 34, 38, 41, 45, 46, 60, 64, 80, 92, 94, 98, 100, 104, 106, 107, 122, 150, 156, 161, 184, 220, 238, 254, 267, 295, 344 | Other pages naming this team |

This team is named in **75 sentences across 38 pages** of the guide.

Every page naming this team outside its own spread: pp. 7, 8, 9, 10, 12, 17, 18, 20, 22, 34, 38, 41, 45, 46, 60, 64, 80, 92, 94, 98, 100, 104, 106, 107, 122, 150, 156, 161, 184, 220, 238, 254, 267, 295, 344.

## 29. Cross-Links

**Conference** — [Independents](../01_Conference_Database/independents.md)

**Master Index** — [Team Index](../00_Master_Index/03_Team_Index.md) · [Power Rating Index](../00_Master_Index/09_Power_Rating_Index.md) · [Coaching Index](../00_Master_Index/04_Coaching_Index.md) · [Quarterback Index](../00_Master_Index/06_Quarterback_Index.md)

**Every other database, for this team:**

- Head coach — [03_Coaching_Database/notre_dame_fighting_irish.md](../03_Coaching_Database/notre_dame_fighting_irish.md)
- Quarterback — [04_Quarterback_Database/notre_dame_fighting_irish.md](../04_Quarterback_Database/notre_dame_fighting_irish.md)
- Power rating — [05_Power_Ratings/00_MAKINEN_RATINGS.md](../05_Power_Ratings/00_MAKINEN_RATINGS.md)
- Win total — [06_Win_Totals/00_ALL_TEAMS.md](../06_Win_Totals/00_ALL_TEAMS.md)
- Futures board — [07_Futures/00_TEAM_FUTURES.md](../07_Futures/00_TEAM_FUTURES.md)
- Returning production — [08_Returning_Production/README.md](../08_Returning_Production/README.md)
- Schedule — [10_Schedule_Intelligence/00_BY_TEAM.md](../10_Schedule_Intelligence/00_BY_TEAM.md)
- Historical angles — [12_Historical_Trends/00_BY_TEAM.md](../12_Historical_Trends/00_BY_TEAM.md)
- Everything at once — [99_Search_Index/02_TEAM_LOOKUP.md](../99_Search_Index/02_TEAM_LOOKUP.md)

