<!-- GENERATED FILE — do not hand-edit.
     Rebuild:  python3 _tools/build_teams.py
     Source:   2026 VSiN College Football Betting Guide (345 pp.) -->

# North Dakota State Bison

> **Source: 2026 VSiN College Football Betting Guide, pp. 252–253** and every other page that names this team. GUIDE CONTENT throughout unless a line says otherwise. No outside research. Nothing inferred to fill a heading.


## 1. Program Snapshot

| | |
| --- | --- |
| **Conference** | Mountain West |
| **Guide pages** | 252–253 |
| **Head coach** | Tim Polasek (3rd season) |
| **2025 straight up** | 12-1 |
| **2025 against the spread** | 6-7 |
| **2025 over/under** | 7-6 |
| **Makinen power rating** | 37.0 |
| **National rank** | #87 of 138 |
| **Conference rank** | #4 of 10 |
| **DraftKings win total** | 8.5 |
| **Makinen projected wins** | 8.0–4.0 overall, 4.8–3.2 in conference |
| **Schedule strength** | 28.33 (#130 toughest of 138) |

### Season outlook — VSiN's analysis in reference form (p. 252)

- The guide describes North Dakota State as the big fish landed by commissioner Gloria Nevarez after what it calls the Defecting Five left for the Pac-12. *(p. 252)*
- It credits a great programme with a long history of dominating the FCS and challenging strong FBS teams, noting NDSU retains favorite status even while stepping up. *(p. 252)*
- The distinguishing factor is roster construction: the Bison arrive with a largely homegrown roster and few transfers, and Tim Polasek lost considerable talent to the portal. *(p. 252)*
- It frames the season as a true test of the programme's formula. *(p. 252)*
- Recommendation is Under 8.5 wins. The guide understands why NDSU is a short price to win the conference given its record, while judging expectations somewhat high. *(p. 252)*
- Its reasoning is specific: portal losses were immense with little brought back, and while the Mountain West is no power conference, these are teams built with former Power Four players carrying real talent — so there should be doubts about walking in and dominating. *(p. 252)*
- Schedule detail: road games at Air Force, UNLV, Hawaii and New Mexico — trips to every contender. The home schedule is far from daunting with Fordham and Sacramento State, though Wyoming, Nevada and UTEP could all improve. It answers its own question about winning most of those road games with a blunt doubtful. *(p. 252)*

*TTW reference notes summarising VSiN's analysis. GUIDE CONTENT — facts, conclusions and reasoning are the guide's; the wording is TTW's.*


## 2. Conference

**Mountain West** — see the conference file for league-wide context: [mountain_west.md](../01_Conference_Database/mountain_west.md)

Makinen projects **4.8–3.2** in conference play, ranking this team #4 of 10 in the Mountain West by power rating.

## 3. VSiN Team Rank / Conference Rank

- **National rank:** #87 of 138 *(p. 253)*
- **Conference rank:** #4 of 10 *(p. 253)*
- **Matt Youmans' Preseason Top 50:** not ranked *(pp. 16–20)*

## 4. Steve Makinen Power Rating

**37.0** — as printed on p. 253, reproduced exactly and not converted to any other scale.

Ranks #87 of 138 nationally and #4 of 10 in the Mountain West. Methodology is set out on pp. 46–47.

> No comparison against the TTW workbook is made here. That is a later-phase deliverable.

## 5. Home-Field Advantage Reference

- **Home field rating:** 3.5 *(p. 252)*
- **Road field rating:** 0.5 *(p. 252)*

Printed on the team page as `field ratings (HOME/ROAD)` and verified against the same values in the conference projected standings table. The guide's general treatment of home-field advantage is on p. 21.

## 6. Head Coach

**Tim Polasek** — 3rd season at the programme *(p. 252)*.

## 7. Coordinator Notes

Not addressed in guide.

## 8. Coaching Continuity / Changes

**Steve Makinen's Stability Score: 12** *(p. 43)*

| Component | Status | Points |
| --- | --- | --- |
| Head coach | returns | 4 |
| Offensive coordinator | returns | 3 |
| Defensive coordinator | returns | 3 |
| Starting quarterback | new for 2026 | 0 |
| Returning starters | 10 | 2 |

Scoring is the guide's own: head coach 4, offensive coordinator 3, defensive coordinator 3, starting quarterback 4, plus points for returning starters. A returning transfer quarterback counts as a **new** quarterback in this system *(p. 41)*.

- It frames the season as a true test of the programme's formula. *(p. 252)*

Also referenced on **pp. 5, 270** — not reproduced here.

## 9. Quarterback Situation

**Starting quarterback continuity:** the Stability Score table treats this team as having a new starting quarterback for 2026 *(p. 43)*. A transfer expected to start counts as new in that system.

**Returning starting quarterback:** no — the guide marks returning quarterbacks with an asterisk on the returning-starters line *(p. 252)*.

- **Does the run of quarterback development continue?** Cole Payton going to the Eagles at pick 178 marked the fifth Bison starting quarterback drafted in 11 years, and Nathan Hayes now carries that pressure. Hayes appeared in 10 games, completing 57% for 381 yards with four touchdowns and one interception, and has been in the system since 2022 so there should be no learning curve. The guide's counterweight is the supporting cast: he takes over an offense missing its top two rushers and top two receivers. *(p. 253)*

## 10. Returning Production

| | Returning starters |
| --- | --- |
| **Total** | 10 |
| **Offence** | 5 |
| **Defence** | 5 |

Read by position from p. 252. The three figures sit under the printed labels `total / offense / defense`; they are matched to those labels by their place on the page, because the PDF text layer emits them in a different order than they are printed.

The Stability Score table prints the same figure independently: **10 returning starters** *(p. 43)*.

## 11. Transfer Portal

- The distinguishing factor is roster construction: the Bison arrive with a largely homegrown roster and few transfers, and Tim Polasek lost considerable talent to the portal. *(p. 252)*
- Its reasoning is specific: portal losses were immense with little brought back, and while the Mountain West is no power conference, these are teams built with former Power Four players carrying real talent — so there should be doubts about walking in and dominating. *(p. 252)*
- **Will the lack of talent from the transfer portal burn the Bison?** North Dakota State took a different approach from most, signing 29 high school players and only nine transfers, of whom three played at Power Four programmes last season — one being punter Caleb McGrath. It lost significant talent including two First-Team All-MVFC offensive linemen. The guide explains why the model worked before and why it may not now: recruiting and developing high school players succeeded at the FCS level, but Group of Five teams constantly retool with castoffs and former top prospects seeking a new home. With just 10 starters back and zero portal starters added, it leaves the question genuinely open. *(p. 253)*


Referenced in the guide on **pp. 161** — those passages are not reproduced here; see the pages for VSiN's own wording.

## 12. Recruiting / Roster Notes

- **Will the lack of talent from the transfer portal burn the Bison?** North Dakota State took a different approach from most, signing 29 high school players and only nine transfers, of whom three played at Power Four programmes last season — one being punter Caleb McGrath. It lost significant talent including two First-Team All-MVFC offensive linemen. The guide explains why the model worked before and why it may not now: recruiting and developing high school players succeeded at the FCS level, but Group of Five teams constantly retool with castoffs and former top prospects seeking a new home. With just 10 starters back and zero portal starters added, it leaves the question genuinely open. *(p. 253)*

## 13. Offensive Identity

- **Does the run of quarterback development continue?** Cole Payton going to the Eagles at pick 178 marked the fifth Bison starting quarterback drafted in 11 years, and Nathan Hayes now carries that pressure. Hayes appeared in 10 games, completing 57% for 381 yards with four touchdowns and one interception, and has been in the system since 2022 so there should be no learning curve. The guide's counterweight is the supporting cast: he takes over an offense missing its top two rushers and top two receivers. *(p. 253)*


Referenced in the guide on **pp. 161** — those passages are not reproduced here; see the pages for VSiN's own wording.

## 14. Defensive Identity

Not addressed in guide.

## 15. Key Strengths

Not addressed in guide.

## 16. Key Weaknesses

Not addressed in guide.

## 17. Schedule Overview

Schedule strength **28.33**, **#130 toughest of 138** *(p. 252)*.

| Date | Opponent | Site | Projected line | Opponent power rating |
| --- | --- | --- | --- | --- |
| 8/29 | JACKSONVILLE ST | Home | -5.8 | 34.0 |
| 9/5 | FORDHAM | Home | -39.2 | 2.0 |
| 9/12 | at Air Force | Away | +2 | 37.0 |
| 9/19 | at Sacramento St | Away | -14.2 | 21.5 |
| 10/3 | WYOMING | Home | -10.6 | 30.5 |
| 10/10 | at UNLV | Away | +9.7 | 44.0 |
| 10/17 | NEVADA | Home | -12.5 | 28.0 |
| 10/24 | at New Mexico | Away | +6.8 | 42.0 |
| 10/31 | UTEP | Home | -18.2 | 22.5 |
| 11/14 | at Hawaii | Away | +4.7 | 38.5 |
| 11/21 | N ILLINOIS | Home | -18.5 | 21.5 |
| 11/27 | at San Jose St | Away | -8.2 | 27.0 |

Projected lines and opponent power ratings are Makinen's, printed alongside the schedule on the team page.

## 18. Difficult Stretches / Trap Spots

Not addressed in guide.

## 19. Win Total Discussion

- **DraftKings win total:** 8.5 *(p. 243)*
- **Makinen projected wins:** 8.0 *(p. 243)*
- **Difference:** -0.5 wins against the posted number
- **Recommendation on the team page:** **UNDER 8.5** *(p. 252)*

## 20. Futures / Conference / Playoff Discussion

Prices printed on the team page *(p. 253)*:

| Market | Price |
| --- | --- |
| CFP Championship | 600-1 |
| make the playoff | 1000-1 |
| MOUNTAIN WEST | +300 |

**VSiN staff predictions naming this team** *(p. 4)*:

| Category | Votes |
| --- | --- |
| MWC CCHAMPION | 3 of 22 |

## 21. Betting Notes / Best Bets

**Host best bets naming this team** *(pp. 5–15)*:

| Contributor | Pick | Page |
| --- | --- | --- |
| Pauly Howard | NORTH DAKOTA STATE UNDER 9.5 WINS (-200) | 7 |
| Stormy Buonantony | NORTH DAKOTA STATE ALT UNDER 8.5 WINS (+145) | 14 |
| Jonathan Von Tobel | NORTH DAKOTA STATE UNDER 8.5 WINS (+140) | 15 |

Where contributors disagree, every position is kept separately and none is reconciled.

- Recommendation is Under 8.5 wins. The guide understands why NDSU is a short price to win the conference given its record, while judging expectations somewhat high. *(p. 252)*
- Its reasoning is specific: portal losses were immense with little brought back, and while the Mountain West is no power conference, these are teams built with former Power Four players carrying real talent — so there should be doubts about walking in and dominating. *(p. 252)*
- Schedule detail: road games at Air Force, UNLV, Hawaii and New Mexico — trips to every contender. The home schedule is far from daunting with Fordham and Sacramento State, though Wyoming, Nevada and UTEP could all improve. It answers its own question about winning most of those road games with a blunt doubtful. *(p. 252)*
- **How does North Dakota State handle being a favorite at a higher level?** The guide notes the role is nothing new — NDSU was favored in every game last season and closed as a 30-plus point favorite in six of 13 — but stresses this is a higher level. It observes the conference did the Bison no favors in scheduling their inaugural season, with road trips to Air Force, UNLV, New Mexico and Hawaii that it calls coin-flip spots. Its summary is that the days of bullying a lesser conference are gone: NDSU is certainly capable of winning the league, but the road is much tougher. *(p. 253)*

Also referenced on **pp. 9, 15, 247** — not reproduced here.

## 22. Historical / Situational Trends

- The guide describes North Dakota State as the big fish landed by commissioner Gloria Nevarez after what it calls the Defecting Five left for the Pac-12. *(p. 252)*
- It credits a great programme with a long history of dominating the FCS and challenging strong FBS teams, noting NDSU retains favorite status even while stepping up. *(p. 252)*
- **Does the run of quarterback development continue?** Cole Payton going to the Eagles at pick 178 marked the fifth Bison starting quarterback drafted in 11 years, and Nathan Hayes now carries that pressure. Hayes appeared in 10 games, completing 57% for 381 yards with four touchdowns and one interception, and has been in the system since 2022 so there should be no learning curve. The guide's counterweight is the supporting cast: he takes over an offense missing its top two rushers and top two receivers. *(p. 253)*


Referenced in the guide on **pp. 9, 14, 155, 161** — those passages are not reproduced here; see the pages for VSiN's own wording.

## 23. Important Statistics

The guide prints no 2025 statistics for this team, stating instead: **"PARTICIPATED PARTICIPATED IN FCS IN 2025 IN FCS IN 2025"** *(p. 253)*.

This is the guide's own note, not a gap in extraction.

## 24. Bull Case

- It credits a great programme with a long history of dominating the FCS and challenging strong FBS teams, noting NDSU retains favorite status even while stepping up. *(p. 252)*

*1 further statement about this team carries both positive and negative language and is not classified to either side. It appears in full under the season outlook and *Open Questions / Risks*.*

> *Assembled from statements the guide makes about this team. The statements are GUIDE CONTENT with page references; the selection of which ones argue this side is PERSONAL INFERENCE.*

## 25. Bear Case

- Makinen projects **8.0 wins** against a posted total of 8.5 — **-0.5** against this team.
- The team page recommends **UNDER 8.5**.
- The distinguishing factor is roster construction: the Bison arrive with a largely homegrown roster and few transfers, and Tim Polasek lost considerable talent to the portal. *(p. 252)*
- Its reasoning is specific: portal losses were immense with little brought back, and while the Mountain West is no power conference, these are teams built with former Power Four players carrying real talent — so there should be doubts about walking in and dominating. *(p. 252)*

*1 further statement about this team carries both positive and negative language and is not classified to either side. It appears in full under the season outlook and *Open Questions / Risks*.*

> *Assembled from statements the guide makes about this team. The statements are GUIDE CONTENT with page references; the selection of which ones argue this side is PERSONAL INFERENCE.*

## 26. Open Questions / Risks

VSiN poses *Three Burning Questions for the 2026 Season* *(p. 253)*. Its analysis, in reference form:

### How does North Dakota State handle being a favorite at a higher level?

The guide notes the role is nothing new — NDSU was favored in every game last season and closed as a 30-plus point favorite in six of 13 — but stresses this is a higher level. It observes the conference did the Bison no favors in scheduling their inaugural season, with road trips to Air Force, UNLV, New Mexico and Hawaii that it calls coin-flip spots. Its summary is that the days of bullying a lesser conference are gone: NDSU is certainly capable of winning the league, but the road is much tougher. *(p. 253)*

### Will the lack of talent from the transfer portal burn the Bison?

North Dakota State took a different approach from most, signing 29 high school players and only nine transfers, of whom three played at Power Four programmes last season — one being punter Caleb McGrath. It lost significant talent including two First-Team All-MVFC offensive linemen. The guide explains why the model worked before and why it may not now: recruiting and developing high school players succeeded at the FCS level, but Group of Five teams constantly retool with castoffs and former top prospects seeking a new home. With just 10 starters back and zero portal starters added, it leaves the question genuinely open. *(p. 253)*

### Does the run of quarterback development continue?

Cole Payton going to the Eagles at pick 178 marked the fifth Bison starting quarterback drafted in 11 years, and Nathan Hayes now carries that pressure. Hayes appeared in 10 games, completing 57% for 381 yards with four touchdowns and one interception, and has been in the system since 2022 so there should be no learning curve. The guide's counterweight is the supporting cast: he takes over an offense missing its top two rushers and top two receivers. *(p. 253)*

*TTW reference notes. The questions are VSiN's; the answers summarise VSiN's reasoning in TTW's wording.*

## 27. Source Conflicts

No source conflict identified for this team.

## 28. Relevant Page References

| Pages | Content |
| --- | --- |
| 252–253 | This team's two-page preview |
| 243 | Mountain West conference preview and projected standings |
| 4 | VSiN staff season predictions |
| 46–47 | Steve Makinen power rating methodology |
| 21 | Home-field advantage article |
| 5, 7, 9, 13, 14, 15, 46, 155, 161, 167, 210, 244, 245, 247, 248, 250, 254, 256, 258, 260, 262, 270 | Other pages naming this team |

This team is named in **52 sentences across 25 pages** of the guide.

Every page naming this team outside its own spread: pp. 5, 7, 9, 13, 14, 15, 46, 155, 161, 167, 210, 244, 245, 247, 248, 250, 254, 256, 258, 260, 262, 270.

## 29. Cross-Links

**Conference** — [Mountain West](../01_Conference_Database/mountain_west.md)

**Master Index** — [Team Index](../00_Master_Index/03_Team_Index.md) · [Power Rating Index](../00_Master_Index/09_Power_Rating_Index.md) · [Coaching Index](../00_Master_Index/04_Coaching_Index.md) · [Quarterback Index](../00_Master_Index/06_Quarterback_Index.md)

**Every other database, for this team:**

- Head coach — [03_Coaching_Database/north_dakota_state_bison.md](../03_Coaching_Database/north_dakota_state_bison.md)
- Quarterback — [04_Quarterback_Database/north_dakota_state_bison.md](../04_Quarterback_Database/north_dakota_state_bison.md)
- Power rating — [05_Power_Ratings/00_MAKINEN_RATINGS.md](../05_Power_Ratings/00_MAKINEN_RATINGS.md)
- Win total — [06_Win_Totals/00_ALL_TEAMS.md](../06_Win_Totals/00_ALL_TEAMS.md)
- Futures board — [07_Futures/00_TEAM_FUTURES.md](../07_Futures/00_TEAM_FUTURES.md)
- Returning production — [08_Returning_Production/README.md](../08_Returning_Production/README.md)
- Schedule — [10_Schedule_Intelligence/00_BY_TEAM.md](../10_Schedule_Intelligence/00_BY_TEAM.md)
- Historical angles — [12_Historical_Trends/00_BY_TEAM.md](../12_Historical_Trends/00_BY_TEAM.md)
- Everything at once — [99_Search_Index/02_TEAM_LOOKUP.md](../99_Search_Index/02_TEAM_LOOKUP.md)

