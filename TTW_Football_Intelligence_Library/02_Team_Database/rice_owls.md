<!-- GENERATED FILE — do not hand-edit.
     Rebuild:  python3 _tools/build_teams.py
     Source:   2026 VSiN College Football Betting Guide (345 pp.) -->

# Rice Owls

> **Source: 2026 VSiN College Football Betting Guide, pp. 64–65** and every other page that names this team. GUIDE CONTENT throughout unless a line says otherwise. No outside research. Nothing inferred to fill a heading.


## 1. Program Snapshot

| | |
| --- | --- |
| **Conference** | American |
| **Guide pages** | 64–65 |
| **Head coach** | Scott Abell (2nd season) |
| **2025 straight up** | 5-8 |
| **2025 against the spread** | 5-8 |
| **2025 over/under** | 7-6 |
| **Makinen power rating** | 27.5 |
| **National rank** | #120 of 138 |
| **Conference rank** | #12 of 14 |
| **DraftKings win total** | 3.5 |
| **Makinen projected wins** | 3.3–8.7 overall, 1.9–6.1 in conference |
| **Schedule strength** | 37.1 (#76 toughest of 138) |

### Season outlook — VSiN's analysis in reference form (p. 64)

- Last season was transitional as Scott Abell took what the guide calls one of the most challenging jobs in FBS, stepping up from FCS Davidson. *(p. 64)*
- The constraints are concrete: enrollment under 4,000, strict academic requirements, and no winning season since 2014. *(p. 64)*
- It raises the obvious counterargument — being in Houston amid Texas high school talent should make roster building easier even with a low acceptance rate — and observes it simply has not. *(p. 64)*
- Even last season's bowl came because others withdrew and Rice's Academic Progress Rate scores merited an opportunity, off a 5-7 regular season with 19.1 points per game, offense down nearly a full yard per play and defense conceding 1.3 more. *(p. 64)*
- It expects another tough year while noting Abell seems up for it and brought in a decent number of Power Four transfers. *(p. 64)*
- Recommendation is Under 3.5 wins. The schedule is the crux: favorable last season, not now — Rice faces all the teams projected average or better apart from Navy and does not play Charlotte or UAB, its only two conference wins. *(p. 64)*
- Notre Dame is added along with a trip to Fresno State. The guide flags the finale specifically: if Rice is 3-8 going in, which it calls plausible, the last game is against Army — a terrible matchup for a less accomplished option team. Projection is 3.72 wins. *(p. 64)*

*TTW reference notes summarising VSiN's analysis. GUIDE CONTENT — facts, conclusions and reasoning are the guide's; the wording is TTW's.*


## 2. Conference

**American** — see the conference file for league-wide context: [american.md](../01_Conference_Database/american.md)

Makinen projects **1.9–6.1** in conference play, ranking this team #12 of 14 in the American by power rating.

## 3. VSiN Team Rank / Conference Rank

- **National rank:** #120 of 138 *(p. 65)*
- **Conference rank:** #12 of 14 *(p. 65)*
- **Matt Youmans' Preseason Top 50:** not ranked *(pp. 16–20)*

## 4. Steve Makinen Power Rating

**27.5** — as printed on p. 65, reproduced exactly and not converted to any other scale.

Ranks #120 of 138 nationally and #12 of 14 in the American. Methodology is set out on pp. 46–47.

> No comparison against the TTW workbook is made here. That is a later-phase deliverable.

## 5. Home-Field Advantage Reference

- **Home field rating:** 3.2 *(p. 64)*
- **Road field rating:** -0.2 *(p. 64)*

Printed on the team page as `field ratings (HOME/ROAD)` and verified against the same values in the conference projected standings table. The guide's general treatment of home-field advantage is on p. 21.

## 6. Head Coach

**Scott Abell** — 2nd season at the programme *(p. 64)*.

## 7. Coordinator Notes

- **DC Jon Kay** *(p. 65)*

## 8. Coaching Continuity / Changes

**Steve Makinen's Stability Score: 10** *(p. 44)*

| Component | Status | Points |
| --- | --- | --- |
| Head coach | returns | 4 |
| Offensive coordinator | returns | 3 |
| Defensive coordinator | returns | 3 |
| Starting quarterback | new for 2026 | 0 |
| Returning starters | 6 | 0 |

Scoring is the guide's own: head coach 4, offensive coordinator 3, defensive coordinator 3, starting quarterback 4, plus points for returning starters. A returning transfer quarterback counts as a **new** quarterback in this system *(p. 41)*.

- Last season was transitional as Scott Abell took what the guide calls one of the most challenging jobs in FBS, stepping up from FCS Davidson. *(p. 64)*
- It expects another tough year while noting Abell seems up for it and brought in a decent number of Power Four transfers. *(p. 64)*
- **Is everything 'Kay on defense?** The guide finds it a bit surprising that a new head coach would simply promote the previous regime's linebackers coach, Jon Kay, to coordinator, and says it did not really work: 6.3 yards per play and nearly 33 points allowed, the worst defensive effort since 2022. It adds a striking turnover detail — despite a run-heavy approach Rice finished -4 in turnover margin with just seven takeaways, tied for ninth nationally in fewest turnovers lost yet still negative. The top seven tacklers are gone, which it suggests is probably not a bad thing. *(p. 65)*

## 9. Quarterback Situation

**Starting quarterback continuity:** the Stability Score table treats this team as having a new starting quarterback for 2026 *(p. 44)*. A transfer expected to start counts as new in that system.

**Returning starting quarterback:** no — the guide marks returning quarterbacks with an asterisk on the returning-starters line *(p. 64)*.

- **Can the offense improve in a hurry under Jacurri?** Jacurri Brown spent two years each at Miami and UCF but has thrown only 148 passes with more interceptions than touchdowns, nine to six. What he does well is run: 137 carries for 821 yards, better than six per carry. The guide supplies the schematic precedent from Abell's Davidson, where quarterbacks attempted only 152 passes in 11 games while the team ran 640 times for 3,720 yards, with quarterback Andrew Frazier third in both rushing attempts and yards. Quinton Jackson was Rice's best offensive player with 889 yards at 4.9 per carry. With Rice at 647 rushing attempts against 224 passes last season, it expects an even starker split. *(p. 65)*

Also referenced on **pp. 57** — not reproduced here.

## 10. Returning Production

| | Returning starters |
| --- | --- |
| **Total** | 6 |
| **Offence** | 3 |
| **Defence** | 3 |

Read by position from p. 64. The three figures sit under the printed labels `total / offense / defense`; they are matched to those labels by their place on the page, because the PDF text layer emits them in a different order than they are printed.

The Stability Score table prints the same figure independently: **6 returning starters** *(p. 44)*.

## 11. Transfer Portal

- It expects another tough year while noting Abell seems up for it and brought in a decent number of Power Four transfers. *(p. 64)*
- **Can the offense improve in a hurry under Jacurri?** Jacurri Brown spent two years each at Miami and UCF but has thrown only 148 passes with more interceptions than touchdowns, nine to six. What he does well is run: 137 carries for 821 yards, better than six per carry. The guide supplies the schematic precedent from Abell's Davidson, where quarterbacks attempted only 152 passes in 11 games while the team ran 640 times for 3,720 yards, with quarterback Andrew Frazier third in both rushing attempts and yards. Quinton Jackson was Rice's best offensive player with 889 yards at 4.9 per carry. With Rice at 647 rushing attempts against 224 passes last season, it expects an even starker split. *(p. 65)*


Referenced in the guide on **pp. 57** — those passages are not reproduced here; see the pages for VSiN's own wording.

## 12. Recruiting / Roster Notes

- The constraints are concrete: enrollment under 4,000, strict academic requirements, and no winning season since 2014. *(p. 64)*
- It raises the obvious counterargument — being in Houston amid Texas high school talent should make roster building easier even with a low acceptance rate — and observes it simply has not. *(p. 64)*


Referenced in the guide on **pp. 57** — those passages are not reproduced here; see the pages for VSiN's own wording.

## 13. Offensive Identity

- Even last season's bowl came because others withdrew and Rice's Academic Progress Rate scores merited an opportunity, off a 5-7 regular season with 19.1 points per game, offense down nearly a full yard per play and defense conceding 1.3 more. *(p. 64)*
- **Can the offense improve in a hurry under Jacurri?** Jacurri Brown spent two years each at Miami and UCF but has thrown only 148 passes with more interceptions than touchdowns, nine to six. What he does well is run: 137 carries for 821 yards, better than six per carry. The guide supplies the schematic precedent from Abell's Davidson, where quarterbacks attempted only 152 passes in 11 games while the team ran 640 times for 3,720 yards, with quarterback Andrew Frazier third in both rushing attempts and yards. Quinton Jackson was Rice's best offensive player with 889 yards at 4.9 per carry. With Rice at 647 rushing attempts against 224 passes last season, it expects an even starker split. *(p. 65)*
- **Can the option offense even be successful?** The guide distinguishes between option attacks — Navy has modified theirs and Army runs theirs like a well-oiled machine — then makes a conference-specific argument: Rice is attempting its own version in a league that already contains two service academies, which makes scheming easier for opposing defenses seeing similar core concepts and blocking. It supports this with results: Rice did not face Army last season but managed 3.1 yards per carry against Navy, 2.4 against Memphis, 3.8 against UTSA, 3.4 against UAB and 3.8 against South Florida. It allows Brown could be a difference-maker as a better runner than Chase Jenkins while concluding this may not be the best conference for the approach. *(p. 65)*

## 14. Defensive Identity

- Even last season's bowl came because others withdrew and Rice's Academic Progress Rate scores merited an opportunity, off a 5-7 regular season with 19.1 points per game, offense down nearly a full yard per play and defense conceding 1.3 more. *(p. 64)*
- **Is everything 'Kay on defense?** The guide finds it a bit surprising that a new head coach would simply promote the previous regime's linebackers coach, Jon Kay, to coordinator, and says it did not really work: 6.3 yards per play and nearly 33 points allowed, the worst defensive effort since 2022. It adds a striking turnover detail — despite a run-heavy approach Rice finished -4 in turnover margin with just seven takeaways, tied for ninth nationally in fewest turnovers lost yet still negative. The top seven tacklers are gone, which it suggests is probably not a bad thing. *(p. 65)*

## 15. Key Strengths

National top-25 finishes in the guide's printed categories *(p. 65)*:

| Side | Category | Value | National rank |
| --- | --- | --- | --- |
| Offense | RUSH ATTEMPTS PER GAME | 48.9 | #4 |
| Offense | TIME OF POSSESSION | 32:25 | #13 |
| Offense | TURNOVERS | 0.9 | #15 |
| Offense | COMPLETION % | 66.3% | #21 |
| Offense | RUSH YARDS PER GAME | 192.8 | #25 |

## 16. Key Weaknesses

National bottom-25 finishes in the guide's printed categories *(p. 65)*:

| Side | Category | Value | National rank |
| --- | --- | --- | --- |
| Offense | PASSING YARDS PER GAME | 96.3 | #135 |
| Offense | PASS ATTEMPTS PER GAME | 17.1 | #133 |
| Defense | TURNOVERS | 0.6 | #133 |
| Offense | YARDS PER PLAY | 4.3 | #131 |
| Offense | YARDS PER PASS ATTEMPT | 5.6 | #131 |
| Offense | TOTAL YARDS PER GAME | 289.1 | #128 |
| Defense | YARDS PER POINT | 12.2 | #128 |
| Defense | YARDS PER PASS ATTEMPT | 8.6 | #124 |
| Offense | POINTS PER GAME | 17.5 | #123 |
| Defense | POINTS PER GAME | 34.3 | #122 |
| Defense | YARDS PER PLAY | 6.3 | #122 |

## 17. Schedule Overview

Schedule strength **37.1**, **#76 toughest of 138** *(p. 64)*.

| Date | Opponent | Site | Projected line | Opponent power rating |
| --- | --- | --- | --- | --- |
| 9/5 | HOUSTON BAPTIST | Home | -33 | -1.5 |
| 9/12 | at Notre Dame | Away | +45 | 68.5 |
| 9/19 | W MICHIGAN | Home | +6.5 | 37.5 |
| 9/26 | at Fresno St | Away | +18 | 42.5 |
| 10/3 | TX-SAN ANTONIO | Home | +9.8 | 41.0 |
| 10/10 | at East Carolina | Away | +16.2 | 40.5 |
| 10/17 | TULSA | Home | +1.2 | 32.5 |
| 10/24 | at Fla Atlantic | Away | +8.2 | 33.5 |
| 11/7 | at North Texas | Away | +10.7 | 34.5 |
| 11/14 | TULANE | Home | +12.4 | 42.5 |
| 11/19 | at Temple | Away | +10.1 | 34.0 |
| 11/28 | ARMY | Home | +10.1 | 40.0 |

Projected lines and opponent power ratings are Makinen's, printed alongside the schedule on the team page.

## 18. Difficult Stretches / Trap Spots

- Notre Dame is added along with a trip to Fresno State. The guide flags the finale specifically: if Rice is 3-8 going in, which it calls plausible, the last game is against Army — a terrible matchup for a less accomplished option team. Projection is 3.72 wins. *(p. 64)*

## 19. Win Total Discussion

- **DraftKings win total:** 3.5 *(p. 49)*
- **Makinen projected wins:** 3.3 *(p. 49)*
- **Difference:** -0.2 wins against the posted number
- **Recommendation on the team page:** **UNDER 3.5** *(p. 64)*
- **Projection stated in the team-page essay:** 3.72 wins *(p. 64)*

## 20. Futures / Conference / Playoff Discussion

Prices printed on the team page *(p. 65)*:

| Market | Price |
| --- | --- |
| CFP Championship | 5000-1 |
| make the playoff | 500-1 |
| AMERICAN | 200-1 |

## 21. Betting Notes / Best Bets

- Recommendation is Under 3.5 wins. The schedule is the crux: favorable last season, not now — Rice faces all the teams projected average or better apart from Navy and does not play Charlotte or UAB, its only two conference wins. *(p. 64)*
- Notre Dame is added along with a trip to Fresno State. The guide flags the finale specifically: if Rice is 3-8 going in, which it calls plausible, the last game is against Army — a terrible matchup for a less accomplished option team. Projection is 3.72 wins. *(p. 64)*

Also referenced on **pp. 77** — not reproduced here.

## 22. Historical / Situational Trends

- Even last season's bowl came because others withdrew and Rice's Academic Progress Rate scores merited an opportunity, off a 5-7 regular season with 19.1 points per game, offense down nearly a full yard per play and defense conceding 1.3 more. *(p. 64)*


Referenced in the guide on **pp. 57, 77** — those passages are not reproduced here; see the pages for VSiN's own wording.

## 23. Important Statistics

As printed on p. 65. Each category carries a value and a national rank.

### Offence

| Category | Value | National rank |
| --- | --- | --- |
| POINTS PER GAME | 17.5 | 123 |
| YARDS PER POINT | 16.5 | 109 |
| PLAYS PER GAME | 67.5 | 81 |
| TIME OF POSSESSION | 32:25 | 13 |
| 3RD DOWN CONV. % | 33.5% | 110 |
| TOTAL YARDS PER GAME | 289.1 | 128 |
| YARDS PER PLAY | 4.3 | 131 |
| RUSH ATTEMPTS PER GAME | 48.9 | 4 |
| RUSH YARDS PER GAME | 192.8 | 25 |
| YARDS PER RUSH ATTEMPT | 3.9 | 93 |
| PASS ATTEMPTS PER GAME | 17.1 | 133 |
| COMPLETION % | 66.3% | 21 |
| PASSING YARDS PER GAME | 96.3 | 135 |
| YARDS PER PASS ATTEMPT | 5.6 | 131 |
| TURNOVERS | 0.9 | 15 |

### Defence

| Category | Value | National rank |
| --- | --- | --- |
| POINTS PER GAME | 34.3 | 122 |
| YARDS PER POINT | 12.2 | 128 |
| 3RD DOWN CONV. % | 43.2% | 100 |
| TOTAL YARDS PER GAME | 416.5 | 106 |
| YARDS PER PLAY | 6.3 | 122 |
| RUSH YARDS PER GAME | 169 | 88 |
| YARDS PER RUSH ATTEMPT | 4.8 | 99 |
| COMPLETION % | 62.8% | 77 |
| PASSING YARDS PER GAME | 247.5 | 110 |
| YARDS PER PASS ATTEMPT | 8.6 | 124 |
| SACKS | 1.9 | 69 |
| TURNOVERS | 0.6 | 133 |

## 24. Bull Case

- The constraints are concrete: enrollment under 4,000, strict academic requirements, and no winning season since 2014. *(p. 64)*
- Recommendation is Under 3.5 wins. The schedule is the crux: favorable last season, not now — Rice faces all the teams projected average or better apart from Navy and does not play Charlotte or UAB, its only two conference wins. *(p. 64)*

> *Assembled from statements the guide makes about this team. The statements are GUIDE CONTENT with page references; the selection of which ones argue this side is PERSONAL INFERENCE.*

## 25. Bear Case

- Makinen projects **3.3 wins** against a posted total of 3.5 — **-0.2** against this team.
- The team page recommends **UNDER 3.5**.
- It expects another tough year while noting Abell seems up for it and brought in a decent number of Power Four transfers. *(p. 64)*

> *Assembled from statements the guide makes about this team. The statements are GUIDE CONTENT with page references; the selection of which ones argue this side is PERSONAL INFERENCE.*

## 26. Open Questions / Risks

VSiN poses *Three Burning Questions for the 2026 Season* *(p. 65)*. Its analysis, in reference form:

### Can the offense improve in a hurry under Jacurri?

Jacurri Brown spent two years each at Miami and UCF but has thrown only 148 passes with more interceptions than touchdowns, nine to six. What he does well is run: 137 carries for 821 yards, better than six per carry. The guide supplies the schematic precedent from Abell's Davidson, where quarterbacks attempted only 152 passes in 11 games while the team ran 640 times for 3,720 yards, with quarterback Andrew Frazier third in both rushing attempts and yards. Quinton Jackson was Rice's best offensive player with 889 yards at 4.9 per carry. With Rice at 647 rushing attempts against 224 passes last season, it expects an even starker split. *(p. 65)*

### Can the option offense even be successful?

The guide distinguishes between option attacks — Navy has modified theirs and Army runs theirs like a well-oiled machine — then makes a conference-specific argument: Rice is attempting its own version in a league that already contains two service academies, which makes scheming easier for opposing defenses seeing similar core concepts and blocking. It supports this with results: Rice did not face Army last season but managed 3.1 yards per carry against Navy, 2.4 against Memphis, 3.8 against UTSA, 3.4 against UAB and 3.8 against South Florida. It allows Brown could be a difference-maker as a better runner than Chase Jenkins while concluding this may not be the best conference for the approach. *(p. 65)*

### Is everything 'Kay on defense?

The guide finds it a bit surprising that a new head coach would simply promote the previous regime's linebackers coach, Jon Kay, to coordinator, and says it did not really work: 6.3 yards per play and nearly 33 points allowed, the worst defensive effort since 2022. It adds a striking turnover detail — despite a run-heavy approach Rice finished -4 in turnover margin with just seven takeaways, tied for ninth nationally in fewest turnovers lost yet still negative. The top seven tacklers are gone, which it suggests is probably not a bad thing. *(p. 65)*

*TTW reference notes. The questions are VSiN's; the answers summarise VSiN's reasoning in TTW's wording.*

## 27. Source Conflicts

No source conflict identified for this team.

## 28. Relevant Page References

| Pages | Content |
| --- | --- |
| 64–65 | This team's two-page preview |
| 49 | American conference preview and projected standings |
| 4 | VSiN staff season predictions |
| 46–47 | Steve Makinen power rating methodology |
| 21 | Home-field advantage article |
| 5, 52, 57, 68, 77, 144, 212, 240, 241, 270 | Other pages naming this team |

This team is named in **23 sentences across 12 pages** of the guide.

Every page naming this team outside its own spread: pp. 5, 52, 57, 68, 77, 144, 212, 240, 241, 270.

## 29. Cross-Links

**Conference** — [American](../01_Conference_Database/american.md)

**Master Index** — [Team Index](../00_Master_Index/03_Team_Index.md) · [Power Rating Index](../00_Master_Index/09_Power_Rating_Index.md) · [Coaching Index](../00_Master_Index/04_Coaching_Index.md) · [Quarterback Index](../00_Master_Index/06_Quarterback_Index.md)

**Every other database, for this team:**

- Head coach — [03_Coaching_Database/rice_owls.md](../03_Coaching_Database/rice_owls.md)
- Quarterback — [04_Quarterback_Database/rice_owls.md](../04_Quarterback_Database/rice_owls.md)
- Power rating — [05_Power_Ratings/00_MAKINEN_RATINGS.md](../05_Power_Ratings/00_MAKINEN_RATINGS.md)
- Win total — [06_Win_Totals/00_ALL_TEAMS.md](../06_Win_Totals/00_ALL_TEAMS.md)
- Futures board — [07_Futures/00_TEAM_FUTURES.md](../07_Futures/00_TEAM_FUTURES.md)
- Returning production — [08_Returning_Production/README.md](../08_Returning_Production/README.md)
- Schedule — [10_Schedule_Intelligence/00_BY_TEAM.md](../10_Schedule_Intelligence/00_BY_TEAM.md)
- Historical angles — [12_Historical_Trends/00_BY_TEAM.md](../12_Historical_Trends/00_BY_TEAM.md)
- Everything at once — [99_Search_Index/02_TEAM_LOOKUP.md](../99_Search_Index/02_TEAM_LOOKUP.md)

