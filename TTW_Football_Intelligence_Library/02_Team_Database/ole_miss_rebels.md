<!-- GENERATED FILE — do not hand-edit.
     Rebuild:  python3 _tools/build_teams.py
     Source:   2026 VSiN College Football Betting Guide (345 pp.) -->

# Ole Miss Rebels

> **Source: 2026 VSiN College Football Betting Guide, pp. 304–305** and every other page that names this team. GUIDE CONTENT throughout unless a line says otherwise. No outside research. Nothing inferred to fill a heading.


## 1. Program Snapshot

| | |
| --- | --- |
| **Conference** | SEC |
| **Guide pages** | 304–305 |
| **Head coach** | Pete Golding (1st season) |
| **2025 straight up** | 13-2 |
| **2025 against the spread** | 9-6 |
| **2025 over/under** | 8-7 |
| **Makinen power rating** | 60.5 |
| **National rank** | #12 of 138 |
| **Conference rank** | #7 of 16 |
| **DraftKings win total** | 7.5 |
| **Makinen projected wins** | 8.0–4.0 overall, 5.3–3.7 in conference |
| **Schedule strength** | 49.66 (#13 toughest of 138) |

### Season outlook — VSiN's analysis in reference form (p. 304)

- The guide calls Ole Miss the SEC's most compelling narrative: a run to the playoff semifinals where the Rebels led Miami with three minutes left before folding. *(p. 304)*
- Pete Golding is now a first-time head coach charged with sustaining what he achieved on an interim basis, having replaced his offensive staff and retooled the skill positions. The guide poses the question directly — does he have head-coaching ability, or did he benefit from inheriting Lane Kiffin's team mid-run? *(p. 304)*
- Recommendation is Under 7.5 wins. Ole Miss returns 16 starters from a team a drive from the national championship game, but the guide argues the departures matter more: the architects of one of the country's better offenses, plus key pieces on both lines. *(p. 304)*
- Schedule adds to the case: road trips to Florida, Texas and Oklahoma; home games against LSU in an emotionally charged Week 3, Missouri and Georgia; and an opener against a good Louisville side in Nashville. *(p. 304)*
- The guide states its doubt about Golding plainly, and concludes that combined with a taxing schedule it could mean a disappointing season in Oxford. *(p. 304)*

*TTW reference notes summarising VSiN's analysis. GUIDE CONTENT — facts, conclusions and reasoning are the guide's; the wording is TTW's.*


## 2. Conference

**SEC** — see the conference file for league-wide context: [sec.md](../01_Conference_Database/sec.md)

Makinen projects **5.3–3.7** in conference play, ranking this team #7 of 16 in the SEC by power rating.

## 3. VSiN Team Rank / Conference Rank

- **National rank:** #12 of 138 *(p. 305)*
- **Conference rank:** #7 of 16 *(p. 305)*
- **Matt Youmans' Preseason Top 50:** **#9** *(pp. 16–20)*

## 4. Steve Makinen Power Rating

**60.5** — as printed on p. 305, reproduced exactly and not converted to any other scale.

Ranks #12 of 138 nationally and #7 of 16 in the SEC. Methodology is set out on pp. 46–47.

> No comparison against the TTW workbook is made here. That is a later-phase deliverable.

## 5. Home-Field Advantage Reference

- **Home field rating:** 3.3 *(p. 304)*
- **Road field rating:** 0.1 *(p. 304)*

Printed on the team page as `field ratings (HOME/ROAD)` and verified against the same values in the conference projected standings table. The guide's general treatment of home-field advantage is on p. 21.

## 6. Head Coach

**Pete Golding** — 1st season at the programme *(p. 304)*.

Profiled among the guide's new head coaches in The Coaching Carousel *(p. 36)*.

## 7. Coordinator Notes

- **OC Charlie Weis Jr** *(p. 305)*

## 8. Coaching Continuity / Changes

**Steve Makinen's Stability Score: 10** *(p. 43)*

| Component | Status | Points |
| --- | --- | --- |
| Head coach | new for 2026 | 0 |
| Offensive coordinator | new for 2026 | 0 |
| Defensive coordinator | returns | 3 |
| Starting quarterback | returns | 4 |
| Returning starters | 16 | 3 |

Scoring is the guide's own: head coach 4, offensive coordinator 3, defensive coordinator 3, starting quarterback 4, plus points for returning starters. A returning transfer quarterback counts as a **new** quarterback in this system *(p. 41)*.

- Pete Golding is now a first-time head coach charged with sustaining what he achieved on an interim basis, having replaced his offensive staff and retooled the skill positions. The guide poses the question directly — does he have head-coaching ability, or did he benefit from inheriting Lane Kiffin's team mid-run? *(p. 304)*
- The guide states its doubt about Golding plainly, and concludes that combined with a taxing schedule it could mean a disappointing season in Oxford. *(p. 304)*
- **How much will the offense differ after an offseason of change?** Kiffin left before the playoff run but the staff he had chosen stayed through the postseason. Offensive coordinator Charlie Weis Jr., described as central to the offense, has now gone to Baton Rouge. John David Baker, East Carolina's coordinator in 2025, replaces him and to some degree replaces Kiffin. The guide flags the practical consequences: new terminology and unknown play structure and flow. All four top receivers from 2025 are gone via draft, portal or otherwise. Kewan Lacy remains a major piece in the backfield, but the guide warns of a possible large shift in the unit's efficiency. *(p. 305)*
- **Can Pete Golding prove that he can do it on his own?** The guide treats this as the season's crux. Golding had Weis calling plays in the postseason and now does not; he has had to hire a coordinator, rebuild groups such as receiver, and take charge of every facet. It does not dispute the success under him last season but insists the situation is now materially different — a first-time head coach steering a programme with national title ambitions through a brutal conference. As a defensive coach, his coordinator choice is expected to hang over the season if the offense starts slowly. *(p. 305)*

Also referenced on **pp. 11, 18, 38, 54, 95, 296, 327** — not reproduced here.

## 9. Quarterback Situation

**Paul Stone ranks Trinidad Chambliss the #2 quarterback in the country** *(p. 45)*.

**Starting quarterback continuity:** the Stability Score table treats this team as having the same starting quarterback as 2025 *(p. 43)*. A transfer expected to start counts as new in that system.

**Returning starting quarterback:** yes — the guide marks returning quarterbacks with an asterisk on the returning-starters line *(p. 304)*.

- **What can Trinidad Chambliss do for an encore?** Chambliss only got the job through Austin Simmons' injury but never gave it back, finishing as PFF's third-highest graded SEC passer: 65.6% completions, 8.8 yards per attempt, 22 touchdowns to three interceptions. The guide highlights ball security as the contrast with Simmons — a 2.5% turnover-worthy play rate, third-best among SEC starters. He enters 2026 as the unquestioned starter and a Heisman candidate, with the open question being whether he can repeat it amid heavy turnover in the receiver room. *(p. 305)*

Also referenced on **pp. 8, 18, 19, 301, 345** — not reproduced here.

## 10. Returning Production

| | Returning starters |
| --- | --- |
| **Total** | 16 |
| **Offence** | 7 *(includes the returning quarterback)* |
| **Defence** | 9 |

Read by position from p. 304. The three figures sit under the printed labels `total / offense / defense`; they are matched to those labels by their place on the page, because the PDF text layer emits them in a different order than they are printed.

The Stability Score table prints the same figure independently: **16 returning starters** *(p. 43)*.

## 11. Transfer Portal

- **How much will the offense differ after an offseason of change?** Kiffin left before the playoff run but the staff he had chosen stayed through the postseason. Offensive coordinator Charlie Weis Jr., described as central to the offense, has now gone to Baton Rouge. John David Baker, East Carolina's coordinator in 2025, replaces him and to some degree replaces Kiffin. The guide flags the practical consequences: new terminology and unknown play structure and flow. All four top receivers from 2025 are gone via draft, portal or otherwise. Kewan Lacy remains a major piece in the backfield, but the guide warns of a possible large shift in the unit's efficiency. *(p. 305)*


Referenced in the guide on **pp. 8, 11, 18, 19, 73, 195, 296, 297, 331** — those passages are not reproduced here; see the pages for VSiN's own wording.

## 12. Recruiting / Roster Notes

Not addressed in guide under this heading.

The guide names this team on **pp. 11**, but no passage there was extracted under this heading; see those pages for VSiN's own wording.

## 13. Offensive Identity

- Recommendation is Under 7.5 wins. Ole Miss returns 16 starters from a team a drive from the national championship game, but the guide argues the departures matter more: the architects of one of the country's better offenses, plus key pieces on both lines. *(p. 304)*
- **How much will the offense differ after an offseason of change?** Kiffin left before the playoff run but the staff he had chosen stayed through the postseason. Offensive coordinator Charlie Weis Jr., described as central to the offense, has now gone to Baton Rouge. John David Baker, East Carolina's coordinator in 2025, replaces him and to some degree replaces Kiffin. The guide flags the practical consequences: new terminology and unknown play structure and flow. All four top receivers from 2025 are gone via draft, portal or otherwise. Kewan Lacy remains a major piece in the backfield, but the guide warns of a possible large shift in the unit's efficiency. *(p. 305)*


Referenced in the guide on **pp. 8, 18, 300, 318, 327** — those passages are not reproduced here; see the pages for VSiN's own wording.

## 14. Defensive Identity

- Recommendation is Under 7.5 wins. Ole Miss returns 16 starters from a team a drive from the national championship game, but the guide argues the departures matter more: the architects of one of the country's better offenses, plus key pieces on both lines. *(p. 304)*


Referenced in the guide on **pp. 18, 95, 283, 297** — those passages are not reproduced here; see the pages for VSiN's own wording.

## 15. Key Strengths

National top-25 finishes in the guide's printed categories *(p. 305)*:

| Side | Category | Value | National rank |
| --- | --- | --- | --- |
| Offense | TOTAL YARDS PER GAME | 482.3 | #3 |
| Offense | PASSING YARDS PER GAME | 303.4 | #4 |
| Offense | YARDS PER PLAY | 6.6 | #8 |
| Offense | YARDS PER PASS ATTEMPT | 8.9 | #9 |
| Offense | POINTS PER GAME | 36.1 | #11 |
| Offense | PLAYS PER GAME | 73.4 | #13 |
| Defense | COMPLETION % | 57.1% | #20 |
| Offense | TURNOVERS | 0.9 | #21 |

## 16. Key Weaknesses

No category ranks inside the national bottom 25.

## 17. Schedule Overview

Schedule strength **49.66**, **#13 toughest of 138** *(p. 304)*.

| Date | Opponent | Site | Projected line | Opponent power rating |
| --- | --- | --- | --- | --- |
| 9/6 | vs. Louisville | Neutral | -7.2 | 53.5 |
| 9/12 | CHARLOTTE | Home | -42.8 | 20.5 |
| 9/19 | LSU | Home | -3.6 | 60.5 |
| 9/26 | at Florida | Away | -2.2 | 55.5 |
| 10/10 | at Vanderbilt | Away | -6.4 | 51.5 |
| 10/17 | MISSOURI | Home | -8.8 | 54.5 |
| 10/24 | at Texas | Away | +8.6 | 66.0 |
| 10/31 | AUBURN | Home | -11.1 | 52.5 |
| 11/7 | GEORGIA | Home | +3.5 | 67.5 |
| 11/14 | at Oklahoma | Away | +3.4 | 61.0 |
| 11/21 | WOFFORD | Home | -46.8 | 16.5 |
| 11/27 | MISSISSIPPI ST | Home | -16.7 | 47.0 |

Projected lines and opponent power ratings are Makinen's, printed alongside the schedule on the team page.

## 18. Difficult Stretches / Trap Spots

Not addressed in guide under this heading.

The guide names this team on **pp. 305, 310**, but no passage there was extracted under this heading; see those pages for VSiN's own wording.

## 19. Win Total Discussion

- **DraftKings win total:** 7.5 *(p. 283)*
- **Makinen projected wins:** 8.0 *(p. 283)*
- **Difference:** +0.5 wins against the posted number
- **Recommendation on the team page:** **UNDER 7.5** *(p. 304)*

## 20. Futures / Conference / Playoff Discussion

Prices printed on the team page *(p. 305)*:

| Market | Price |
| --- | --- |
| CFP Championship | 30-1 |
| make the playoff | +220 |
| SEC | 11-1 |

## 21. Betting Notes / Best Bets

**Host best bets naming this team** *(pp. 5–15)*:

| Contributor | Pick | Page |
| --- | --- | --- |
| Mitch Moss | OLE MISS OVER 8.5 WINS (+150) & TO MAKE COLLEGE FOOTBALL PLAYOFF (+220) | 11 |

Where contributors disagree, every position is kept separately and none is reconciled.

- Recommendation is Under 7.5 wins. Ole Miss returns 16 starters from a team a drive from the national championship game, but the guide argues the departures matter more: the architects of one of the country's better offenses, plus key pieces on both lines. *(p. 304)*
- The guide states its doubt about Golding plainly, and concludes that combined with a taxing schedule it could mean a disappointing season in Oxford. *(p. 304)*

Also referenced on **pp. 15** — not reproduced here.

## 22. Historical / Situational Trends

- The guide calls Ole Miss the SEC's most compelling narrative: a run to the playoff semifinals where the Rebels led Miami with three minutes left before folding. *(p. 304)*


Referenced in the guide on **pp. 45, 73, 92, 195, 296, 297, 301, 327** — those passages are not reproduced here; see the pages for VSiN's own wording.

## 23. Important Statistics

As printed on p. 305. Each category carries a value and a national rank.

### Offence

| Category | Value | National rank |
| --- | --- | --- |
| POINTS PER GAME | 36.1 | 11 |
| YARDS PER POINT | 13.4 | 37 |
| PLAYS PER GAME | 73.4 | 13 |
| TIME OF POSSESSION | 28:47 | 102 |
| 3RD DOWN CONV. % | 42.1% | 39 |
| TOTAL YARDS PER GAME | 482.3 | 3 |
| YARDS PER PLAY | 6.6 | 8 |
| RUSH ATTEMPTS PER GAME | 38.3 | 37 |
| RUSH YARDS PER GAME | 178.9 | 34 |
| YARDS PER RUSH ATTEMPT | 4.7 | 37 |
| PASS ATTEMPTS PER GAME | 34.1 | 37 |
| COMPLETION % | 63.5% | 46 |
| PASSING YARDS PER GAME | 303.4 | 4 |
| YARDS PER PASS ATTEMPT | 8.9 | 9 |
| TURNOVERS | 0.9 | 21 |

### Defence

| Category | Value | National rank |
| --- | --- | --- |
| POINTS PER GAME | 22.6 | 41 |
| YARDS PER POINT | 16.2 | 29 |
| 3RD DOWN CONV. % | 38.0% | 55 |
| TOTAL YARDS PER GAME | 365.6 | 53 |
| YARDS PER PLAY | 5.3 | 45 |
| RUSH YARDS PER GAME | 154.1 | 71 |
| YARDS PER RUSH ATTEMPT | 4.4 | 73 |
| COMPLETION % | 57.1% | 20 |
| PASSING YARDS PER GAME | 211.5 | 47 |
| YARDS PER PASS ATTEMPT | 6.6 | 33 |
| SACKS | 2.2 | 47 |
| TURNOVERS | 1.1 | 97 |

## 24. Bull Case

- Makinen projects **8.0 wins** against a posted total of 7.5 — **+0.5** in this team's favour.
- Schedule adds to the case: road trips to Florida, Texas and Oklahoma; home games against LSU in an emotionally charged Week 3, Missouri and Georgia; and an opener against a good Louisville side in Nashville. *(p. 304)*

*1 further statement about this team carries both positive and negative language and is not classified to either side. It appears in full under the season outlook and *Open Questions / Risks*.*

> *Assembled from statements the guide makes about this team. The statements are GUIDE CONTENT with page references; the selection of which ones argue this side is PERSONAL INFERENCE.*

## 25. Bear Case

- The team page recommends **UNDER 7.5**.
- The guide calls Ole Miss the SEC's most compelling narrative: a run to the playoff semifinals where the Rebels led Miami with three minutes left before folding. *(p. 304)*
- Pete Golding is now a first-time head coach charged with sustaining what he achieved on an interim basis, having replaced his offensive staff and retooled the skill positions. The guide poses the question directly — does he have head-coaching ability, or did he benefit from inheriting Lane Kiffin's team mid-run? *(p. 304)*

*1 further statement about this team carries both positive and negative language and is not classified to either side. It appears in full under the season outlook and *Open Questions / Risks*.*

> *Assembled from statements the guide makes about this team. The statements are GUIDE CONTENT with page references; the selection of which ones argue this side is PERSONAL INFERENCE.*

## 26. Open Questions / Risks

VSiN poses *Three Burning Questions for the 2026 Season* *(p. 305)*. Its analysis, in reference form:

### What can Trinidad Chambliss do for an encore?

Chambliss only got the job through Austin Simmons' injury but never gave it back, finishing as PFF's third-highest graded SEC passer: 65.6% completions, 8.8 yards per attempt, 22 touchdowns to three interceptions. The guide highlights ball security as the contrast with Simmons — a 2.5% turnover-worthy play rate, third-best among SEC starters. He enters 2026 as the unquestioned starter and a Heisman candidate, with the open question being whether he can repeat it amid heavy turnover in the receiver room. *(p. 305)*

### How much will the offense differ after an offseason of change?

Kiffin left before the playoff run but the staff he had chosen stayed through the postseason. Offensive coordinator Charlie Weis Jr., described as central to the offense, has now gone to Baton Rouge. John David Baker, East Carolina's coordinator in 2025, replaces him and to some degree replaces Kiffin. The guide flags the practical consequences: new terminology and unknown play structure and flow. All four top receivers from 2025 are gone via draft, portal or otherwise. Kewan Lacy remains a major piece in the backfield, but the guide warns of a possible large shift in the unit's efficiency. *(p. 305)*

### Can Pete Golding prove that he can do it on his own?

The guide treats this as the season's crux. Golding had Weis calling plays in the postseason and now does not; he has had to hire a coordinator, rebuild groups such as receiver, and take charge of every facet. It does not dispute the success under him last season but insists the situation is now materially different — a first-time head coach steering a programme with national title ambitions through a brutal conference. As a defensive coach, his coordinator choice is expected to hang over the season if the offense starts slowly. *(p. 305)*

*TTW reference notes. The questions are VSiN's; the answers summarise VSiN's reasoning in TTW's wording.*

## 27. Source Conflicts

No source conflict identified for this team.

## 28. Relevant Page References

| Pages | Content |
| --- | --- |
| 304–305 | This team's two-page preview |
| 283 | SEC conference preview and projected standings |
| 4 | VSiN staff season predictions |
| 46–47 | Steve Makinen power rating methodology |
| 21 | Home-field advantage article |
| 8, 11, 15, 18, 19, 36, 38, 45, 54, 70, 73, 79, 92, 95, 143, 195, 236, 288, 289, 290, 292, 294, 296, 297, 299, 300, 301, 302, 310, 318, 327, 331, 345 | Other pages naming this team |

This team is named in **55 sentences across 36 pages** of the guide.

Every page naming this team outside its own spread: pp. 8, 11, 15, 18, 19, 36, 38, 45, 54, 70, 73, 79, 92, 95, 143, 195, 236, 288, 289, 290, 292, 294, 296, 297, 299, 300, 301, 302, 310, 318, 327, 331, 345.

## 29. Cross-Links

**Conference** — [SEC](../01_Conference_Database/sec.md)

**Master Index** — [Team Index](../00_Master_Index/03_Team_Index.md) · [Power Rating Index](../00_Master_Index/09_Power_Rating_Index.md) · [Coaching Index](../00_Master_Index/04_Coaching_Index.md) · [Quarterback Index](../00_Master_Index/06_Quarterback_Index.md)

**Every other database, for this team:**

- Head coach — [03_Coaching_Database/ole_miss_rebels.md](../03_Coaching_Database/ole_miss_rebels.md)
- Quarterback — [04_Quarterback_Database/ole_miss_rebels.md](../04_Quarterback_Database/ole_miss_rebels.md)
- Power rating — [05_Power_Ratings/00_MAKINEN_RATINGS.md](../05_Power_Ratings/00_MAKINEN_RATINGS.md)
- Win total — [06_Win_Totals/00_ALL_TEAMS.md](../06_Win_Totals/00_ALL_TEAMS.md)
- Futures board — [07_Futures/00_TEAM_FUTURES.md](../07_Futures/00_TEAM_FUTURES.md)
- Returning production — [08_Returning_Production/README.md](../08_Returning_Production/README.md)
- Schedule — [10_Schedule_Intelligence/00_BY_TEAM.md](../10_Schedule_Intelligence/00_BY_TEAM.md)
- Historical angles — [12_Historical_Trends/00_BY_TEAM.md](../12_Historical_Trends/00_BY_TEAM.md)
- Everything at once — [99_Search_Index/02_TEAM_LOOKUP.md](../99_Search_Index/02_TEAM_LOOKUP.md)

