<!-- GENERATED FILE — do not hand-edit.
     Rebuild:  python3 _tools/build_teams.py
     Source:   2026 VSiN College Football Betting Guide (345 pp.) -->

# Oregon Ducks

> **Source: 2026 VSiN College Football Betting Guide, pp. 170–171** and every other page that names this team. GUIDE CONTENT throughout unless a line says otherwise. No outside research. Nothing inferred to fill a heading.


## 1. Program Snapshot

| | |
| --- | --- |
| **Conference** | Big Ten |
| **Guide pages** | 170–171 |
| **Head coach** | Dan Lanning (5th season) |
| **2025 straight up** | 13-2 |
| **2025 against the spread** | 9-6 |
| **2025 over/under** | 8-7 |
| **Makinen power rating** | 68.5 |
| **National rank** | #2 of 138 |
| **Conference rank** | #2 of 18 |
| **DraftKings win total** | 10.5 |
| **Makinen projected wins** | 10.3–1.7 overall, 7.4–1.6 in conference |
| **Schedule strength** | 47.28 (#28 toughest of 138) |

### Season outlook — VSiN's analysis in reference form (p. 170)

- Dan Lanning is 48-8 since 2022, with playoff appearances in both years and a run through the Big Ten in Oregon's first conference season in 2024. *(p. 170)*
- What is missing is a national title, and the guide is pointed about how the last two seasons ended: blowout losses to the eventual champion in both, Ohio State in the Rose Bowl and Indiana in the Peach Bowl. *(p. 170)*
- Oregon starts as a victim of its own success, losing offensive coordinator Will Stein to Kentucky and Tosh Lupoi to Cal, both for head coaching jobs. Drew Mehringer and Chris Hampton are promoted internally. *(p. 170)*
- Despite the staff churn Oregon returns 66% of net production per ESPN's Bill Connelly, headlined by junior quarterback Dante Moore turning down the NFL Draft. Eight starters return on defense, which Lanning calls himself, so the guide expects another elite unit. *(p. 170)*
- Recommendation is Over 10.5 wins, with the guide immediately conceding there is never much margin for error above 10.5, especially with trips to USC and Ohio State. *(p. 170)*
- It lists what must go right: the offensive line gelling, and young skill players stepping up from productive backup roles. *(p. 170)*
- Its case is that Oregon may have the most talented roster in college football with much to prove after two humiliating season endings, and it frames 2026 as potentially a show-no-mercy year with the best shot at a title. *(p. 170)*

*TTW reference notes summarising VSiN's analysis. GUIDE CONTENT — facts, conclusions and reasoning are the guide's; the wording is TTW's.*


## 2. Conference

**Big Ten** — see the conference file for league-wide context: [big_ten.md](../01_Conference_Database/big_ten.md)

Makinen projects **7.4–1.6** in conference play, ranking this team #2 of 18 in the Big Ten by power rating.

## 3. VSiN Team Rank / Conference Rank

- **National rank:** #2 of 138 *(p. 171)*
- **Conference rank:** #2 of 18 *(p. 171)*
- **Matt Youmans' Preseason Top 50:** **#2** *(pp. 16–20)*

## 4. Steve Makinen Power Rating

**68.5** — as printed on p. 171, reproduced exactly and not converted to any other scale.

Ranks #2 of 138 nationally and #2 of 18 in the Big Ten. Methodology is set out on pp. 46–47.

> No comparison against the TTW workbook is made here. That is a later-phase deliverable.

## 5. Home-Field Advantage Reference

- **Home field rating:** 3.9 *(p. 170)*
- **Road field rating:** 0.9 *(p. 170)*

Printed on the team page as `field ratings (HOME/ROAD)` and verified against the same values in the conference projected standings table. The guide's general treatment of home-field advantage is on p. 21.

## 6. Head Coach

**Dan Lanning** — 5th season at the programme *(p. 170)*.

## 7. Coordinator Notes

- **OC Will Stein** *(p. 170)*

## 8. Coaching Continuity / Changes

**Steve Makinen's Stability Score: 14** *(p. 43)*

| Component | Status | Points |
| --- | --- | --- |
| Head coach | returns | 4 |
| Offensive coordinator | new for 2026 | 0 |
| Defensive coordinator | returns | 3 |
| Starting quarterback | returns | 4 |
| Returning starters | 15 | 3 |

Scoring is the guide's own: head coach 4, offensive coordinator 3, defensive coordinator 3, starting quarterback 4, plus points for returning starters. A returning transfer quarterback counts as a **new** quarterback in this system *(p. 41)*.

- Dan Lanning is 48-8 since 2022, with playoff appearances in both years and a run through the Big Ten in Oregon's first conference season in 2024. *(p. 170)*
- Oregon starts as a victim of its own success, losing offensive coordinator Will Stein to Kentucky and Tosh Lupoi to Cal, both for head coaching jobs. Drew Mehringer and Chris Hampton are promoted internally. *(p. 170)*

Also referenced on **pp. 10, 18, 19, 20, 25, 83, 164** — not reproduced here.

## 9. Quarterback Situation

**Paul Stone ranks Dante Moore the #4 quarterback in the country** *(p. 45)*.

**Starting quarterback continuity:** the Stability Score table treats this team as having the same starting quarterback as 2025 *(p. 43)*. A transfer expected to start counts as new in that system.

**Returning starting quarterback:** yes — the guide marks returning quarterbacks with an asterisk on the returning-starters line *(p. 170)*.

- Despite the staff churn Oregon returns 66% of net production per ESPN's Bill Connelly, headlined by junior quarterback Dante Moore turning down the NFL Draft. Eight starters return on defense, which Lanning calls himself, so the guide expects another elite unit. *(p. 170)*
- **Can Dante Moore get it done when it matters most?** Moore is justifiably a Heisman favorite after 3,565 yards and 30 touchdowns in his first year starting, with abundant support: Jordon Davison and Dierre Hill Jr combined for 1,323 rushing yards as freshman backups, sophomore receivers Dakorien Moore and Jeremiah McClellan should start, and Evan Stewart returns from a knee injury. The guide expects monster numbers again but keeps the big-stage question open, citing the two meetings with Indiana in which Moore was sacked nine times with four turnovers, including a pick-six on the opening snap of the Peach Bowl. *(p. 171)*

Also referenced on **pp. 36, 140, 149, 295** — not reproduced here.

## 10. Returning Production

| | Returning starters |
| --- | --- |
| **Total** | 15 |
| **Offence** | 6 *(includes the returning quarterback)* |
| **Defence** | 9 |

Read by position from p. 170. The three figures sit under the printed labels `total / offense / defense`; they are matched to those labels by their place on the page, because the PDF text layer emits them in a different order than they are printed.

The Stability Score table prints the same figure independently: **15 returning starters** *(p. 43)*.

## 11. Transfer Portal

- **Will the defensive line be able to create more negative plays?** Oregon may have the country's best defensive line, returning tackles Bear Alexander and A'Mauri Washington plus edge Matayo Uiagalelei and linebacker Teitum Tuioti, who have 45.5 sacks between them over two seasons. The guide sets a counterweight against the gaudy totals: Oregon still struggled to get off the field, ranking 52nd nationally in third-down stops. Tackle transfers D'Antre Robinson from North Carolina and Jerome Simmons from Louisiana-Monroe are expected to help as run-stopping depth. *(p. 171)*


Referenced in the guide on **pp. 11, 83, 123, 129, 163, 164, 183, 281** — those passages are not reproduced here; see the pages for VSiN's own wording.

## 12. Recruiting / Roster Notes

Not addressed in guide under this heading.

The guide names this team on **pp. 45, 83, 129, 183, 251**, but no passage there was extracted under this heading; see those pages for VSiN's own wording.

## 13. Offensive Identity

- It lists what must go right: the offensive line gelling, and young skill players stepping up from productive backup roles. *(p. 170)*
- **Can Dante Moore get it done when it matters most?** Moore is justifiably a Heisman favorite after 3,565 yards and 30 touchdowns in his first year starting, with abundant support: Jordon Davison and Dierre Hill Jr combined for 1,323 rushing yards as freshman backups, sophomore receivers Dakorien Moore and Jeremiah McClellan should start, and Evan Stewart returns from a knee injury. The guide expects monster numbers again but keeps the big-stage question open, citing the two meetings with Indiana in which Moore was sacked nine times with four turnovers, including a pick-six on the opening snap of the Peach Bowl. *(p. 171)*
- **Can the offensive line hold up in the postseason?** Rimington Trophy finalist Iapani Laloulu anchors at center for a third season with Dave Iuli back at right guard, and three starters need replacing. The guide poses a genuine puzzle: this is the only line to be a Joe Moore Award finalist in each of the last three years, so why does it struggle in big games? Its evidence is the combined playoff exits of the last two seasons — 11 three-and-outs, 11 sacks allowed and 21 tackles for loss. Reaching a title game requires the group to be better when it matters. *(p. 171)*


Referenced in the guide on **pp. 10, 18, 25, 36, 83, 164, 182, 294** — those passages are not reproduced here; see the pages for VSiN's own wording.

## 14. Defensive Identity

- Despite the staff churn Oregon returns 66% of net production per ESPN's Bill Connelly, headlined by junior quarterback Dante Moore turning down the NFL Draft. Eight starters return on defense, which Lanning calls himself, so the guide expects another elite unit. *(p. 170)*
- **Will the defensive line be able to create more negative plays?** Oregon may have the country's best defensive line, returning tackles Bear Alexander and A'Mauri Washington plus edge Matayo Uiagalelei and linebacker Teitum Tuioti, who have 45.5 sacks between them over two seasons. The guide sets a counterweight against the gaudy totals: Oregon still struggled to get off the field, ranking 52nd nationally in third-down stops. Tackle transfers D'Antre Robinson from North Carolina and Jerome Simmons from Louisiana-Monroe are expected to help as run-stopping depth. *(p. 171)*


Referenced in the guide on **pp. 18, 20, 83, 149, 163, 182, 279, 281** — those passages are not reproduced here; see the pages for VSiN's own wording.

## 15. Key Strengths

National top-25 finishes in the guide's printed categories *(p. 171)*:

| Side | Category | Value | National rank |
| --- | --- | --- | --- |
| Offense | COMPLETION % | 71.4% | #2 |
| Defense | PASSING YARDS PER GAME | 155.1 | #2 |
| Defense | COMPLETION % | 51.7% | #3 |
| Defense | YARDS PER PASS ATTEMPT | 5.5 | #4 |
| Defense | TOTAL YARDS PER GAME | 275.8 | #5 |
| Defense | YARDS PER PLAY | 4.3 | #5 |
| Offense | YARDS PER PLAY | 6.7 | #6 |
| Defense | POINTS PER GAME | 18.2 | #11 |
| Offense | TOTAL YARDS PER GAME | 448.4 | #12 |
| Offense | POINTS PER GAME | 35.4 | #13 |
| Offense | YARDS PER RUSH ATTEMPT | 5.3 | #13 |
| Offense | YARDS PER PASS ATTEMPT | 8.6 | #13 |
| Offense | YARDS PER POINT | 12.7 | #18 |
| Defense | YARDS PER RUSH ATTEMPT | 3.5 | #19 |
| Offense | RUSH YARDS PER GAME | 194.7 | #22 |
| Offense | 3RD DOWN CONV. % | 45.2% | #24 |

## 16. Key Weaknesses

No category ranks inside the national bottom 25.

## 17. Schedule Overview

Schedule strength **47.28**, **#28 toughest of 138** *(p. 170)*.

| Date | Opponent | Site | Projected line | Opponent power rating |
| --- | --- | --- | --- | --- |
| 9/5 | BOISE ST | Home | -22 | 50.5 |
| 9/12 | at Oklahoma St | Away | -20.9 | 46.5 |
| 9/18 | PORTLAND ST | Home | -69 | 3.5 |
| 9/26 | at USC | Away | -5.6 | 60.0 |
| 10/10 | UCLA | Home | -24.1 | 48.0 |
| 10/17 | NEBRASKA | Home | -21.8 | 50.5 |
| 10/24 | at Illinois | Away | -15.1 | 51.0 |
| 10/31 | NORTHWESTERN | Home | -26.9 | 45.5 |
| 11/7 | at Ohio St | Away | +5.4 | 71.0 |
| 11/14 | MICHIGAN | Home | -12.6 | 59.0 |
| 11/20 | at Michigan St | Away | -25.7 | 42.5 |
| 11/28 | WASHINGTON | Home | -16.4 | 56.5 |

Projected lines and opponent power ratings are Makinen's, printed alongside the schedule on the team page.

## 18. Difficult Stretches / Trap Spots

Not addressed in guide.

## 19. Win Total Discussion

- **DraftKings win total:** 10.5 *(p. 149)*
- **Makinen projected wins:** 10.3 *(p. 149)*
- **Difference:** -0.2 wins against the posted number
- **Recommendation on the team page:** **OVER 10.5** *(p. 170)*

## 20. Futures / Conference / Playoff Discussion

Prices printed on the team page *(p. 171)*:

| Market | Price |
| --- | --- |
| CFP Championship | +800 |
| make the playoff | -320 |
| BIG TEN | +260 |

**VSiN staff predictions naming this team** *(p. 4)*:

| Category | Votes |
| --- | --- |
| BIG TEN CHAMPION | 13 of 22 |
| CFP FINAL FOUR #2 | 9 of 22 |
| CFP FINAL FOUR #3 | 3 of 22 |
| CFP FINAL FOUR #4 | 2 of 22 |
| CFP TITLE GAME #2 | 7 of 22 |
| CFP CHAMPION | 4 of 22 |

## 21. Betting Notes / Best Bets

**Host best bets naming this team** *(pp. 5–15)*:

| Contributor | Pick | Page |
| --- | --- | --- |
| Adam Burke | OREGON TO REACH BIG TEN CHAMPIONSHIP GAME (+100) | 11 |
| Stormy Buonantony | OREGON TO REACH BIG TEN TITLE GAME (+100) | 14 |

Where contributors disagree, every position is kept separately and none is reconciled.

- Recommendation is Over 10.5 wins, with the guide immediately conceding there is never much margin for error above 10.5, especially with trips to USC and Ohio State. *(p. 170)*
- Its case is that Oregon may have the most talented roster in college football with much to prove after two humiliating season endings, and it frames 2026 as potentially a show-no-mercy year with the best shot at a title. *(p. 170)*

Also referenced on **pp. 6, 11, 12, 14, 17, 38, 266, 317** — not reproduced here.

## 22. Historical / Situational Trends

- Dan Lanning is 48-8 since 2022, with playoff appearances in both years and a run through the Big Ten in Oregon's first conference season in 2024. *(p. 170)*
- What is missing is a national title, and the guide is pointed about how the last two seasons ended: blowout losses to the eventual champion in both, Ohio State in the Rose Bowl and Indiana in the Peach Bowl. *(p. 170)*


Referenced in the guide on **pp. 11, 12, 19, 20, 25, 38, 45, 83, 154, 166, 172, 176, 181, 281, 317** — those passages are not reproduced here; see the pages for VSiN's own wording.

## 23. Important Statistics

As printed on p. 171. Each category carries a value and a national rank.

### Offence

| Category | Value | National rank |
| --- | --- | --- |
| POINTS PER GAME | 35.4 | 13 |
| YARDS PER POINT | 12.7 | 18 |
| PLAYS PER GAME | 67.4 | 83 |
| TIME OF POSSESSION | 30:51 | 38 |
| 3RD DOWN CONV. % | 45.2% | 24 |
| TOTAL YARDS PER GAME | 448.4 | 12 |
| YARDS PER PLAY | 6.7 | 6 |
| RUSH ATTEMPTS PER GAME | 36.6 | 51 |
| RUSH YARDS PER GAME | 194.7 | 22 |
| YARDS PER RUSH ATTEMPT | 5.3 | 13 |
| PASS ATTEMPTS PER GAME | 29.4 | 87 |
| COMPLETION % | 71.4% | 2 |
| PASSING YARDS PER GAME | 253.6 | 35 |
| YARDS PER PASS ATTEMPT | 8.6 | 13 |
| TURNOVERS | 1.1 | 36 |

### Defence

| Category | Value | National rank |
| --- | --- | --- |
| POINTS PER GAME | 18.2 | 11 |
| YARDS PER POINT | 15.1 | 49 |
| 3RD DOWN CONV. % | 38.8% | 61 |
| TOTAL YARDS PER GAME | 275.8 | 5 |
| YARDS PER PLAY | 4.3 | 5 |
| RUSH YARDS PER GAME | 120.7 | 26 |
| YARDS PER RUSH ATTEMPT | 3.5 | 19 |
| COMPLETION % | 51.7% | 3 |
| PASSING YARDS PER GAME | 155.1 | 2 |
| YARDS PER PASS ATTEMPT | 5.5 | 4 |
| SACKS | 1.9 | 68 |
| TURNOVERS | 1.5 | 39 |

## 24. Bull Case

- The team page recommends **OVER 10.5**.
- Oregon starts as a victim of its own success, losing offensive coordinator Will Stein to Kentucky and Tosh Lupoi to Cal, both for head coaching jobs. Drew Mehringer and Chris Hampton are promoted internally. *(p. 170)*
- Its case is that Oregon may have the most talented roster in college football with much to prove after two humiliating season endings, and it frames 2026 as potentially a show-no-mercy year with the best shot at a title. *(p. 170)*

*1 further statement about this team carries both positive and negative language and is not classified to either side. It appears in full under the season outlook and *Open Questions / Risks*.*

> *Assembled from statements the guide makes about this team. The statements are GUIDE CONTENT with page references; the selection of which ones argue this side is PERSONAL INFERENCE.*

## 25. Bear Case

- Makinen projects **10.3 wins** against a posted total of 10.5 — **-0.2** against this team.
- What is missing is a national title, and the guide is pointed about how the last two seasons ended: blowout losses to the eventual champion in both, Ohio State in the Rose Bowl and Indiana in the Peach Bowl. *(p. 170)*

*1 further statement about this team carries both positive and negative language and is not classified to either side. It appears in full under the season outlook and *Open Questions / Risks*.*

> *Assembled from statements the guide makes about this team. The statements are GUIDE CONTENT with page references; the selection of which ones argue this side is PERSONAL INFERENCE.*

## 26. Open Questions / Risks

VSiN poses *Three Burning Questions for the 2026 Season* *(p. 171)*. Its analysis, in reference form:

### Can Dante Moore get it done when it matters most?

Moore is justifiably a Heisman favorite after 3,565 yards and 30 touchdowns in his first year starting, with abundant support: Jordon Davison and Dierre Hill Jr combined for 1,323 rushing yards as freshman backups, sophomore receivers Dakorien Moore and Jeremiah McClellan should start, and Evan Stewart returns from a knee injury. The guide expects monster numbers again but keeps the big-stage question open, citing the two meetings with Indiana in which Moore was sacked nine times with four turnovers, including a pick-six on the opening snap of the Peach Bowl. *(p. 171)*

### Can the offensive line hold up in the postseason?

Rimington Trophy finalist Iapani Laloulu anchors at center for a third season with Dave Iuli back at right guard, and three starters need replacing. The guide poses a genuine puzzle: this is the only line to be a Joe Moore Award finalist in each of the last three years, so why does it struggle in big games? Its evidence is the combined playoff exits of the last two seasons — 11 three-and-outs, 11 sacks allowed and 21 tackles for loss. Reaching a title game requires the group to be better when it matters. *(p. 171)*

### Will the defensive line be able to create more negative plays?

Oregon may have the country's best defensive line, returning tackles Bear Alexander and A'Mauri Washington plus edge Matayo Uiagalelei and linebacker Teitum Tuioti, who have 45.5 sacks between them over two seasons. The guide sets a counterweight against the gaudy totals: Oregon still struggled to get off the field, ranking 52nd nationally in third-down stops. Tackle transfers D'Antre Robinson from North Carolina and Jerome Simmons from Louisiana-Monroe are expected to help as run-stopping depth. *(p. 171)*

*TTW reference notes. The questions are VSiN's; the answers summarise VSiN's reasoning in TTW's wording.*

## 27. Source Conflicts

No source conflict identified for this team.

## 28. Relevant Page References

| Pages | Content |
| --- | --- |
| 170–171 | This team's two-page preview |
| 149 | Big Ten conference preview and projected standings |
| 4 | VSiN staff season predictions |
| 46–47 | Steve Makinen power rating methodology |
| 21 | Home-field advantage article |
| 5, 6, 7, 8, 10, 11, 12, 13, 14, 17, 18, 19, 20, 22, 25, 36, 38, 45, 46, 83, 115, 119, 123, 129, 137, 140, 150, 152, 154, 156, 162, 163, 164, 166, 168, 172, 174, 176, 178, 181, 182, 183, 184, 251, 265, 266, 279, 281, 294, 295, 317 | Other pages naming this team |

This team is named in **92 sentences across 54 pages** of the guide.

Every page naming this team outside its own spread: pp. 5, 6, 7, 8, 10, 11, 12, 13, 14, 17, 18, 19, 20, 22, 25, 36, 38, 45, 46, 83, 115, 119, 123, 129, 137, 140, 150, 152, 154, 156, 162, 163, 164, 166, 168, 172, 174, 176, 178, 181, 182, 183, 184, 251, 265, 266, 279, 281, 294, 295, 317.

## 29. Cross-Links

**Conference** — [Big Ten](../01_Conference_Database/big_ten.md)

**Master Index** — [Team Index](../00_Master_Index/03_Team_Index.md) · [Power Rating Index](../00_Master_Index/09_Power_Rating_Index.md) · [Coaching Index](../00_Master_Index/04_Coaching_Index.md) · [Quarterback Index](../00_Master_Index/06_Quarterback_Index.md)

**Every other database, for this team:**

- Head coach — [03_Coaching_Database/oregon_ducks.md](../03_Coaching_Database/oregon_ducks.md)
- Quarterback — [04_Quarterback_Database/oregon_ducks.md](../04_Quarterback_Database/oregon_ducks.md)
- Power rating — [05_Power_Ratings/00_MAKINEN_RATINGS.md](../05_Power_Ratings/00_MAKINEN_RATINGS.md)
- Win total — [06_Win_Totals/00_ALL_TEAMS.md](../06_Win_Totals/00_ALL_TEAMS.md)
- Futures board — [07_Futures/00_TEAM_FUTURES.md](../07_Futures/00_TEAM_FUTURES.md)
- Returning production — [08_Returning_Production/README.md](../08_Returning_Production/README.md)
- Schedule — [10_Schedule_Intelligence/00_BY_TEAM.md](../10_Schedule_Intelligence/00_BY_TEAM.md)
- Historical angles — [12_Historical_Trends/00_BY_TEAM.md](../12_Historical_Trends/00_BY_TEAM.md)
- Everything at once — [99_Search_Index/02_TEAM_LOOKUP.md](../99_Search_Index/02_TEAM_LOOKUP.md)

