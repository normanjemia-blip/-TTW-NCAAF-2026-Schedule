<!-- GENERATED FILE — do not hand-edit.
     Rebuild:  python3 _tools/build_teams.py
     Source:   2026 VSiN College Football Betting Guide (345 pp.) -->

# South Florida Bulls

> **Source: 2026 VSiN College Football Betting Guide, pp. 66–67** and every other page that names this team. GUIDE CONTENT throughout unless a line says otherwise. No outside research. Nothing inferred to fill a heading.


## 1. Program Snapshot

| | |
| --- | --- |
| **Conference** | American |
| **Guide pages** | 66–67 |
| **Head coach** | Brian Hartline (1st season) |
| **2025 straight up** | 9-4 |
| **2025 against the spread** | 9-4 |
| **2025 over/under** | 7-6 |
| **Makinen power rating** | 41.0 |
| **National rank** | #75 of 138 |
| **Conference rank** | #5 of 14 |
| **DraftKings win total** | 8.5 |
| **Makinen projected wins** | 8.4–3.6 overall, 4.8–3.2 in conference |
| **Schedule strength** | 31.46 (#112 toughest of 138) |

### Season outlook — VSiN's analysis in reference form (p. 66)

- South Florida proved a good stop for Alex Golesh, who posted three winning seasons in Tampa before moving up to Auburn. *(p. 66)*
- Brian Hartline takes over, described as the world's best wide receiver recruiter leaving Ohio State for his own programme, with the guide noting he has never coached anywhere else in college. *(p. 66)*
- His first task is sustaining Golesh's offensive standards. Five teams averaged 40-plus points last season and South Florida was one, averaging 49 in American play. *(p. 66)*
- The guide identifies what really set the ceiling: the defense improved at a similar pace to the offense. *(p. 66)*
- Recommendation is Under 8.5 wins, with a basically new roster and a first-time head coach. *(p. 66)*
- Context for the number: South Florida went 9-3 in the regular season with over 4,000 yards and 42 touchdowns from Byrum Brown, who has gone. *(p. 66)*
- Although the non-conference slate is substantially easier, the Bulls play Memphis and UTSA on short weeks, with Memphis coming off a bye. *(p. 66)*
- The guide makes a sharp market observation: South Florida could be favored in 10 or 11 games, yet the vig slants toward the Under, reflecting how many unanswered questions have the market down on the programme. Projection is 8.56 wins. *(p. 66)*

*TTW reference notes summarising VSiN's analysis. GUIDE CONTENT — facts, conclusions and reasoning are the guide's; the wording is TTW's.*


## 2. Conference

**American** — see the conference file for league-wide context: [american.md](../01_Conference_Database/american.md)

Makinen projects **4.8–3.2** in conference play, ranking this team #5 of 14 in the American by power rating.

## 3. VSiN Team Rank / Conference Rank

- **National rank:** #75 of 138 *(p. 67)*
- **Conference rank:** #5 of 14 *(p. 67)*
- **Matt Youmans' Preseason Top 50:** not ranked *(pp. 16–20)*

## 4. Steve Makinen Power Rating

**41.0** — as printed on p. 67, reproduced exactly and not converted to any other scale.

Ranks #75 of 138 nationally and #5 of 14 in the American. Methodology is set out on pp. 46–47.

> No comparison against the TTW workbook is made here. That is a later-phase deliverable.

## 5. Home-Field Advantage Reference

- **Home field rating:** 2.9 *(p. 66)*
- **Road field rating:** 0.3 *(p. 66)*

Printed on the team page as `field ratings (HOME/ROAD)` and verified against the same values in the conference projected standings table. The guide's general treatment of home-field advantage is on p. 21.

## 6. Head Coach

**Brian Hartline** — 1st season at the programme *(p. 66)*.

Profiled among the guide's new head coaches in The Coaching Carousel *(p. 29)*.

## 7. Coordinator Notes

- **OC Tim Beck** *(p. 67)*

## 8. Coaching Continuity / Changes

**Steve Makinen's Stability Score: 0** *(p. 44)*

| Component | Status | Points |
| --- | --- | --- |
| Head coach | new for 2026 | 0 |
| Offensive coordinator | new for 2026 | 0 |
| Defensive coordinator | new for 2026 | 0 |
| Starting quarterback | new for 2026 | 0 |
| Returning starters | 7 | 0 |

Scoring is the guide's own: head coach 4, offensive coordinator 3, defensive coordinator 3, starting quarterback 4, plus points for returning starters. A returning transfer quarterback counts as a **new** quarterback in this system *(p. 41)*.

- South Florida proved a good stop for Alex Golesh, who posted three winning seasons in Tampa before moving up to Auburn. *(p. 66)*
- Brian Hartline takes over, described as the world's best wide receiver recruiter leaving Ohio State for his own programme, with the guide noting he has never coached anywhere else in college. *(p. 66)*
- **What the Beck?** The guide explains why Hartline chose Beck — coaching since the late 1980s — while stating its own reservation openly: it took him 35 years to get a head job, Coastal Carolina did not go well at 20-18, and it personally finds him underwhelming as a coordinator. On the roster it notes most portal acquisitions did not play much previously, then supplies the counterweight: 247Sports graded South Florida's portal haul the best in the conference and 58th nationally with 44 commits — a lot of scouting and a lot of new faces. *(p. 67)*
- **Could the defense outpace the offense in 2026?** By contrast the guide rates the Josh Aldridge hire highly, noting he worked at conference rival East Carolina under Blake Harrell, whom it calls an outstanding defensive mind and good head coach. It then advances a conference-level thesis: stopping the run is the key in the American because most teams run well or use run-heavy schemes, and while everybody allows passing yards and 60-plus completion percentages, the teams that stop the run position themselves to succeed. Aldridge's East Carolina unit allowed 3.6 yards per carry and South Florida allowed 3.7. Its conclusion is that this may not be a polished defense but it is a big front seven. *(p. 67)*

Also referenced on **pp. 20, 54, 327** — not reproduced here.

## 9. Quarterback Situation

**Starting quarterback continuity:** the Stability Score table treats this team as having a new starting quarterback for 2026 *(p. 44)*. A transfer expected to start counts as new in that system.

**Returning starting quarterback:** no — the guide marks returning quarterbacks with an asterisk on the returning-starters line *(p. 66)*.

- Context for the number: South Florida went 9-3 in the regular season with over 4,000 yards and 42 touchdowns from Byrum Brown, who has gone. *(p. 66)*
- **Hype or hope?** The guide frames the roster philosophy: not all highly touted recruits make it, and the portal lets them drop to a Group of Six programme to get back on the radar. At South Florida most such players are quarterbacks — Hartline and coordinator Tim Beck added Michael Van Buren via Mississippi State and LSU, and Luke Kromenhoek via Florida State and Mississippi State. It credits Van Buren with genuine success, including an 8-to-2 ratio at LSU last season across four starts and relief work. It ends on the real question: with so many unproven players, whether Hartline is a programme builder or thrived on a foundation already laid at Ohio State. *(p. 67)*

Also referenced on **pp. 20, 45** — not reproduced here.

## 10. Returning Production

| | Returning starters |
| --- | --- |
| **Total** | 7 |
| **Offence** | 4 |
| **Defence** | 3 |

Read by position from p. 66. The three figures sit under the printed labels `total / offense / defense`; they are matched to those labels by their place on the page, because the PDF text layer emits them in a different order than they are printed.

The Stability Score table prints the same figure independently: **7 returning starters** *(p. 44)*.

## 11. Transfer Portal

- **Hype or hope?** The guide frames the roster philosophy: not all highly touted recruits make it, and the portal lets them drop to a Group of Six programme to get back on the radar. At South Florida most such players are quarterbacks — Hartline and coordinator Tim Beck added Michael Van Buren via Mississippi State and LSU, and Luke Kromenhoek via Florida State and Mississippi State. It credits Van Buren with genuine success, including an 8-to-2 ratio at LSU last season across four starts and relief work. It ends on the real question: with so many unproven players, whether Hartline is a programme builder or thrived on a foundation already laid at Ohio State. *(p. 67)*
- **What the Beck?** The guide explains why Hartline chose Beck — coaching since the late 1980s — while stating its own reservation openly: it took him 35 years to get a head job, Coastal Carolina did not go well at 20-18, and it personally finds him underwhelming as a coordinator. On the roster it notes most portal acquisitions did not play much previously, then supplies the counterweight: 247Sports graded South Florida's portal haul the best in the conference and 58th nationally with 44 commits — a lot of scouting and a lot of new faces. *(p. 67)*


Referenced in the guide on **pp. 289** — those passages are not reproduced here; see the pages for VSiN's own wording.

## 12. Recruiting / Roster Notes

- Brian Hartline takes over, described as the world's best wide receiver recruiter leaving Ohio State for his own programme, with the guide noting he has never coached anywhere else in college. *(p. 66)*
- **Hype or hope?** The guide frames the roster philosophy: not all highly touted recruits make it, and the portal lets them drop to a Group of Six programme to get back on the radar. At South Florida most such players are quarterbacks — Hartline and coordinator Tim Beck added Michael Van Buren via Mississippi State and LSU, and Luke Kromenhoek via Florida State and Mississippi State. It credits Van Buren with genuine success, including an 8-to-2 ratio at LSU last season across four starts and relief work. It ends on the real question: with so many unproven players, whether Hartline is a programme builder or thrived on a foundation already laid at Ohio State. *(p. 67)*

## 13. Offensive Identity

- His first task is sustaining Golesh's offensive standards. Five teams averaged 40-plus points last season and South Florida was one, averaging 49 in American play. *(p. 66)*


Referenced in the guide on **pp. 20, 45, 75, 289** — those passages are not reproduced here; see the pages for VSiN's own wording.

## 14. Defensive Identity

- The guide identifies what really set the ceiling: the defense improved at a similar pace to the offense. *(p. 66)*
- **Could the defense outpace the offense in 2026?** By contrast the guide rates the Josh Aldridge hire highly, noting he worked at conference rival East Carolina under Blake Harrell, whom it calls an outstanding defensive mind and good head coach. It then advances a conference-level thesis: stopping the run is the key in the American because most teams run well or use run-heavy schemes, and while everybody allows passing yards and 60-plus completion percentages, the teams that stop the run position themselves to succeed. Aldridge's East Carolina unit allowed 3.6 yards per carry and South Florida allowed 3.7. Its conclusion is that this may not be a polished defense but it is a big front seven. *(p. 67)*

## 15. Key Strengths

National top-25 finishes in the guide's printed categories *(p. 67)*:

| Side | Category | Value | National rank |
| --- | --- | --- | --- |
| Offense | TOTAL YARDS PER GAME | 486.3 | #2 |
| Offense | POINTS PER GAME | 38.6 | #5 |
| Offense | YARDS PER PLAY | 6.6 | #7 |
| Defense | TURNOVERS | 2 | #7 |
| Offense | YARDS PER PASS ATTEMPT | 8.8 | #11 |
| Offense | RUSH YARDS PER GAME | 208.8 | #12 |
| Offense | YARDS PER POINT | 12.6 | #15 |
| Offense | PASSING YARDS PER GAME | 277.6 | #15 |
| Offense | 3RD DOWN CONV. % | 46.8% | #16 |
| Offense | YARDS PER RUSH ATTEMPT | 5.2 | #16 |
| Offense | PLAYS PER GAME | 73.2 | #17 |
| Offense | COMPLETION % | 66.2% | #23 |

## 16. Key Weaknesses

National bottom-25 finishes in the guide's printed categories *(p. 67)*:

| Side | Category | Value | National rank |
| --- | --- | --- | --- |
| Offense | TIME OF POSSESSION | 26:06 | #133 |

## 17. Schedule Overview

Schedule strength **31.46**, **#112 toughest of 138** *(p. 66)*.

| Date | Opponent | Site | Projected line | Opponent power rating |
| --- | --- | --- | --- | --- |
| 9/5 | FLA INTERNATIONAL | Home | -15.5 | 28.5 |
| 9/12 | at Army | Away | +1.2 | 40.0 |
| 9/19 | DELAWARE ST | Home | -33.3 | 10.5 |
| 9/26 | at Bowling Green | Away | -13.4 | 26.5 |
| 10/3 | TEMPLE | Home | -10.7 | 34.0 |
| 10/8 | at TX-San Antonio | Away | +3.8 | 41.0 |
| 10/17 | KENT ST | Home | -21 | 23.5 |
| 10/31 | UAB | Home | -19 | 26.0 |
| 11/6 | at East Carolina | Away | +2.2 | 40.5 |
| 11/12 | MEMPHIS | Home | -2.2 | 41.0 |
| 11/21 | at Fla Atlantic | Away | -5.8 | 33.5 |
| 11/28 | TULANE | Home | -0.8 | 42.5 |

Projected lines and opponent power ratings are Makinen's, printed alongside the schedule on the team page.

## 18. Difficult Stretches / Trap Spots

- Although the non-conference slate is substantially easier, the Bulls play Memphis and UTSA on short weeks, with Memphis coming off a bye. *(p. 66)*


Referenced in the guide on **pp. 220** — those passages are not reproduced here; see the pages for VSiN's own wording.

## 19. Win Total Discussion

- **DraftKings win total:** 8.5 *(p. 49)*
- **Makinen projected wins:** 8.4 *(p. 49)*
- **Difference:** -0.1 wins against the posted number
- **Recommendation on the team page:** **UNDER 8.5** *(p. 66)*
- **Projection stated in the team-page essay:** 8.56 wins *(p. 66)*

**Selected as one of Steve Makinen's win-total bets** *(pp. 22–27)*: **UNDER 8.5**.

## 20. Futures / Conference / Playoff Discussion

Prices printed on the team page *(p. 67)*:

| Market | Price |
| --- | --- |
| CFP Championship | 600-1 |
| make the playoff | 11-1 |
| AMERICAN | +800 |

**VSiN staff predictions naming this team** *(p. 4)*:

| Category | Votes |
| --- | --- |
| AAC CHAMPION | 1 of 22 |

## 21. Betting Notes / Best Bets

- Recommendation is Under 8.5 wins, with a basically new roster and a first-time head coach. *(p. 66)*
- The guide makes a sharp market observation: South Florida could be favored in 10 or 11 games, yet the vig slants toward the Under, reflecting how many unanswered questions have the market down on the programme. Projection is 8.56 wins. *(p. 66)*

## 22. Historical / Situational Trends

- South Florida proved a good stop for Alex Golesh, who posted three winning seasons in Tampa before moving up to Auburn. *(p. 66)*
- Context for the number: South Florida went 9-3 in the regular season with over 4,000 yards and 42 touchdowns from Byrum Brown, who has gone. *(p. 66)*


Referenced in the guide on **pp. 45, 327** — those passages are not reproduced here; see the pages for VSiN's own wording.

## 23. Important Statistics

As printed on p. 67. Each category carries a value and a national rank.

### Offence

| Category | Value | National rank |
| --- | --- | --- |
| POINTS PER GAME | 38.6 | 5 |
| YARDS PER POINT | 12.6 | 15 |
| PLAYS PER GAME | 73.2 | 17 |
| TIME OF POSSESSION | 26:06 | 133 |
| 3RD DOWN CONV. % | 46.8% | 16 |
| TOTAL YARDS PER GAME | 486.3 | 2 |
| YARDS PER PLAY | 6.6 | 7 |
| RUSH ATTEMPTS PER GAME | 39.8 | 29 |
| RUSH YARDS PER GAME | 208.8 | 12 |
| YARDS PER RUSH ATTEMPT | 5.2 | 16 |
| PASS ATTEMPTS PER GAME | 31.6 | 61 |
| COMPLETION % | 66.2% | 23 |
| PASSING YARDS PER GAME | 277.6 | 15 |
| YARDS PER PASS ATTEMPT | 8.8 | 11 |
| TURNOVERS | 1.6 | 98 |

### Defence

| Category | Value | National rank |
| --- | --- | --- |
| POINTS PER GAME | 24.2 | 49 |
| YARDS PER POINT | 16.2 | 28 |
| 3RD DOWN CONV. % | 40.1% | 70 |
| TOTAL YARDS PER GAME | 392.4 | 78 |
| YARDS PER PLAY | 5.1 | 36 |
| RUSH YARDS PER GAME | 149.8 | 62 |
| YARDS PER RUSH ATTEMPT | 3.9 | 42 |
| COMPLETION % | 61.5% | 59 |
| PASSING YARDS PER GAME | 242.6 | 103 |
| YARDS PER PASS ATTEMPT | 6.6 | 37 |
| SACKS | 2.3 | 34 |
| TURNOVERS | 2 | 7 |

## 24. Bull Case

- South Florida proved a good stop for Alex Golesh, who posted three winning seasons in Tampa before moving up to Auburn. *(p. 66)*
- Brian Hartline takes over, described as the world's best wide receiver recruiter leaving Ohio State for his own programme, with the guide noting he has never coached anywhere else in college. *(p. 66)*
- The guide identifies what really set the ceiling: the defense improved at a similar pace to the offense. *(p. 66)*

*1 further statement about this team carries both positive and negative language and is not classified to either side. It appears in full under the season outlook and *Open Questions / Risks*.*

> *Assembled from statements the guide makes about this team. The statements are GUIDE CONTENT with page references; the selection of which ones argue this side is PERSONAL INFERENCE.*

## 25. Bear Case

- Makinen projects **8.4 wins** against a posted total of 8.5 — **-0.1** against this team.
- The team page recommends **UNDER 8.5**.
- The guide makes a sharp market observation: South Florida could be favored in 10 or 11 games, yet the vig slants toward the Under, reflecting how many unanswered questions have the market down on the programme. Projection is 8.56 wins. *(p. 66)*

*1 further statement about this team carries both positive and negative language and is not classified to either side. It appears in full under the season outlook and *Open Questions / Risks*.*

> *Assembled from statements the guide makes about this team. The statements are GUIDE CONTENT with page references; the selection of which ones argue this side is PERSONAL INFERENCE.*

## 26. Open Questions / Risks

VSiN poses *Three Burning Questions for the 2026 Season* *(p. 67)*. Its analysis, in reference form:

### Hype or hope?

The guide frames the roster philosophy: not all highly touted recruits make it, and the portal lets them drop to a Group of Six programme to get back on the radar. At South Florida most such players are quarterbacks — Hartline and coordinator Tim Beck added Michael Van Buren via Mississippi State and LSU, and Luke Kromenhoek via Florida State and Mississippi State. It credits Van Buren with genuine success, including an 8-to-2 ratio at LSU last season across four starts and relief work. It ends on the real question: with so many unproven players, whether Hartline is a programme builder or thrived on a foundation already laid at Ohio State. *(p. 67)*

### What the Beck?

The guide explains why Hartline chose Beck — coaching since the late 1980s — while stating its own reservation openly: it took him 35 years to get a head job, Coastal Carolina did not go well at 20-18, and it personally finds him underwhelming as a coordinator. On the roster it notes most portal acquisitions did not play much previously, then supplies the counterweight: 247Sports graded South Florida's portal haul the best in the conference and 58th nationally with 44 commits — a lot of scouting and a lot of new faces. *(p. 67)*

### Could the defense outpace the offense in 2026?

By contrast the guide rates the Josh Aldridge hire highly, noting he worked at conference rival East Carolina under Blake Harrell, whom it calls an outstanding defensive mind and good head coach. It then advances a conference-level thesis: stopping the run is the key in the American because most teams run well or use run-heavy schemes, and while everybody allows passing yards and 60-plus completion percentages, the teams that stop the run position themselves to succeed. Aldridge's East Carolina unit allowed 3.6 yards per carry and South Florida allowed 3.7. Its conclusion is that this may not be a polished defense but it is a big front seven. *(p. 67)*

*TTW reference notes. The questions are VSiN's; the answers summarise VSiN's reasoning in TTW's wording.*

## 27. Source Conflicts

No source conflict identified for this team.

## 28. Relevant Page References

| Pages | Content |
| --- | --- |
| 66–67 | This team's two-page preview |
| 49 | American conference preview and projected standings |
| 4 | VSiN staff season predictions |
| 46–47 | Steve Makinen power rating methodology |
| 21 | Home-field advantage article |
| 20, 27, 36, 45, 54, 75, 220, 266, 288, 289, 327 | Other pages naming this team |

This team is named in **13 sentences across 12 pages** of the guide.

Every page naming this team outside its own spread: pp. 20, 27, 36, 45, 54, 75, 220, 266, 288, 289, 327.

## 29. Cross-Links

**Conference** — [American](../01_Conference_Database/american.md)

**Master Index** — [Team Index](../00_Master_Index/03_Team_Index.md) · [Power Rating Index](../00_Master_Index/09_Power_Rating_Index.md) · [Coaching Index](../00_Master_Index/04_Coaching_Index.md) · [Quarterback Index](../00_Master_Index/06_Quarterback_Index.md)

**Every other database, for this team:**

- Head coach — [03_Coaching_Database/south_florida_bulls.md](../03_Coaching_Database/south_florida_bulls.md)
- Quarterback — [04_Quarterback_Database/south_florida_bulls.md](../04_Quarterback_Database/south_florida_bulls.md)
- Power rating — [05_Power_Ratings/00_MAKINEN_RATINGS.md](../05_Power_Ratings/00_MAKINEN_RATINGS.md)
- Win total — [06_Win_Totals/00_ALL_TEAMS.md](../06_Win_Totals/00_ALL_TEAMS.md)
- Futures board — [07_Futures/00_TEAM_FUTURES.md](../07_Futures/00_TEAM_FUTURES.md)
- Returning production — [08_Returning_Production/README.md](../08_Returning_Production/README.md)
- Schedule — [10_Schedule_Intelligence/00_BY_TEAM.md](../10_Schedule_Intelligence/00_BY_TEAM.md)
- Historical angles — [12_Historical_Trends/00_BY_TEAM.md](../12_Historical_Trends/00_BY_TEAM.md)
- Everything at once — [99_Search_Index/02_TEAM_LOOKUP.md](../99_Search_Index/02_TEAM_LOOKUP.md)

